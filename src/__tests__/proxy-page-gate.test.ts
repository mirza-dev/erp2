...
describe("page-gate — authenticated", () => {
    ...
    it("no-role authd kullanıcı = viewer → /dashboard 200 ama /dashboard/settings redirect", async () => {
        ...
        const blocked = await middleware(req("/dashboard/settings"));
        expect(blocked.status).toBeGreaterThanOrEqual(300);
        expect(blocked.status).toBeLessThan(400);
...
describe("page-gate — demo (viewer muamelesi)", () => {
    ...
    it("demo → /dashboard/settings redirect+forbidden (viewer'a kapalı)", async () => {
