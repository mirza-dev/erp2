                        ...
                        return (
                            <div style={{ background: "var(--surface-raised)", border: "var(--line-width) solid var(--surface-border)", borderRadius: "8px", overflow: "auto", boxShadow: "var(--surface-shadow-sm)" }}>
                                ...
                                {filteredDrafts.slice(previewPage * PAGE_SIZE, (previewPage + 1) * PAGE_SIZE).map((draft, rowIdx) => {
                                    ...
                                    return (
                                        <div key={draft.id} style={{ display: "grid", gridTemplateColumns: `72px repeat(${visibleFields.length}, minmax(150px, 1fr))`, borderBottom: rowIdx < filteredDrafts.length - 1 ? "var(--line-width) solid var(--border-tertiary)" : "none", background: rowHasMissing ? "var(--danger-bg)" : globalIdx % 2 === 0 ? "transparent" : "var(--bg-secondary)", minWidth: "760px" }}>
