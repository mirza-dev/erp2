...
export interface OrderFormLineInit {
    productId: string;
    productName: string;
    productSku: string;
    unit: string;
...
export interface OrderFormInitial {
    ...
    customerId: string;
    notes: string;
    quoteValidUntil: string; // "YYYY-MM-DD" veya ""
    lines: OrderFormLineInit[];
}
...
// Edit prefill: aktif ürün listesinde olmayan (pasif/silinmiş) bir satır ürününü
// kaybetmemek için sipariş satırı verisinden minimal Product stub üret. Stok
// alanları 0 → uyarı gösterilmez ama satır korunur ve select'te seçili kalır.
export function synthesizeProductStub(l: OrderFormLineInit): Product {
    return {
        id: l.productId, name: l.productName, sku: l.productSku, category: "",
        unit: l.unit, price: l.unitPrice, currency: "USD",
        on_hand: 0, reserved: 0, available_now: 0, quoted: 0, promisable: 0,
        incoming: 0, forecasted: 0, minStockLevel: 0, isActive: false,
        productType: "commercial", warehouse: "",
    };
}
...
export default function OrderForm({ mode, orderId, initial }: OrderFormProps) {
    ...
    // new mode: query-param prefill (customerId / customerName)
    const prefillIdRef   = useRef(searchParams.get("customerId"));
    const prefillNameRef = useRef(searchParams.get("customerName"));

    // Prefill — edit: initial'dan; new: query-param'dan. Müşteri + (edit'te) satırlar.
    // products/customers async yüklendiği için onlara bağlı; prefillDoneRef bir kez çalıştırır.
    useEffect(() => {
        if (prefillDoneRef.current) return;
        if (customers.length === 0) return;
        if (isEdit && products.length === 0) return; // edit satırları product lookup ister
        prefillDoneRef.current = true;

        if (isEdit && initial) {
            const cust = customers.find(c => c.id === initial.customerId);
            if (cust) setSelectedCustomer(cust);
            const stubs: Product[] = [];
            const mapped: OrderLine[] = initial.lines.map(li => {
                let product = products.find(p => p.id === li.productId) ?? null;
                if (!product) {
                    product = synthesizeProductStub(li);
                    stubs.push(product);
                }
                return {
                    id: safeRandomUUID(),
            ...
            if (stubs.length > 0) setExtraProducts(stubs);
            if (mapped.length > 0) setLines(mapped);
            return;
        }

        // new mode query-param prefill
    ...
    }, [customers, products, isEdit, initial]);
    ...
    // Satır ürünü seçenekleri = aktif ürünler + (edit'te) pasif satır stub'ları.
    const productOptions: Product[] = extraProducts.length > 0
        ? [...products, ...extraProducts.filter(s => !products.some(p => p.id === s.id))]
        : products;

    // Line operations
    const updateProduct = (lineId: string, productId: string) => {
        const product = productOptions.find((p: Product) => p.id === productId) || null;
    ...
    const buildOrderLines = (): OrderLineItem[] =>
        lines
    ...
    // new mode — addOrder (data-context POST)
    const buildAndSave = async (status: "draft" | "pending_approval") => {
    ...
    // edit mode — PUT /api/orders/[id]
    const handleSaveEdit = async () => {
        if (isDemo) { toast({ type: "info", message: DEMO_BLOCK_TOAST }); return; }
        setSubmitAttempted(true);
        if (!canSubmit || !selectedCustomer || !orderId) return;
        setIsSubmitting(true);
        try {
            const body = {
                ...
                customer_tax_office: selectedCustomer.taxOffice,
                customer_tax_number: selectedCustomer.taxNumber,
                currency,
                notes,
                quote_valid_until: quoteValidUntil || undefined,
                lines: buildOrderLines().map(l => ({
            ...
            if (!res.ok) {
                ...
                throw new Error(errBody.error || `Sipariş güncellenemedi (${res.status})`);
            }
            toast({ type: "success", message: "Sipariş güncellendi" });
            router.push(`/dashboard/orders/${orderId}`);
        } catch (err) {
            const msg = err instanceof Error ? err.message : "Sipariş güncellenemedi. Lütfen tekrar deneyin.";
            toast({ type: "error", message: msg });
        } finally {
            setIsSubmitting(false);
        }
    ...
    const primarySave = () => { if (isEdit) handleSaveEdit(); else buildAndSave("pending_approval"); };

    const backHref = isEdit && orderId ? `/dashboard/orders/${orderId}` : "/dashboard/orders";
    const breadcrumbLabel = isEdit ? (initial?.orderNumber ?? "Sipariş") : "Yeni Sipariş";
                                        ...
                                        <tr key={line.id} style={{ borderBottom: "0.5px solid var(--border-tertiary)" }}>
                                            ...
                                            <td style={{ padding: "8px 12px" }}>
                                                <input
                                                    ...
                                                    aria-label={`Satır ${idx + 1} birim fiyat`}
                                            ...
                                            <td style={{ padding: "8px 12px" }}>
                                                <input
                                                    ...
                                                    aria-label={`Satır ${idx + 1} iskonto yüzdesi`}
