 ...
 * Renkler concrete hex (self-contained); fiyat YOK — bu bir taleptir. Marka rengi
 * `data.accentColor`dan (company_settings.document_accent_color, mig.112).
 */
import { RFQ_LABELS, fmtRfqDate, type RfqDocData } from "@/lib/rfq-document-helpers";
import { DEFAULT_DOCUMENT_ACCENT, resolveDocumentAccent } from "@/lib/document-accent";

const INK = "#1a2230";
const MUTED = "#5b6573";
const BORDER = "#d0d5dd";

/** Arşiv sarmalayıcısının `<style>`ı. `accent` yalnız `resolveDocumentAccent` çıktısı (#RRGGBB). */
export function rfqPageCss(accent: string = DEFAULT_DOCUMENT_ACCENT): string {
    return `
...
.rfq-doc th { background: ${accent}; color: #fff; font-weight: 600; }
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 18 }}>
                <div>
                    {data.logoSrc ? (
                        ...
                        <div style={{ fontFamily: "var(--font-doc-heading, 'Montserrat')", fontWeight: 800, fontSize: 20, color: accent }}>
                            {data.sellerName || "—"}
                        </div>
                    )}
                ...
                <div style={{ textAlign: "right" }}>
                    <h1 style={{ fontSize: 18, color: accent, margin: "0 0 8px" }}>{RFQ_LABELS.title}</h1>
