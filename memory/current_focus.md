...
## Son Tamamlanan İş — 2026-05-29 (Teklif V7 Faz 3 IMPLEMENT EDİLDİ — header iskonto, 3799 test, COMMIT BEKLİYOR + migration APPLY BEKLİYOR)
...
- **Migration (APPLY EDİLMEDİ):** `070_quotes_discount.sql` kolon `numeric(15,2) not null default 0` (mevcut teklifler 0 → etkilenmez, legacy snapshot korunur). `071_quotes_rpc_discount.sql` 069 RPC'leri üzerine CREATE OR REPLACE — `discount_amount` payload (COALESCE 0) + **V3-A6 draft guard** (update_quote_with_lines başı: status<>'draft' → `42501` RAISE, belt-and-suspenders; route zaten 409 ön-kapısı koyuyor [id]/route.ts:80). V7-A1 SECURITY DEFINER YOK, V7-A2 NULLIF korundu.
