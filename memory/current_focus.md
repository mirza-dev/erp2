...
**Yapılanlar (A→G):**
- **A backend (gerçek COGS):** `supabase/migrations/087_dashboard_monthly_cogs.sql` — `dashboard_monthly_cogs(p_start date)` RPC `Σ(order_lines.qty × products.cost_price)` ay+ürün-para bazında, iptal/taslak hariç (ciro filtresiyle aynı), `security definer`+revoke/grant. `src/lib/supabase/dashboard-finance.ts` (`dbGetMonthlyCogs`). **`GET /api/dashboard/finance`** — `getCurrentUserPermissions`; `reportingCurrency`=`company_settings.currency` (default USD); `view_purchase_costs` varsa son-12-ay COGS, yoksa `cogs:null`+`canViewCosts:false`. **CANLI DOĞRULANDI:** API gerçek EUR+USD COGS döndü (`{2026-05 EUR 12000, USD 48532, 2026-06...}`) → **migration 087 KULLANICI APPLY ETMİŞ ✅.**
- **B view-model (`dashboard-view-model.ts` yeniden yazıldı, no-silent-delete):** **tek raporlama para birimine normalizasyon** (`toReporting` orta kur=(buying+selling)/2, eksik kur→değişmez defansif; eski `MoneyByCurrency`/`addMoney`/`dominantCurrency`/`formatMoneyByCurrency` KORUNDU=test edilebilir, kullanılmıyor). `monthlyRevenueReporting`/`monthlyOrderCounts`/`cogsToReporting`/`stockValueByCategoryReporting`/`receivablesAging`(dürüst, siparişten türev)/`financeSummary`/`productionDailySeries`(gerçek scrap)/`recentOrdersView`(normalize+RBAC)/`aiPointsFromOpsSummary`/`currencySymbol`/`formatReportingM/Compact`. **`buildKpis` → 6 KPI** (Açık Alacak eklendi). **RBAC:** sales→ciro/stok/donut/tutar; purchase_costs→maliyet/Finansal Özet; financial_summary→Açık Alacak/aging; yetkisiz→"—".
- **C üretim fire (gerçek, mevcut kolon):** `UretimKaydi += scrap`; `mapProductionEntry += scrap: row.scrap_qty ?? 0` (`production_entries.scrap_qty` mig.001'den beri var, `/api/production` `select("*")` → ek fetch yok). production page `addUretimKaydi`'ye `scrap:0`.
- **D bileşenler:** `KpiCard` sadık (sağ-üst ok + trend delta ikonu + sparkline); **`FinancePanel`** (brüt kâr hero + money-flow bar + AgingBars + **dürüst etiket**: "Faturalanan siparişlerden tahmini · ödeme entegrasyonu beklemede"; `canViewCosts` prop → yetki-yok vs veri-yok AYRI mesaj [substrate honesty, cold-start "yetki yok" yanıltması düzeltildi]); **`ProductionPanel`** (good/scrap BarChart); **`AiPanel`** (collapsible kapalı, açılınca lazy `/api/ai/ops-summary` fetch + 15dk cache + `aiPointsFromOpsSummary` → headline + 2-kolon tonlu maddeler + "Tüm analizi gör"); `OverviewPanel`+=`onToggle`; `AgingBars`+=`symbol`. **HeatPanel/MarginGauge KURULMADI** (tasarım düzeninde render edilmiyor). Stok/Reorder/Alerts/Orders/charts reuse.
- **E page.tsx (sadık `DashDetailed`):** mount'ta `/api/dashboard/finance`+`/api/exchange-rates` fetch (defansif). PageHeader (segment **dekoratif** local-state + "Rapor indir"→`window.print()`) → kpi-strip(6) → "Ciro & Maliyet Trendi"(`showCost`) → `overview-grid-1-1` [Finance·Production·Orders | Stock·Reorder·Alerts] → AiPanel. **shell.jsx PORT EDİLMEDİ.**
- **F css:** layout/badge/seg sınıfları + `--chart-*` zaten vardı (önceki tur). **G testler:** `dashboard-view-model`(33, normalizasyon+6KPI+receivables+production+aiPoints+RBAC iki yön), `overview-panels-render`(8 YENİ, Finance/Production/AiPanel renderToStaticMarkup+hex-yok+yetki/veri ayrımı), `dashboard-finance-route`(YENİ, RBAC cogs null/dolu+reportingCurrency), `overview-charts-render`/`dashboard-overview-preservation`/`dashboard-collapsible-sections` güncellendi. **no-silent-delete:** link kartları kalktı ama Sidebar'da "Stok & Ürünler"/"Veri Aktarım Merkezi" linkleri korunur (işlev kaybı yok); StatsCards/StockDataGrid/AIAlerts/RecentOrders/AISummaryCard dosyaları repoda kalır. Dashboard salt-bakış → `faz7-ui-masking`+`button-source-regression` "Yeni Sipariş CTA" beklentileri güncellendi (CTA liste sayfasında).

**ADVISOR done-check (push-blocker yakaladı, green suite göremezdi):** brüt kâr/marj KDV-dahil `grandTotal` ile vergisiz COGS karışıyordu → marj şişiyordu. Fix: `grossToNetRevenue(g)=g/1.20` (KDV sabit %20, domain), `financeSummary` NET ciro tabanı; KPI ciro+trend BRÜT kalır; FinancePanel "Ciro"→"Net Ciro". Görsel: marj %53→**%44** (doğru). KDV-tuzağı testi (`financeSummary(1200,710)` şişer / `financeSummary(grossToNetRevenue(1200),710)`=%29). tsc 0 · lint 0 · build 0 (`ƒ /api/dashboard/finance` + `○ /dashboard` + `ƒ Proxy`) · **4965 test** (4942→). **DURUM: GREEN, PUSH BEKLİYOR** + migration 087 APPLY EDİLDİ ✅ (canlı API kanıtı). **Bilinen tasarım sapması (kasıtlı, danışman onaylı):** segment/Rapor-indir dekoratif (tasarımda `range` tüketilmiyor, butonun onClick'i yok) → backend filtreleme/rapor-export YOK; Alacak = fatura/ödeme tablosu yok → siparişten türev dürüst etiketle. **Sıradaki:** mirror push onayı (kullanıcı).

<details><summary>Önceki: Uyarılar → Takvim Görünümü — Faz 3 (cila + responsive + a11y + animasyon) (PUSH `ddac29d`)</summary>

`design_handoff_alerts_calendar/` Apple-Calendar takvim işinin SON fazı. **Plan:** `ALERTS_CALENDAR_PLAN.md` Faz 0–3 TAMAMLANDI. **Çerçeve (advisor dürüstlük):** Faz 3'ün ~%80'i Faz 1'de gönderilmişti (cal-fade/cal-pop-in/cal-fade-up stagger index×55ms + slide-in-right + `@media(prefers-reduced-motion)` `*` guard + 260ms hover popover + pointer-events:none + üst/alt auto konum + role=tablist/aria-selected + aria-label/aria-pressed + popover aria-hidden + tema-uyumlu #fff yalnız accent yüzeyde; drawer zaten min(480px,100vw)+role=dialog+aria-modal+ESC). **Bu tur gerçek kalan 4 kalem:** **(1) Responsive** (`src/app/dashboard/alerts/page.tsx`): `windowWidth` state SSR-safe init (`typeof window!=="undefined"?innerWidth:1200`) + resize listener `useEffect`, **OrderForm precedent** (`isMobile=windowWidth<768`); inline-specificity sorununu `!important`siz çözer. `<768` → `layoutStyle` spread override `{gridTemplateColumns:"1fr", height:"auto", overflow:"visible"}` + `dayPanelStyle` `{borderLeft:none, borderTop:..., maxHeight:"50vh"}`. **ADVISOR KRİTİK:** asıl risk grid değil **height/overflow modeli** — `height:calc(100vh-88px)`+`overflow:hidden` mobilde kalırsa yığılmış grid+panel KIRPILIR/scroll olmaz → mobilde `height:auto`+`overflow:visible` (doküman scroll'una bırak). **ADVISOR done-check 2. yakalama (uygulandı):** dış `layoutStyle` yetmez — İÇ konteynerler de fixed-height modeline göreydi (takvim kolonu `overflow:hidden`, grid div `flex:1+overflowY:auto+minHeight:0`); mobilde parent `height:auto` olunca `flex:1` dolacak tanımlı alan yok → tarayıcıya-bağlı çökme/kırpma (jsdom göremez). Fix: takvim kolonu `overflow:isMobile?"visible":"hidden"`, grid div mobilde `{flex:"none"}` (overflowY:auto DÜŞER) → doküman akışına girer. Inert `.alerts-calendar-layout`/`.alerts-day-panel` className'leri KORUNDU (`feedback_no_silent_deletes`, zararsız hook). **(2) Drawer focus-dönüşü** (`AlertCalendarDrawer.tsx`): yeni `useEffect` ([] dep) — mount'ta `prevFocus=document.activeElement` capture + `closeBtnRef.current?.focus()` (Kapat butonuna `ref`, Button forwardRef<HTMLButtonElement> doğrulandı), cleanup'ta `prevFocus?.focus?.()`. Full focus-trap (cycling) DROP=repo-tutarlı (aria-modal purist boşluk, bloklamaz). **(3) Tema-audit:** alert bileşenlerinde `var(--...)` dışı hex YOK (yalnız onaylı #fff = DayCell bugün dairesi + ClassificationTabs count badge, ikisi de accent üzerinde) → test ile kilit. **(4) Backdrop `fade-in 0.2s ease-out`** (mevcut keyframe; reduced-motion `*` kapsar). **+6 davranışsal test** (`alerts-calendar-faz3.test.tsx`, RTL/jsdom, `order-form-interaction` precedent): responsive masaüstü `1fr 380px`+overflow:hidden / mobil(390) `1fr`+height:auto+overflow:visible+panel maxHeight:50vh / resize(1200→500) event → 1fr; focus-dönüşü trigger→drawer aç(Kapat odak)→unmount→trigger geri; tema hex-yok 6 dosya; reduced-motion guard globals.css. **ADVISOR caveat:** jsdom layout engine'i yok → görsel kırpılma yalnız tarayıcı-smoke, test koşullu DEĞERLERİ kilitler. tsc 0 · lint 0 · build 0 (`ƒ Proxy`) · **4899 test** (+6). **DURUM: PUSH EDİLDİ `ddac29d`** (`origin/main == origin/codex-experiment == ddac29d` mirror doğrulandı `git ls-remote`, iki Coolify deploy; test status check bypass; React Doctor advisory bloklamadı; commit yalnız 6 Faz 3 dosyası — `dashboard/` junk + `.claude/settings.local.json` HARİÇ). **Sıradaki:** tarayıcı smoke (mobil tek-kolon TAM scroll [kırpılma yok] + drawer kapanınca focus tetikleyiciye döner + koyu/aydınlık + reduced-motion'da animasyon kapalı).

</details>

<details><summary>Önceki: Faz 2 (drawer zenginliği: inline formlar + related-orders)</summary>
...
**Kullanıcı kararları (AskUserQuestion):** dil = **yerel TR/EN sözlük** (`STR`+login-only `useState`, global i18n YOK); Google = **Supabase `signInWithOAuth`** (**Clerk REDDEDİLDİ** — uygulama tümüyle Supabase Auth: RLS/`proxy.ts` `sb-*`/RBAC `app_metadata.roles`/demo cookie üzerine kurulu, ikinci stack altyapı migrasyonu olur); reset = **`resetPasswordForEmail`**; Beni hatırla = **kozmetik** (kendi kararım, varsayılan açık, session-persistence bağlanmadı).

**ADVISOR kritik (prod-kıran, done-öncesi yakalandı):** `@supabase/ssr` PKCE → OAuth dönüşündeki `?code=` **server-side exchange ŞART**. `redirectTo:/dashboard` doğrudan kullanılsaydı `proxy.ts` session-cookie henüz yokken görür → 307 `/login`'e geri atar, `/dashboard` client JS hiç yüklenmez, code asla exchange edilmez → buton sessizce çalışmaz (provider etkinleştirmek ÇÖZMEZ). Çözüm: yeni **`GET /auth/callback`** route `exchangeCodeForSession(code)` + relative `Location` (Coolify/Traefik reverse-proxy host pass-through fix, `parasut/oauth/callback` precedent'i) + `proxy.ts` `ALWAYS_PUBLIC`'e eklendi.

**Yapılan dosyalar:**
- **`src/app/globals.css`** — eski `.login-shell` kart bloğu (~287 satır) → **monolith CSS** (`.login-monolith`/`.monolith-hex`/`.mono-*`/`.login-chrome`/`.seg`/`.icon-btn`/`.input-wrap`/`.check`/`.login-divider`/`.login-foot` + `lg-panel`/`lg-shake` keyframes). Token'lar zaten var (eklenmedi), `.spinner`+`@keyframes spin` global korundu, `prefers-reduced-motion` guard korundu. Eski class'lar yalnız login+testi tüketiyordu (silme gerekçeli). Splice marker'larla yapıldı.
- **`src/app/login/page.tsx`** — tam yeniden yazım. `<ThemeProvider>` ile sarıldı → `useTheme` login'de çalışır (login dashboard dışında, context yoktu; `applyDom` `<html data-theme>`'i günceller). `STR` TR/EN sözlük + `lang` state. Chrome (dil seg + tema butonu `resolved`→Sun/Moon). `RovenLogo size=32/27 gap=8` + `mono-brand-tag`. Form: Mail/Lock lead ikonlu `input-wrap`, göster/gizle, Beni hatırla `role=checkbox` Space/Enter, Şifremi unuttum `handleForgot`. Primary+Google için **`Button` reuse** (`.btn*` CSS gerekmez). Google "G" çok-renkli SVG = **tema-muaf** (marka renkleri, logo precedent). `?error=oauth` mount'ta okunup `authError`'a (Suspense'siz, `window.location.search`). Forgot başarısı inline `notice` (`role=status`; login'de ToastProvider yok).
- **`src/app/auth/callback/route.ts`** (YENİ) — code varsa exchange→307 `/dashboard`, yoksa/hata→307 `/login?error=oauth`. `server.ts` route handler'da cookie yazar (doğrulandı).
- **`src/proxy.ts`** — `ALWAYS_PUBLIC` += `/auth/callback`.
- **`eslint.config.mjs`** — `design_handoff_*` globalIgnores'a (referans prototip = production değil; `.agents`/`skills` paterni).
- **Testler:** `login-page.test.tsx` (matchMedia mock + OAuth/reset mock + 6→11 test: validation/lang-EN/tema-toggle/Google/forgot), `auth-callback-route.test.ts` (YENİ, +3: code→exchange+307, hata/yok→/login?error=oauth).

**Doğrulama:** tsc 0 · lint 0 · **4812→4821 test** (+9) · build 0 (`/auth/callback` + `ƒ Proxy` mevcut). Migration/backend YOK.

**+ Davetiye-bazlı erişim kilidi** (kullanıcı: "kayıtlı olmayan giremesin bile" — Google açılınca her hesap viewer olarak giriyordu, ticari liste sızıntısı). İki katman: **(1) BİRİNCİL = Supabase "Allow new users to sign up" OFF** (kullanıcı yapar); **(2) kod `isProvisionedUser`** (permissions.ts pure helper) — admin createUser her zaman `app_metadata.roles` set eder, self-signup'ta yok → temiz ayrım; `proxy.ts` provize değilse `/api/**`→403, sayfa→`/login?error=unauthorized`. **ADVISOR yıkıcı boşluk yakaladı:** `/api/seed` ALWAYS_PUBLIC→guard bypass + `!!user` → self-signup DB silebilirdi → seed session-branch **admin-only** (`parseRoles().includes("admin")`). +13 test (isProvisionedUser/proxy/login/4 cascade fixture). **4821→4834.**
...
**Yapılan (commit zinciri `f3cb649` → `1b760d6` → `079df51` → `7861594` → `2fd2ef6` → `e8f44c4`):**
...
- **landing `page.tsx`** `<RovenLogo size={17} wordmarkSize={15} />` (global gap 5 alır) · **login `page.tsx`** `size={32} wordmarkSize={26} gap={6}` (kullanıcı isteğiyle büyütülüp yaklaştırıldı; explicit → global'den izole).
...
**Kayıp yok:** Uyarılara erişim Sidebar'daki "Uyarılar" nav öğesinde aynı `activeAlertCount` rozetiyle `/dashboard/alerts`'e bağlı kalır (`Sidebar.tsx:103`).

**Uygulama (4 dosya):**
- `Topbar.tsx` — uyarı `<Link>` bloğu + ölü `AlertTriangle`/`useData` import + `activeAlertCount` destructure SİLİNDİ (`Menu` kalır). Sayfa başlığı `.topbar-page-context` ortadan kalktı → markanın yanına (`.topbar-divider` ince dikey ayraç + `.topbar-page-title` sessiz metin). Sağ küme `.topbar-right-extras` (ticker+health) + avatar ayrı.
- `ExchangeRatesTicker.tsx` — düzleştirme: `tickerStyle` dış kutu border/bg + `sourceStyle` accent rozet + per-kur `rateStyle` dolu pill + `ratesStyle`/`labelStyle` ("A/S") + `SYMBOLS` kaldırıldı → kurlar düz metin (`USD 40,123 / 40,288 · EUR …`); `codeStyle`/`valueStyle` korundu. **Veri/precision değişmez** (3 ondalık, USD+EUR, 20dk refresh, `isRatePayload`, `null→render yok`). Kaynak `title` tooltip + aria-label'da.
- `SystemHealthIndicator.tsx` — dolu border+bg pill → 8px renkli nokta + etiket; `metaForState`'ten `icon`/`border`/`background` çıktı; lucide importları silindi; "checking" `pulse-dot` animasyonu (mevcut keyframe).
- `globals.css` — `.topbar-shell` grid→flex (`justify-content:space-between`); `.topbar-page-title` pill→sessiz metin; yeni `.topbar-divider` + `.topbar-right-extras` (tek küme yüzeyi: subtle bg + tek border + radius); mobil blok `grid-template-columns`/`.topbar-page-context` temizlendi.
...
**Backend/migration/permission DEĞİŞMEDİ.** tsc 0 · lint 0 · **4616→4627 test** · build 0 (`ƒ Proxy`). 3 dosya değişti (page.tsx + AIAlerts.tsx + AISummaryCard.tsx) + 1 yeni test + CLAUDE.md/memory. **DURUM: PUSH EDİLDİ (`44d4e54`) — iki branch birebir aynı, iki Coolify prod deploy.**
...
- **#1 [HEADLINE — fonksiyonel boşluk] Alan düzenleme UI'ı YOK'tu (danışman yakaladı, kapsam kullanıcıya soruldu → "Evet ekle"):** detay tablosu alanın etiket/tip/birim/seçeneklerini yalnız düz metin gösteriyordu; per-row yalnız zorunluluk-toggle + ↑↓ + Sil. `PATCH /api/product-types/[id]/fields/[fieldId]` + `dbUpdateProductTypeField` düzenlemeyi **tam destekliyordu** ama UI sadece `{required}`/`{sort_order}` gönderiyordu → yazım hatası = sil+yeniden ekle = o tipteki TÜM ürünlerde attribute kaybı. Fix: satır-içi **"Düzenle"** → edit modal (`role=dialog`+`aria-modal`+`aria-labelledby="edit-field-title"`); `editDraft`/`editFieldId` state; label_tr(zorunlu)/label_en/field_type/unit/options düzenlenebilir; **`field_key` READ-ONLY** (footgun: `attributes` JSONB kimliği → orphan riski) + tip-değiştirme caveat'ı (`var(--warning-text)`); PATCH body `{label_tr,label_en,field_type,unit,options}`. Paylaşılan saf **`buildFieldPayload(draft)`** (Ekle+Düzenle DRY; select→options array, non-select→null, number→unit, non-number→null=stale unit önler).
- **#2 [MED] Liste N+1 fetch + sessiz yanlış veri:** `loadTypes` her tip için ayrı `?withFields=1` çağırıyordu (8 tip=9 istek) yalnız fieldCount için; kısmi fail→16-alanlı Vana "0 alan". Fix: `dbListProductTypes` `.select("*, product_type_fields(count)")` nested-count + unwrap (`[{count:N}]`→`fieldCount:N`, **additive** — ProductTypeRow reshape edilmez; product-create selector + import extract/classify etkilenmez); cache 60s+tag + 4 field route `revalidateTag` doğrulandı; `page.tsx` N fetch kaldırıldı (9→1 istek).
- **#3 [MED] Yıkıcı `confirm()` → a11y modal:** `deleteType`+`deleteField` `window.confirm()` → state + inline `role=dialog`+`aria-modal`+`aria-labelledby` modal (deleteField "ürünlerde alan kaybolur" uyarısı); `window.location.href`→`router.push`.
- **Çürütülen iddia (danışman flagledi→şema okundu):** reorder `(i+1)*10` sort_order çakışır mı? — HAYIR (056'da `UNIQUE(product_type_id, sort_order)` YOK; yalnız non-unique index + `UNIQUE(product_type_id, field_key)`).
...
## Önceki — 2026-06-04 (**Satın Alma Siparişi (Yeni) — ürün seçilince birim fiyat + KDV otomatik gelmiyor [FONKSİYONEL bug]**)

Kullanıcı `/dashboard/purchase/orders/new` formunda "ürün seçilince otomatik birim fiyat bilgisi gelmiyor ve hesaplanma yapılmıyor kdv tutarıyla beraber" dedi + birkaç ekran görüntüsü gönderdi. **Ekran görüntüleri macOS TCC (`TemporaryItems` korumalı klasör) nedeniyle OKUNAMADI** — Read/cp/cp-sandbox-disabled hepsi `Operation not permitted`; kullanıcıya bildirildi (görüntüyü terminale sürükle veya projeye kaydet) + **AskUserQuestion ile sayfa "Satın Alma Siparişi" olarak kesinleşti.** "Önce doğrula sonra düzelt" + danışman triyajı.

**Kök neden (danışman onayladı):** `new/page.tsx:274` ürün `<select>`'inin `onChange`'i yalnız `product_id` set ediyor, `unit_price`'a HİÇ dokunmuyor → varsayılan `"0"` kalıyor → `grandTotal` (`:144`) `qty*price*(1-disc/100)` ile hesaplandığından price=0 → satır toplamı 0 → "Tahmini Toplam (KDV dahil)" `0,00` kalıyor. **KDV ayrı bug DEĞİL — fiyatın 0 kalmasının türevi.** Karşılaştırma: OrderForm:194 koşulsuz `unitPrice: product.price` set ediyor (çalışıyor); QuoteForm:442 currency-guard'lı set ediyor. **#2 ekran görüntüsü ("böyle bir hata alıyorum") GÖRÜLMEDİ** — muhtemelen submit validasyonu `:169` "birim fiyat sıfırdan büyük olmalı" = aynı kök neden, ama VARSAYIM (test/probe görülmeyen görüntüyü doğrulayamaz).

- **#1 (HEADLINE — fonksiyonel) unit_price auto-fill yok:** yeni pure helper `pickPurchaseUnitPrice(product)` = ürünün **`cost_price`**'ı (satın alma = MALİYET fiyatı; `price` SATIŞ fiyatıdır, **fallback KULLANILMADI** — danışman footgun: yanlış/yüksek değer PO'yu şişirir; `cost_price` null → boş string, kullanıcı elle girer, validasyon zaten 0/negatifi reddediyor). Yeni `handleProductSelect(idx, productId)` `product_id`+`unit_price` birlikte set eder; `<select>` `onChange` buna yönlendirildi (eski salt-`product_id` onChange kaldırıldı).
- **#2 (para birimi güvenliği):** `cost_price` `product.currency` cinsinden, PO currency tedarikçiden (`:132`) — farklıysa otomatik dolan rakam yanlış birimde (danışman: sessiz şişirme riski). Karar **koşulsuz doldur + görünür uyarı** (QuoteForm'un sessiz boş-bırakması kullanıcının asıl şikâyetini tekrar üretir; OrderForm zaten koşulsuz dolduruyor = kullanıcının "çalışıyor" saydığı patern): fiyat koşulsuz dolar + `product.currency !== currency` satırlarda inline `var(--warning-text)` uyarı ("Ürün fiyatı {X} cinsinden — PO {Y}, kontrol edin"). Otomatik kur çevrimi YOK (kur verisi yok).
- **#3 (görünürlük — "kdv tutarı" şikâyetine doğrudan yanıt):** form yalnız tek "Tahmini Toplam (KDV dahil)" gösteriyordu → yeni pure helper `computePoTotals(lines)` ({subtotal/discount/vat/grand}, KDV %20 sabit, qty/price=0 satır atlanır = eski `grandTotal` davranışı; `grandTotal` useMemo `totals = computePoTotals` ile değiştirildi — tek hesap kaynağı). Alt blok: Ara Toplam · İskonto (varsa) · KDV (%20) · **Tahmini Genel Toplam (KDV dahil)** + "Kesin tutar sipariş kaydedilince hesaplanır" notu (danışman: kaydedilmemiş form = tahmin, sunucu RPC otoritatif → "Tahmini" çerçevesi korundu, "Genel Toplam"a indirgenmedi).
- **Veri doğrulaması (danışman'ın "fix no-op olur mu?" KRİTİK uyarısı):** `/api/products?all=1` `redactProductsForPerms` ile redakte (cost_price ← `view_purchase_costs`); PO formunu kullanan admin/purchaser bu yetkiye sahip → cost_price gelir. Seed (`api/seed/route.ts`) 8 üründen 7'sine cost_price set (yalnız `CT-SS-DN50` bilinçli null = "fiyat eksik" senaryosu). Canlı probe ile redaksiyon-sonrası DOĞRULANDI (aşağı).
- **Bilinçli DROP:** `price` (satış) fallback (footgun), otomatik kur çevrimi (veri yok), server/route/RPC/`validatePoLines` (Faz 3 audit'li, sağlam), OrderForm/QuoteForm (kapsam dışı bu tur — **NOT: QuoteForm:442 currency-guard'ı ayrı potansiyel şikâyet kaynağı; kullanıcı teklif formunda da şikâyet ederse ayrı tur**).
- **+11 test:** `purchase-orders-ui.test.ts` genişletme (pickPurchaseUnitPrice 3 + computePoTotals 5 + source-regex onChange/currency-uyarı/KDV-kırılım 3). Mevcut 28 test DOKUNULMADI (28→41 dosyada). tsc/lint/build temiz, **4449→4460 test yeşil**.
- **Canlı E2E — GÜVENLİ salt-okuma probe** (prod `ryvxpolvhvsycuqyphoa`, Playwright login→cookie, dev server port 3000; **MUTASYON YOK — "Sipariş Oluştur" ASLA TIKLANMADI** çünkü POST kalıcı `purchase_orders`+`po_counters`+`audit_log` satırı bırakır/geri-alınamaz): login→/dashboard · `GET /dashboard/purchase/orders/new` 200 + başlık · ürün select + KDV (%20) + Genel Toplam satırları render · **`GET /api/products?all=1` → cost_price non-null 7/8 (REDAKSIYON E2E KULLANICIYI ENGELLEMİYOR — danışmanın kritik "no-op olur mu?" check'i CANLI kanıtla kapandı, fix production'da çalışır)** → 🎉 PROBE PASSED (10/10). Auto-fill/KDV DOM davranışı offline vitest + build ile kanıtlı (canlı zorlanamaz — mutasyon olur).
- **Kullanıcı netleştirmesi (commit sonrası):** "birim fiyat gelmek zorunda değil yine gelebilir ama alınan fiyat değişken olabilir tedarikçiye bağlı olarak" → mevcut **düzenlenebilir öneri** davranışı bunu zaten karşılıyor (cost_price kilitsiz başlangıç, kullanıcı tedarikçiye göre üzerine yazar); AskUserQuestion 3 seçenek (öneri+ipucu / şimdiki hali / autofill kaldır) → kullanıcı **"şimdiki hali (öneri, ipuçsuz)"** seçti → KOD DEĞİŞMEDİ.
- **DURUM: COMMIT+PUSH EDİLDİ (`2509dcf` → main, `b37764a..2509dcf`, Coolify deploy tetiklendi).** React Doctor advisory bloklamadı; "test" status check bypass edildi. ⚠️ **Ekran görüntüsü #2 GÖRÜLMEDİ** — submit validasyon hatasıysa bu fix kapatır; başka bir hataysa kullanıcı bildirmeli. Plan: `~/.claude/plans/stok-r-nler-sayfas-n-detayl-calm-charm.md` (PO yeni form içeriğiyle güncel).

---

## Önceki İş — 2026-06-03 (**Ayarlar sayfası — final ürün (modal a11y + tablist a11y + entity render bug + hata mesajı paritesi)** — COMMIT+PUSH `b37764a`)

Kullanıcı "sırada ayarlar sayfası var son ürün haline eksiksiz hatasız bugsuz hale gelsin her işlevi hatasız çalışmalı e2e test et gör ultrathink" dedi. Kapsam: `/dashboard/settings` (5 sekme: Firma/Kullanıcı/Bildirimler/API/Yapay Zeka + `ResetDemoSection` "Tehlikeli Bölge") + 7 route + 3 helper. "Önce doğrula sonra düzelt" + danışman triyajı. **Sayfa olgun (1239 satır + 6 mevcut test dosyası) — güvenlik/veri-bütünlüğü bug'ı YOK** (RBAC `manage_settings` company PATCH/logo POST, user/* session-auth 401, şifre cookie'siz fresh-client doğrulama, avatar SVG yasağı+orphan cleanup+path traversal sanitize, preferences strict-boolean+whitelist, company GET `SAFE_COMPANY_FIELDS` allow-list, `revalidateTag`, demo guard). Bu tur = **0 fonksiyonel headline + 3 a11y/disiplin + 1 görünür render hatası (danışman dürüstlük onayı — "uydurma headline yok"; hepsi adlandırılmış precedent'li):**
- **#1 (MED — headline) `ResetDemoSection` onay modalı a11y eksik (en yıkıcı aksiyon):** `ResetDemoSection.tsx:114-190` confirm overlay+panel iki çıplak `<div>` — `role/aria-modal/aria-labelledby` HİÇ yok; bu modal `POST /api/seed` (TÜM prod veriyi siler) onaylıyor → ekran okuyucuya görünmüyor. Fix: panel `role="dialog"`+`aria-modal="true"`+`aria-labelledby="reset-demo-confirm-title"`, başlık "Emin misiniz?" id'li; focus-trap/Escape DROP (production `470578c`/PO/customers/vendors precedent).
- **#2 (MED) Sol sekme menüsü tablist a11y + dirty-dot text-eşdeğeri yok:** `page.tsx:1190` düz `<button>` dizisi (role/aria-selected yok), sağ içerik tabpanel değil, dirty nokta yalnız görsel `<span>` (kaydedilmemiş-değişiklik ekran okuyucuya ulaşmıyor). Fix: konteyner `role="tablist"`+`aria-label="Ayarlar bölümleri"`; her buton `role="tab"`+`aria-selected={activeTab===key}`+`id={settings-tab-${key}}`+`aria-controls="settings-tabpanel"`; sağ içerik `role="tabpanel"`+`id="settings-tabpanel"`+`aria-labelledby={settings-tab-${activeTab}}` (aktif buton id — danışman "kırık-ARIA tuzağı için tek aktif id" uyarısı); dirty nokta `aria-hidden` + dirty durum butonun erişilebilir adında (`${label} (kaydedilmemiş değişiklikler)`); **roving-tabindex DROP** (Tab-odaklı butonlar = WCAG bar korunur, danışman onayı).
- **#3 (LOW-MED → görünür) `&apos;` JS string literal'i ham görünüyor:** `page.tsx:927` ApiTab ternary dalı `"Bağlantı yok — &apos;Bağlan&apos; ile…"` JS string'di (JSX text DEĞİL) → `&apos;` decode olmaz, kullanıcı ham `&apos;` görür. Fix: düz tek tırnak `'` (U+0027; danışman tuzak uyarısı — başka HTML entity `&#39;` de decode olmaz, bug tekrarlar). **Doğrulandı (grep):** yalnız `:927`; `:953` `Paraşüt&apos;e bağlan` JSX text content → doğru decode, DOKUNULMADI.
- **#4 (LOW-MED) FirmaTab kayıt hatası jenerik (somut repro):** `handleSave:157` `throw new Error(await res.text())` → catch jenerik "Kayıt başarısız. Tekrar deneyin." toast'lıyordu; ama client `validate()` name max-length kontrol etmez, sunucu `validateCompanyPatch` >200 char reddeder → 201-char ad → sunucunun spesifik `{error:"Firma adı en fazla 200 karakter…"}` 400 mesajı düşüyordu (kardeş `KullaniciTab:399`/`BildirimlerTab:661` zaten `errBody.error` parse). Fix: `errBody?.error` parse + catch `err.message` yüzeye çıkarır (`handleLogoFile:175` precedent).
- **Bilinçli DROP (danışman doğruladı):** modal focus-trap/Escape (tüm turlarla tutarlı), roving-tabindex (WCAG dışı geliştirme), ApiTab/AiTab refactor (çalışıyor — Paraşüt OAuth token + observability), 7 route+3 helper (denetlendi+6 test kapsıyor), `patchUserMetadata` GET-merge-SET race (dokümante+UI `isMutating` lock+tek-tab senaryo).
- **+10 test:** yeni `settings-ui.test.ts` (source-regex — ResetDemoSection modal a11y 2, tablist/tab/tabpanel+dirty-dot 4, `&apos;` :927 temiz+:953 korundu 2, FirmaTab errBody parse 2). 6 mevcut settings test dosyası DOKUNULMADI. tsc/lint/build temiz, **4439→4449 test yeşil**.
- **Canlı E2E — GÜVENLİ salt-okuma probe** (prod `ryvxpolvhvsycuqyphoa`, Playwright login→cookie, port 3000; **MUTASYON YOK — ResetDemoSection'a ASLA tıklanmadı, onay modalı bile AÇILMADI** çünkü `POST /api/seed` tüm prod veriyi siler/geri-alınamaz; Firma PATCH/şifre/avatar/preferences de tetiklenmedi): login→/dashboard · `GET /dashboard/settings` 200 + başlık "Ayarlar" · **`role="tablist"` canlı render (Finding 2 a11y kanıtı)** · FirmaTab+KullanıcıTab+ApiTab içerik render · **sayfada ham `&apos;` YOK (Finding 3 fix kanıtı)** · Tehlikeli Bölge görünür ama TIKLANMADI → 🎉 PROBE PASSED (10/10). Modal a11y/FirmaTab-hata yalnız offline vitest.
- **DURUM: COMMIT+PUSH EDİLDİ (`b37764a` → main, `c8057e5..b37764a`, Coolify deploy tetiklendi).** React Doctor advisory bloklamadı; "test" status check bypass edildi.

---

## Önceki İş — 2026-06-03 (**Cariler (Müşteriler) sayfası — final ürün (toplu-silme bayat satır + hover antipattern + modal a11y + validation parity)** — COMMIT+PUSH `c8057e5`)
...
- **DURUM: COMMIT+PUSH EDİLDİ (`c8057e5` → main, `0504bdc..c8057e5`, Coolify deploy tetiklendi).** React Doctor advisory bloklamadı; "test" status check bypass. Değişen: `dashboard/customers/page.tsx` (bulk+hover+2 modal a11y) + `CustomerDetailPanel.tsx` (panel a11y) + `api/customers/[id]/route.ts` (DELETE revalidateTag + PATCH validateStringLengths) + `data-context.tsx` (addCustomer parse) + yeni `customers-ui.test.ts` + `customer-patch-route.test.ts` genişletildi + CLAUDE.md + 2 memory.
