...
---
**Aktif:** Purchase & Alert plan — Faz 10 + Review KAPALI; sırada Faz 11+ (rol matrix, SMTP production deploy)
**Önceki:** Faz 10 Review KAPALI (2026-05-18; 2755 test) — P2 reliability: `dbGetOpenShortagesByProductId` `if (error || !data) return []` Supabase DB/permission/query hatası sessizce yutuyordu → route 200 `{items:[]}` → drawer "Açık shortage kalmadı" empty branch → kullanıcı yanıltıcı bilgi. Düzeltme: `if (error) throw new Error(...); if (!data) return [];` — error explicit throw (handleApiError 500 maps, drawer shortageError set → empty branch tetiklenmez); defensive `data=null` (beklenmedik) için `[]`. Test güncellemesi: "supabase error → empty array" → "supabase error → throw" (rejects.toThrow). 2 dosya.
**Önceki:** Faz 10 KAPALI (2026-05-18; 2755 test) — order_shortage drawer (M3: bilgi yoğunluğu + iki yönlendirme): yeni `dbGetOpenShortagesByProductId(productId)` helper (shortages+sales_orders JOIN, status='open'+commercial_status='approved' filter, createdAt DESC, PostgREST array shape defensive); yeni `GET /api/products/[id]/shortages` route; AlertDetailDrawer: (1) drawerActionLinks order_shortage → "Üretim emri başlat (yeni sekmede)" primary newTab=true `/dashboard/production?productId={entityId}&qty={qty}` + "Satın alma planla" secondary (eski "Siparişleri incele" primary kaldırıldı), (2) yeni İLGİLİ SİPARİŞLER bölümü (`hasOrderShortage && !isOrphaned`; void async IIFE fetch — react-hooks/set-state-in-effect koruması, cancelled flag cleanup; 4 dal loading/error/empty/list; satırlar order_number+customer+shortageQty+İhtiyaç/Mevcut + Link → /dashboard/orders/{orderId} + aria-label), (3) actionLink render newTab → target=_blank + rel=noopener + ↗. Production page: `prefillLineFromQuery` pure helper export + Suspense wrapper (Next 15 useSearchParams) + prefilledRef guard + ?productId&qty parse + firstEmpty override veya prepend + toast. +34 test (helper 7 + route 4 + drawer source-regex 9 + production prefill helper 7 + source-regex 7). 6 dosya (4 yeni + 2 modifiye).
**Önceki:** Faz 9 Review Bulgular KAPALI (2026-05-18; 2721 test) — 2 bulgu: (1) P2 veri minimizasyonu: `print/page.tsx` `dbListAllActiveProducts()` tüm aktif ürün kataloğunu (ProductRow 35 alan: cost_price, parasut_*, on_hand, reserved, product_notes, daily_usage, ...) RSC payload üzerinden client'a serialize ediyordu; belge sadece 4 alan kullanıyor. Düzeltme: yeni `dbGetProductRefsByIds(ids)` helper + `ProductRef` view model (id/sku/name/unit); component prop `ProductRow[]` → `ProductRef[]`; print page `po.lines.map(l => l.product_id)` + `Set` dedup → sadece belgedeki ürünler için 4 sütun. (2) P3 gerçek render testi: önceki testler source-regex'e dayanıyordu, JSX/conditional render/leak regression yakalanmıyordu. Düzeltme: `react-dom/server.renderToStaticMarkup` ile vitest node env'inde jsdom-free real render (dep yok); 4 grup: render content + conditional branches (cancelled/notes/logo/company/vendor null) + toolbar print-gizleme (po-no-print class + window.print) + leak absence (rendered HTML'de cost_price/parasut_*/received_qty/created_by/vendor.notes substring YOK). `vi.mock("next/link")` plain `<a>` stub + fixture helper'ları. +24 test. 4 dosya değişti (helper + component prop tipi + print page + 2 test).
**Önceki:** Faz 9 KAPALI (2026-05-18; 2697 test) — PO PDF render (server-side HTML print): `PurchaseOrderDocument.tsx` (yeni client component; A4 portrait, `@page` + `@media print` CSS, logo `<img>` next/image-yerine bilinçli + eslint-disable, header→title→meta grid→lines table→totals→notes→cancel reason, status TR map, currency-aware Intl format, `window.print()` toolbar `.po-no-print` ile gizlenir, güvenlik: `created_by`/`audit_log`/`received_qty`/vendor.notes DOM dışı), `print/page.tsx` (yeni server component RSC, paralel fetch, null po → notFound, `export const dynamic = "force-dynamic"`), detail page'e "📄 Yazdır / PDF" Link butonu (target=_blank, demo izinli — read-only). Export'lu pure helper: `formatPoCurrency`/`formatPoDate`. +25 test (document 18 + page 7). 173 dosya. Migration yok. Yeni route `/dashboard/purchase/orders/[id]/print`.
**Önceki:** Faz 8 Review Bulgular KAPALI (2026-05-18; 2672 test) — 5 bulgu + 4 payload testi + slice guard: (1) P2 zero-width/bidi bypass: `sanitizeFeedbackForPrompt` step 1a `ZERO_WIDTH_AND_BIDI_RE` → empty (önceki FEEDBACK_STEP1_RE'yi tamamlar); +2 test. (2) P2 PostgreSQL PUBLIC execute: migration 054 `REVOKE ALL FROM public, anon, authenticated` + `GRANT TO service_role`; migration 055 genişletildi (public+anon+authenticated). (3) P3-001 plan hizalama: `purchase-aksiyon-plan.md` `af.created_at` cutoff → `ar.decided_at`. (4) P3-003 C0 karakter: `purchase-aksiyon-plan.md` NUL(0x00)+US(0x1F) → U+0000/U+001F. (5) P3-004 defense-in-depth: `ai-service.ts` `sanitizedItems` map `recentRejections` → `sanitizeFeedbackForPrompt` re-sanitize. 172 dosya. +5 test (payload×4 + slice×1).
**Önceki:** Faz 8 KAPALI (2026-05-17; 2665 test) — AI rejection feedback prompt entegrasyonu: Migration 054 (`get_recent_rejections_for_products` RPC, ROW_NUMBER PARTITION BY entity_id, 90-gün cutoff `r.decided_at`, SECURITY DEFINER + STABLE + REVOKE ALL/GRANT service_role), `sanitizeFeedbackForPrompt` helper (5+1 adım, 8+2 saldırı vektörü), `dbGetRecentRejectionsForProducts` bulk fetch + per-row sanitize, `PurchaseSuggestionItem.recentRejections` opsiyonel alan + PURCHASE_COPILOT_SYSTEM prompt clause (echo etmemesi vurgulu), purchase-copilot route'a try/catch'li RPC çağrısı (RPC fail non-fatal, empty array → alan yazılmaz, token tasarrufu), ai-service.ts recentRejections defense-in-depth re-sanitize. +21 test (sanitize 11 + bulk fetch 6 + integration 4). 171 dosya.
**Önceki:** Faz 7 Kapanış 2. Tur KAPALI (2026-05-17; 2646 test) — P3 tam kapanış: route.ts fire-and-forget → await .catch(log) (200 öncesi resolve garantili) + serviceCheckOverdueShipments güvenlik ağı (stale overdue_shipment alertleri CRON'da toplu resolve, Promise.all paralel fetch, { alerted, resolved } dönüş). +2 yeni test (stale resolve × 2 senaryo) + 5 mevcut test resolved alanı güncellendi. 5 dosya.
**Önceki:** Faz 7 Kapanış 1. Tur KAPALI (2026-05-17; 2644 test) — P2 shipDate strict takvim validasyonu (roundtrip guard: 2026-02-31/2026-99-99 → 400, RPC öncesi), P3 server-side dbBatchResolveAlerts fire-and-forget (client PATCH kaldırıldı). +3 test. 168 dosya.
**Önceki:** Faz 7 KAPALI (2026-05-16; 2641 test) — overdue_shipment alert inline ship form: Migration 053 (shipment_tracking_number + shipment_carrier), ShipMeta interface + serviceTransitionOrder genişletme, POST /api/orders/[id]/ship (yeni endpoint), OrderAlertDrawer inline form (shipDate/tracking/carrier, demo guard, aria), actionFor() overdue_shipment case. 12 test (8 endpoint + 4 source-regression). 168 dosya.
...
**Önceki:** Coolify migration TAMAMEN KAPALI (2026-05-16; 2599 test). Production: `https://erp.getmedspace.com` (GoDaddy domain, Hetzner VPS 138.199.204.138, Coolify). GitHub Actions cron: `APP_URL=https://erp.getmedspace.com`, `CRON_SECRET=kokpit-pmt-2026`, 8 endpoint hourly+six-hourly ✅. vercel.json silindi (commit 1f95ae7). Vercel Hobby deploy'ları artık önemsiz. Sentry source map upload: `.github/workflows/sentry-release.yml` main push'ta çalışır (SENTRY_AUTH_TOKEN secret gerekli — henüz set edilmediyse Sentry upload sessizce atlanır).
**Önceki:** Faz 4 follow-up KAPALI (2026-05-13; 2599 test) — useSearchParams Suspense fix (Vercel build kritik) + fromDraft preload + Revize endpoint+UI + audit timeline (helper+endpoint+UI dikey liste) + expectedDateDirty flag + double toast fix. +17 test. **Vercel deploy May 6'dan beri fail; bu commit ile düzelmesi muhtemel — `gh api .../statuses` ile kontrol edilmeli.**
**Önceki:** Faz 4 KAPALI (2026-05-12; 2582 test) — PO UI sayfaları: /dashboard/purchase/orders (list+status tabs) + /new (form, vendor lead_time auto-fill) + /[id] (detail, status CTAs, cancel modal admin). Sidebar "Siparişler" linki. +source_recommendation_ids validation gap (Faz 3 final cleanup). +6 test (4 UI smoke + 2 validation). Faz 5 mal kabul UI'ı eklenecek.
**Önceki:** Faz 3 advisor 2. tur KAPALI (2026-05-12; 2576 test) — P2 follow-up: validatePoLines null/""/UUID kontrol + currency whitelist (POST+PATCH) + P3 regression lock (confirm/cancel revalidateTag products assert). +5 test, fixture'da product_id valid UUID'ye taşındı.
**Önceki:** Faz 3 advisor fix KAPALI (2026-05-11; 2571 test) — P1.1 role-guard app_metadata switch (privilege escalation kapatıldı) + P1.2 dbTransitionPO CAS (.eq status) + partially_received/received guard + P2.1 vendor active PO guard + P2.2 validatePoLines JS-side (400 vs 500) + P3 products cache revalidate (confirm/cancel). +14 test (4 dosya).
**Önceki:** Faz 3 KAPALI (2026-05-11; 2557 test) — PO backend: migration 049/050/052 + DB types (PurchaseOrderRow/PurchaseOrderLineRow/PoLineRecommendationRow/PoCounterRow) + purchase-commitments B1 fix (incoming=qty-received_qty) + role-guard.ts (B7) + purchase-orders.ts helper (VALID_PO_TRANSITIONS, RPC delegates) + purchase-order-service.ts (serviceSendPO/Confirm/Cancel/Revise) + 6 API routes (GET/POST/PATCH/PUT + send/confirm/cancel) + 50 new tests (3 files)
**Önceki:** Faz 2 KAPALI (2026-05-10; 2498 test) — vendors entity: migration 048 + VendorRow type + vendors.ts helper + /api/vendors routes + /dashboard/vendors page + sidebar "Satın Alma" group
