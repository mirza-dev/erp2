...
---
**Aktif:** Vercel → Coolify migration Faz D (browser smoke kaldı). Staging URL: `https://erp.getmedspace.com`. Tüm API/CRON smoke yeşil (2026-05-16). Sırada: browser smoke (login+dashboard+vendors+purchase orders+products) → Faz E (DNS cutover erp.kokpit.app → Hetzner IP + GitHub Actions secrets güncelle + Vercel pause).
**Önceki:** Purchase & Alert plan — Faz 1-4 follow-up KAPALI. Faz sırası: 1=alert sync_issue retry ✅, 2=vendors ✅, 3=PO schema+RPC ✅, 4=PO UI ✅, 5=mal kabul (B1), 6=suggested→PO (devam edecek).
**Son:** Coolify Faz D otomatik smoke KAPALI (2026-05-16; 2599 test) — staging URL doğrulandı (erp.getmedspace.com); /api/health=200, /login=200, /dashboard auth gate=307, /api/products=401, CSP/HSTS/headers ✅, CRON_SECRET=`kokpit-pmt-2026` + 8 CRON endpoint 200 ✅, /api/health?detail=true tüm migrations OK ✅. Kalan: browser smoke (kullanıcı tarafında).
