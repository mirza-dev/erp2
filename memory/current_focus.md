...
---
**Aktif:** Vercel → Coolify migration (Hetzner/Vargonen VPS, GitHub Actions cron). Plan: `~/.claude/plans/g11-a-neri-tutarl-ethereal-balloon.md`. Faz A (kod) ✅; Faz B/C/D/E/F kullanıcı tarafında. Vercel deploy May 9'dan beri fail (cron limit); Coolify cutover sonrası vercel.json silinecek.
**Önceki:** Purchase & Alert plan — Faz 1-4 follow-up KAPALI. Plan: `/Users/mirzasaribiyik/Projects/erp2/purchase-aksiyon-plan.md`. Faz sırası: 1=alert sync_issue retry ✅, 2=vendors ✅, 3=PO schema+RPC ✅, 4=PO UI ✅, 5=mal kabul (B1), 6=suggested→PO, 7=overdue ship inline, 8=AI feedback prompt (M4 bulk), 9=PDF, 10=order_shortage drawer.
**Son:** Coolify migration Faz A KAPALI (2026-05-13; 2599 test) — kod scaffolding: next.config standalone+image-unoptimize, sentry SENTRY_ENVIRONMENT desteği, Dockerfile multi-stage secret-free, .dockerignore, GitHub Actions crons.yml (8 endpoint, fail-isolation, retry, off-peak), sentry-release.yml (source map upload Docker dışında), .env.example güncellemesi, README Deployment bölümü. Build standalone output OK. Kullanıcı sırada: VPS al + Coolify install + env vars + smoke test + DNS cutover.
