...
- **KALAN (kullanıcı):** (1) **Kaydetme hatası nedeni** — redeploy sonrası Kaydet → toast artık gerçek nedeni/SQLSTATE kodunu söyler; 403 çıkarsa bu Coolify ortamında `ADMIN_EMAILS`'e e-postanı ekle VEYA Supabase'de kullanıcının `app_metadata.roles=["admin"]` (sonra yeniden giriş). (2) manuel smoke — yeni teklif → satıra Not → kaydet → yenile (korunur) → Önizle/PDF ürün altında görünür + notsuz satırda yok → Gönder → mail PDF + arşivde görünür → Genel Notlar ayrı çalışır → koyu/aydınlık tema; demo modda kaydet bloklu.
