...
**⚠️ Araç notu:** bu koşuda semgrep+gitleaks sandbox `Bash` izni reddiyle ÇALIŞAMADI (`dangerouslyDisableSandbox` dahil) → mekanik SAST/secret atlandı; bulgular elle+hedefli-grep. Tam kapsama için izinli ortamda koşturulmalı. N1 (orders detay UI-only UTC) / N2 (`rfq-validation` mig.103 sonrası ölü zorunlu alanlar) bilinçli açık. **Kalan:** push (main + codex FF) + isteğe bağlı N1/N2.
