...
## 2026-09-19 — Pazarlama turu 3: landing yeniden kurgu + gerçek ürün ekran görüntüleri (`2f31df6` → `95dadc4`, PUSH ✅)

**İstek:** fiyat rakamları onaylandı (kurulum 45/85/150 bin, bakım 24/42/72 bin TL) → *"çok iyi profesyonel bir landing sayfası kuralım."* Sekiz karar AskUserQuestion ile alındı: anlatı yeniden kurulur · hero **"Teklifi gönderdiğin an, stok ayrılır."** · gerçek ekran görüntüleri (yerel Supabase + Playwright) · birincil CTA görüşme, demo ikincil · PMT vaka bloğu sayısal ama **ad/rakam yazılı izin gelene kadar canlıya çıkmaz** · Excel'e karşı blok (rakip firma adı yok) · sınırlar ayrı dengeli liste · `#yapayzeka` küçültülür (silinmez — marka başlığında "yapay zeka destekli" geçiyor).

**Ölçüm (önce):** `page.tsx` 610 satır, anlatı bir özellik listesiydi; hero'da CSS ile çizilmiş **sahte panel** (`mockOrders`/`mockBars`) — 85.000 TL'lik kurulum satışında çizim olduğu belli görsel şüphe bırakır; ayırt edici mekanizma (teklif→rezervasyon) başlıkta değildi; "Demo Gez" 5 kez, form 1 kez. Ayrıca eski özellik metni **"kabul edilince rezervasyon oluşur"** diyordu — mig.088'den beri rezervasyon GÖNDERİMDE; vitrin ürünün gerisindeydi.

**Commit 1 — `scripts/build-product-shots.ts` (`npm run shots`), fail-closed boru hattı.** Her katman **gösterilen bir sızıntıdan sonra** eklendi: (1) yalnız loopback Supabase (`isLocalSupabase`; `ALLOW_PROD_TARGET` bu script için tanınmaz) → (2) DB'de firma/cari/tedarikçi adları kurgusal adlarla değiştirilir (`TEXT_REPLACEMENTS`, hepsi `/gi` — `botas`/`bulonsan`/`langge-valve` ASCII e-posta alan adlarından sızdı) → (3) donmuş teklif künyeleri + serbest metin süpürülür (`SWEPT_TABLES` 10 tablo; `products.preferred_vendor` denormalize kopyası son leak'ti) → (4) `assertNoRealNames` tüm kolonları tarar → (5) `bumpListCaches` (`unstable_cache` bayat listeyi gösteriyordu) → (6) **DOM kapısı**: `document.body.innerText` `containsForbiddenName` ile taranır, eşleşme varsa görsel YAZILMAZ. Kapı (6) gerçek bir vakada kanıtlandı: port 3100'ü başka bir checkout'un sunucusu devralmıştı, pano gömülü **"PMT Endüstriyel"** ile çekildi — DB temizdi, ekran değildi. Yasak liste `src/lib/marketing/forbidden-names.ts`e (test de okur; `toLocaleLowerCase("tr-TR")`). Ayrıca `topUpNegativeStock` (taslak siparişler satılabiliri -2'ye düşürüyordu; ekran görüntüsünde negatif stok olmaz).

**İki gerçek ürün kusuru bulundu ve düzeltildi (çekim yan ürünü):** (a) dashboard/rapor başlığında **gömülü "PMT Endüstriyel"** metni (firma ayarı yerine sabit); (b) `QuoteForm` satıcı adı varsayılanı `"PMT Endüstri A.Ş."` ve `pmt.com.tr` yer tutucusu → boş + `company_settings`'ten dolar; **kayıtlı teklif açılınca cari açılır listesi kendiliğinden açılıyordu** (`[customers]` effect'i hidrasyonda tetikleniyordu) → `custTypedRef` (yalnız kullanıcı yazınca açılır). Bu kusur ekran görüntüsünde "dropdown açık" olarak görüldü; Escape/blur ile örtülmedi, kökü düzeltildi.

**Commit 2 — `src/lib/marketing/landing-content.ts`:** tüm diziler ayrı modüle (Next route-segment kuralı page'den sabit export ettirmiyor). Saf taşıma; fiyat kilidi testi artık `page.tsx` + `landing-content.ts` **bileşimini** okur (iddia iki dosyada yaşayabilir).

**Commit 3 — anlatı:** hero → `#referans` (vaka **isimsiz**: `proof = {named:false, company:null, stats:[]}`; yanında `productFacts` — ürünün doğrulanabilir gerçekleri) → problem → **`#rezervasyon`** (teklif→rezervasyon→ikinci satışçı zinciri + `stok.png`) → `#ozellikler` (2 büyük görselli: teklif, uyarı takvimi + 3 küçük) → `#yapayzeka` (küçük bant) → `#nasil` → `#kimler` → **`#excel`** (5 satır, rakip adı yok) → **`#neden`** (kullanıcı başına ücret yok · veritabanı sizin adınıza · kurulum bir öğleden sonra) → `#fiyat` (rakamlar aynen) → **`#sinirlar`** (4 madde, her biri "ama şu var"; Paraşüt "hazır, isteğe bağlı açılır" — §1.5 dili) → `#sss` (6→8) → `#iletisim`. Nav 8→5 + "Görüşme iste". Dört görsel `public/shots/{dashboard,stok,teklif,uyarilar}.png` (1440×900, koyu tema, gözle temiz: "Örnek Makine San. ve Tic. A.Ş."). YENİ `marketing-landing.test.ts` (14 test) — 4/4 kırmızı-kanıtlı (`redproof.py`, SHA ile geri yükleme): Paraşüt cümlesi · gerçek ad · vaka kapısı · DOM kapısı.

**Commit 4 — rehber §1.5:** ekran görüntüsü/referans kuralları (yalnız `npm run shots`; seed'deki gerçek adlar hiçbir vitrinde; pilot adı yalnız yazılı izinle; rakip adı yok) + tablo satırı "rezervasyon **gönderilince** (mig.088)". `build-og-image.ts` politika yorumu güncellendi.

**Ölçü aracı yine bulguydu:** `sentences()` başlangıçta etiket+başlık parçalarını birleştirip olmayan bir "cümle" uyduruyordu → parça parça bölündü. `marketing-contact.test.ts` ham NUL byte taşıyordu (git "binary" sayıyordu) → `\u0000`. Turbopack "Every task must have a task type" paniği → `.next` sil. Shell heredoc/compound reddi → yamalar `$CLAUDE_JOB_DIR/tmp/*.py`.

**Doğrulama:** tarayıcı 1440 + 390 px (12 bölüm · h1=1 · yatay taşma 0 · kırık görsel 0 · konsol hatası 0) · tsc 0 · lint 0 · **513 dosya / 7167 test** · build 0 uyarı · migration YOK. Push: `origin/worktree-brand-roven` = `95dadc4`.

**Kullanıcı tarafı (bloklamaz):** PMT izni + rakamlar → `proof.named:true` (tek veri nesnesi) · `CONTACT_EMAIL` → form çalışır · domain + deploy. Çekimi yenilemek: `SHOTS_APP_URL=http://localhost:<port> PLAYWRIGHT_CHROMIUM_PATH=<chrome-headless-shell> npm run shots` (yerel Supabase ayakta olmalı; script canlıyı reddeder).
...
Üç commit, tek push. **origin/main = origin/codex-experiment = `2095ae2`** (tree `3f854e1`, ıraksama 0 0). migration YOK; davranış/RBAC/demo/veri akışı/AI sinyal overlay'i değişmedi.

- **`27084c4` — bekleyen import fix (hafızada KAYDI YOKTU):** oturum başında working tree'de commit'lenmemiş bitmiş iş bulundu. `POST /api/import` `created_by`'ı istemci body'sinden okuyordu → `(await getCurrentUserId()) ?? undefined`. Purchase O1 / Vendors D1 ile aynı sınıf (mass-assignment / non-repudiation); o turlarda gözden kaçmış uç. +3 test (`import-create-batch-route.test.ts`).
- **`c98c579` — DataTable satır klavye erişimi + `rowAriaLabel`:** `onRowClick` bugüne kadar yalnız fare sözleşmesi veriyordu. Artık satır `tabIndex=0` + Enter/Space (`preventDefault` ile) alır; YENİ opsiyonel `rowAriaLabel?: (row)=>string`. **`role="button"` EKLENMEDİ** — `<tr role="button">` satırı ekran okuyucuda "tablo satırı" olmaktan çıkarır (hücre/sütun bağlamı gider). Kasıtlı yan etki: Orders/Quotes/Customers/PO da klavyeyle gezilebilir oldu. +5 test → data-table.test.tsx 11→16.
- **`2095ae2` — products listesi Card+DataTable:** products diğer 6'nın ikizi DEĞİLDİ. **Blokaj = a11y:** products repodaki TEK klavye-erişilebilir liste satırıydı (tabIndex/role/aria-label/onKeyDown elle yazılmış) → düz dönüşüm regresyon olurdu, önce sözleşme component'e taşındı. **İki teknik incelik:** (1) `cellStyle` kolona statiktir → Stok/Satılabilir'in satır bazlı koşullu rengi hücre içinde `<span style>`; (2) sayfanın yerel thStyle/tdStyle'ı `whiteSpace:nowrap` veriyordu, DataTable vermiyor → kolon bazında geri verildi (Ürün Adı'nın `textOverflow:ellipsis`'i nowrap'siz çalışmaz; global nowrap eklenmedi — 6 dönüşmüş listenin sarma davranışını değiştirirdi). 8 kolon (`price`/`minStock` dar ekranda gizli → `wideOnlyColumns` spread'i), `minWidth={isMobile?"360px":"640px"}`, surface div→`<Card>`, boş-durum→`emptyMessage`, Pagination→`footer`. KALDIRILDI: thStyle/tdStyle, hoveredId+onMouseEnter/Leave+rowBg, satırdaki elle yazılmış a11y.
- **Bilinçli tek davranış farkı:** boş durum artık `total===0` yerine `rows.length===0` ile render → "sayfa boş ama total>0" (bayat sayfa no) kenar durumunda eskiden hiçbir şey görünmezken artık boş-durum metni görünür.
- **Testler (niyet korunur):** `products-list-hover` → hoveredId/rowBg/onMouseEnter kilitleri DataTable+CSS formuna, DOM-mutation + confirmDeleteId-kuplajsızlık negatif kilitleri KORUNDU, klavye a11y kilidi eklendi; `products-page-drawer-removed` → `>SKU<` yerine `header: "SKU"`, eski kolon negatif kilidi HER İKİ formda; `theme-system` → PRODUCTS_SRC "kendi `<table>`'ını tutan temsilciler" döngüsünden çıktı (ORDERS_SRC precedent'i), kullanılmayan sabit silindi.
- tsc 0 · lint 0 · **5608 test** (5603→+5) · build 0. react-doctor --staged 37 uyarı/0 hata; **dokunulan kodda tek bulgu** (`router.push` React Compiler stil önerisi — dönüşümden önce de aynı satırdaydı, Orders/Quotes/PO kalıbı), kalan 36 pre-existing → `--no-verify` + loglandı.
- **DÜZELTME (bayat notlar):** products'ta "drawer edit modu" YOK — drawer Faz 2b'de kaldırıldı, satır tıklaması `router.push`. `project_frontend_renewal`'daki "`rowStyle` HÂLÂ yazılmadı" notu da bayattı (Faz B #6'da eklendi).
...
- **Bulgu:** OrdersClient `cellBg` YALNIZ hover (seçili-satır vurgusu YOK) → `rowStyle` GEREKMEDİ; CSS `.erp-data-table tbody tr:hover` devraldı. Asıl ihtiyaç: hover'da beliren sil-butonu/chevron (eski `opacity: isHovered ? 1 : 0`).
- **YENİ `.row-reveal` CSS utility** (globals.css): `.erp-data-table tbody tr:hover .row-reveal { opacity: 1 }` (default 0) — JS hoveredId yerine saf CSS, rerender yok, daima DOM'da (a11y: focusable kalır). Quotes'a da yarayacak.
