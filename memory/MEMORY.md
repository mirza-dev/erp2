...
- [current_focus.md](current_focus.md) — Son: Teklif V7 **Faz 4 PDF Arşiv** (dondurulmuş HTML snapshot) **+ Bulgular 1.+2.+3.+4. review tur**. 4. tur (P3): P3-1 stale doc, P3-2 orphan phantom→`dbArchiveObjectExists` archive GET varlık kontrolü (graceful 404; recover Faz 6), P3-3 logo byte-freeze caveat kabul. 3. tur: bulk delete regresyonu pickSucceededIds+seçim draft-only. 3974 test, **4. tur COMMIT+PUSH BEKLİYOR (1.+2.+3. push edildi `da09dce`) + 075/076 APPLY EDİLDİ ✅**; önceki: Revizyon Zinciri (074 ✅)
...
- [project_quotes.md](project_quotes.md) — Teklif modülü: V7 master plan; Faz 1a/1b/2/3 ✅ + Faz 5 numara (073) ✅ + Revizyon zinciri (074) ✅ + **Faz 4 PDF Arşiv** (075-076, dondurulmuş HTML snapshot) + **Bulgular 1.+2.+3.+4. review tur** (4. tur P3: orphan phantom→dbArchiveObjectExists, logo caveat kabul; 3974 test, 4. tur COMMIT+PUSH BEKLİYOR [1.+2.+3. `da09dce`], 075/076 APPLY EDİLDİ ✅); ERTELENEN: sig rename, **quotes audit katmanı** (modül-geneli), Puppeteer/binary-PDF (frozen HTML seçildi); sıradaki Faz 6 (accept→sipariş, 077)
