...
- [current_focus.md](current_focus.md) — Son: **Satın Alma Siparişleri sayfası final ürün 2. tur** (sayfa zaten `470578c`'de finaldı; danışman triyajı → 2 düzeltme: #1 [MED-headline] sessiz yükleme hatası→`loadError` state+`role=alert` banner+"Yeniden dene" [liste+new form; detay `loadPO` zaten dürüsttü], #2 [LOW] expected_date ham ISO→tr-TR `formatExpectedDate`; DROP: LOW-2 aria-label uydurma + MED-3 bulk-cancel non-issue; 4393→4402 test; GÜVENLİ salt-okuma probe 7/7 [prod 0 PO, MUTASYON YOK], COMMIT BEKLİYOR); önceki: **Paraşüt Sync sayfası final ürün** (kırık Manuel Sync→`/api/parasut/sync-pending` + gerçek `last_sync_at` + dürüst progress; COMMIT+PUSH `ca34198`); önceki: Teklif V7 **Faz 8 — Ertelenen Borçlar Kapanışı** (5 alt-faz/commit): 8a RBAC yazma uçları (`4935e88`), 8b convert ölü kod temizliği (`71a22cd`), 8c quotes audit katmanı helper-seviyesi (`034f8ea`), 8d order_line_description migration 080 (`4218d3e`, APPLY EDİLDİ ✅), 8e Paraşüt iskonto orantılı guard→reconciliation (`4b9c938`). Kullanıcı: sig rename ATLA, drag-reorder ERTELE. **+Bulgular 1.tur (`f7fc9f8`): reconcile guard invoice retry yoluna da uygulandı + 080 doc flip + P3 yorumlar. 4215 test, COMMIT+PUSH EDİLDİ · 080 APPLY EDİLDİ ✅**; **V7 + tüm ertelenen borçlar TAMAMLANDI**; önceki: Faz 7 note_templates (079 APPLY EDİLDİ ✅)
