...
- [current_focus.md](current_focus.md) — Son: Teklif V7 **Faz 6 Bulgular 2. tur (5 bulgu)** — #1 arşiv create-race obje-doğrulama (tri-state `dbArchiveObjectStatus`), #2 accept fail-closed (unknown→502; `dbArchiveObjectConfirmedMissing`→tri-state), #3 order detail arşiv PDF linki, #4 doc drift, #5 lint stale-checkout. **4043 test, COMMIT+PUSH BEKLİYOR + 077 ✅/078 BEKLİYOR**; önceki: Faz 6 Bulgular 1. tur (`b17181e`) + lint fix (`dddb1f9`)
