...
- [current_focus.md](current_focus.md) — Son: Teklif V7 **Faz 7 — Not Şablonları (note_templates)** — migration 079 (080 KALICI DÜŞÜRÜLDÜ: position zaten var); CRUD helper/route (GET açık, mutation admin, soft-delete) + settings sayfası + QuoteForm 3-alan picker (applyTemplateToField/templatesForField) + PMT seed + **Bulgular 1.+2.tur** (1.tur: maybeSingle DB-hata ayrımı, geçersiz ?kind→400, doc drift; 2.tur: unsaved draft restore'da not/teslimat/ödeme kaybı fix [teklif_v3'e eklendi], settings liste body clamp). **4098 test, COMMIT+PUSH EDİLDİ · 079 APPLY EDİLDİ ✅**; **V7 master-plan TAMAMLANDI**; önceki: Faz 6 Bulgular 3. tur (`9b9ecde`, 4043 test, 077/078 APPLY EDİLDİ ✅)
