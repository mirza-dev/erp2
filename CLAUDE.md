...
**Son tamamlanan iş:** **D — Migration durumu kesinleştirildi + gate hygiene (mig.104) + birikmiş smoke checklist** (2026-06-19; GREEN; PUSH BEKLİYOR; **migration YOK; runtime kodu değişmedi**). Rapor `docs/audit/2026-06-19-d-migration-smoke.md`. **`npx tsx scripts/check-migrations.ts` → 17/17 auto-probe ✅ GREEN** (073…100 tüm tablo/kolon/RPC canlıda → eski "088 BLOKER / 091 APPLY bekliyor" notları BAYAT). **Gate hygiene:** `check-migrations.ts` MANUAL map'ine **mig.104** (`reverse_production` REDEFINE — production O1 `for update` fix'i) eklendi; önceden hiç izlenmiyordu → artık `⚠️ 104 … elle doğrula` satırı raporlanır (sessiz untracked kapandı; `manuel: 7→8`). **Smoke doc 3 bölüm:** §1 probe sonucu; §2 8 MANUAL redefine doğrulama SQL'i (089/093/094/095/101/102/103/104 — Studio, kullanıcı; OpenAPI fonksiyon gövdesi görmez; ortamda DATABASE_URL/psql yok); §3 birikmiş browser smoke checklist (A3 guards, customers/products/alerts 403, production O1 eşzamanlı geri-alma, orders Y1/O2, mig.099 birim, quote send/mail, tema+demo). **Sınır:** AI tarafı (probe+gate) kapandı, kalan iş kaçınılmaz kullanıcı-tarafı (MANUAL SQL + tarayıcı smoke). lint 0. Kalan backlog: A2 Upstash · C1 Login canlı · C2 Paraşüt Faz12.
