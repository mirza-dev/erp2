...
**Son tamamlanan iş:** **Faz 8 Bulgular 1. tur — Paraşüt reconcile retry-bypass + doc/P3** (2026-05-31, "önce doğrula sonra düzelt", 3 bulgu; 2 commit). **P1/P2 (FİNANSAL, `f7fc9f8`):** Faz 8e reconciliation guard yalnız ana sync'teydi (`serviceSyncOrderToParasut`); manuel invoice retry (`serviceRetryParasutStep`, step=invoice → `upsertInvoice` header iskontoyu `discount_value`'ya uygular) guard'sız claim alıp sessiz yanlış fatura üretebiliyordu (canlı ama `checkStepDeps` shipment dep'i arkasında latent). Fix: `guardDiscountReconciliation` helper'ı çıkarıldı (inline blok birebir), her iki yolda claim ÖNCESİ; retry'da `step==="invoice"` gating (edoc'ta fatura zaten oluşmuş → strand etmemek için). Test: invoice retry mismatch/subtotal=0 → claim YOK + alert (guard'a `parasut_shipment_document_id` ile honest reachability); discount=0 → claim çağrılır. **P2 doc:** migration 080 APPLY EDİLDİ (kullanıcı onayı) → "BEKLİYOR" referansları hizalandı. **P3 (kozmetik):** convert/route.ts stale "serviceConvertQuoteToOrder korunur" yorumu (8b'de silindi) + quote-service.ts üst yorum "sent→accepted" → accepted /accept'e taşındı + quotes-faz4-archive.test.ts stale `accepted` çağrısı → `rejected` (geçerli non-sent geçiş, gerçek success path). **4215 test** · tsc temiz · lint 0 · build OK (`ƒ Proxy`). **Önceki:** **RBAC Faz 6 TAMAMLANDI — delete policy** (commit `5e0df84`, worktree-rbac-faz6-delete → FF→main; 4212 test). DELETE uçları domain `delete_*` yetkisine bağlandı (customers→delete_customers, vendors→delete_vendors, production→delete_production, orders→delete_sales_orders; quotes zaten delete_quotes/Faz 8a) — **davranış koruyucu** (manage_X tutan rol delete_X'i de tutuyor → kimse erişim kaybetmez, yalnız semantik). `getCurrentUserId` helper + 3 hard-delete helper'ına (customer/order/quote) **audit-after-success** pattern: before-snapshot silmeden ÖNCE çekilir, audit YALNIZ delete başarılı olunca yazılır → FK restrict throw ederse yalan `*_deleted` audit kalmaz (advisor blocker; dbDeactivateVendor güvenli paterni). 409 ilişki kontrolleri zaten mevcuttu. **Bilinen refinement:** customers→invoices, orders→shipments/invoices restrict FK'leri 409 pre-check'te değil → dürüst 500 (audit-after sayesinde correctness etkilenmez), temiz 409 mesajı gelecek tur. Migration YOK (audit_log.actor mevcut). **Önceki:** **RBAC Faz 4 TAMAMLANDI** — quotes + PO finansal GET redaction + archive gate (commit `1db5865`, FF→origin/main; 4197 test). quotes (camelCase, view_sales_prices) + PO (snake_case, view_purchase_costs) + donmuş arşiv PDF gate. Sızıntı yüzeyi advisor ile doğrulandı (preview=localStorage/Mod A, [id] inline PDF render etmez).
...
- **Faz 8 (2026-05-31, "ertelenenleri halledelim"; 5 bağımsız kalem/commit):** **8a (`4935e88`) RBAC** — quotes yazma uçlarına requirePermission (POST/PATCH/revise→manage_quotes, DELETE→delete_quotes; GET auth-only); 7 mevcut route testine role-guard mock. **8b (`71a22cd`) convert ölü kod temizliği** — serviceConvertQuoteToOrder+ConvertResult kaldırıldı (yerini Faz 6 atomik accept aldı); dbFindOrderByQuoteId + /convert 410 stub KORUNDU. **8c (`034f8ea`) quotes audit katmanı** — helper seviyesi (RPC değil, product-types paterni; advisor): dbCreate/Update/Revise → audit_log (best-effort, actor'sız=codebase-tutarlı); RPC repro riski elendi. **8d (`4218d3e`) order_line_description — migration 080 (APPLY EDİLDİ ✅)** — order_lines+=description, accept RPC 078 gövdesi birebir+qli.description delta (master p.name korundu), TS/mapper/UI. **8e (`4b9c938`) Paraşüt iskonto orantılı** — guard→reconciliation: computeHeaderDiscountPct builder'da per-satır discount_value + reconcileParasutDiscount (orantılı toplam kendi kodumuzda, grand_total ile tolerans; mock net_total iskonto yok sayıyor); tolerans aşımı/subtotal=0→blok+alert, uyuşursa fatura OLUŞUR (order_lines mutate edilmez). **Kullanıcı kararları:** sig rename ATLA (kabul edilen isimlendirme), drag-reorder ERTELE. **4098→4098 (8e builder drift-guard +2)** · tsc/lint/build temiz. **DURUM: 5 commit COMMIT+PUSH EDİLDİ · 080 APPLY EDİLDİ ✅ (kullanıcı, 2026-05-31).**
