...
**Son tamamlanan iş:** **Satın Alma Siparişleri sayfası — final ürün (eksik kapatma + salt-okuma canlı E2E)** (`/dashboard/purchase/orders` liste + `[id]` detay + `new` form + `[id]/print` + 9 API route). "Önce doğrula sonra düzelt": tüm sayfalar/route'lar/servis okundu + danışman. Sayfa ÇOK olgun (Faz 3-9 + Bulgular turları); **güvenlik/veri-bütünlüğü bug'ı YOK** (CAS race guard, RPC guard'ları, RBAC redaction, print veri-minimizasyonu sağlam). **5 gerçek polish düzeltmesi (çekirdek + a11y + parite):** (1) **Liste hover DOM-mutation antipattern** (`page.tsx:283-285` `e.currentTarget.style.background`) → `hoveredId` state + `isHovered` koşullu background (orders/quotes precedent'i; tek hedef, confirmId tuzağı yok). (2) **Validation parity** — `notes`/`lines[].notes` doğrulamasızdı → `POST /api/purchase-orders` + `PATCH [id]` + `PUT [id]/lines` üçüne `validateStringLengths(body)` (safeParseJson sonrası, RPC öncesi; recursive → top-level + array-of-objects `lines[].notes` kapsanır; kardeş `from-recommendations` zaten taşıyordu). (3) **Toplu iptal seçimi** received/cancelled PO'ları seçiyordu (→ 409 gürültüsü) → `isPoCancellable` export + `cancellablePageIds` → select-all+satır checkbox yalnız iptal edilebilir PO'larda (satış siparişleri `isOrderCancellable` paritesi). (4) **Client/server fiyat tutarsızlığı** (`new/page.tsx:161` `price < 0` → `unit_price=0` geçiyordu ama server `validatePoLines` `<= 0` reddediyor) → client `<= 0`'a hizalandı. (5) **a11y** — liste toplu-iptal modalı `role=dialog`/`aria-modal`/`aria-labelledby` HİÇ yoktu + detay iptal modalına `aria-labelledby` eklendi. **Kapsam dışı (bilinçli):** RBAC (`requireRole` cancel/receive vs `requirePermission` = B7 tasarımı), `window.location.href` satır nav. **+12 test (4370→4382):** `purchase-orders-route.test.ts` (POST/PATCH/PUT-lines 10k+ notes→400 servis çağrılmaz + kısa→201) + `purchase-orders-ui.test.ts` (`isPoCancellable` pure helper + hover/cancellablePageIds/modal-a11y/fiyat source-regex). tsc/lint/build temiz. **Canlı E2E — SALT-OKUMA + 400 probe** (prod `ryvxpolvhvsycuqyphoa`, ssr-cookie; PO mutasyonu ASLA — geri alınamaz yan etki yok): sayfa 200 · liste 200 · **POST 10k+ üst-seviye notes→400 + nested lines[].notes→400 (yeni guard CANLI kanıt, DB write yok) + currency=GBP→400** → 🎉 SMOKE PASSED (5/5). **DURUM: çalışıldı, offline+canlı yeşil; COMMIT/PUSH kullanıcı onayı bekliyor.** Plan: `~/.claude/plans/stok-r-nler-sayfas-n-detayl-calm-charm.md` (PO içeriğiyle güncel).
