...
- **⚠️ Migration 099 APPLY (Supabase):** `quote_line_items.unit` kolonu + 4 RPC redefine (create/update/send/accept). Uygulanana dek teklif satır birimi DB'ye yazılmaz (`unit` kolonu yok → INSERT hatası VEYA sessizce kaybolur). `npx tsx scripts/check-migrations.ts` 099'u column-probe ile doğrular. Smoke: ürün seç→birim otomatik dolar→kaydet→yenile (korunur); Önizle/PDF "12 metre"; Kabul et→siparişte birim teklifle aynı (COALESCE).
