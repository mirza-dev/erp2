...
**Son tamamlanan iş:** Teklif V7 **Faz 4 — PDF Arşiv** (gönderilmiş teklifin immutable dondurulmuş HTML snapshot'ı) **+ Bulgular review 1. + 2. tur** (5+5 bulgu doğrulandı+düzeltildi), 3960 test, **COMMIT+PUSH BEKLİYOR + migration 075/076 APPLY BEKLİYOR**

- **Review 2. tur (Bulgular, 2026-05-30):** **B1 (P2) müşteri adresi** gönderimde zorunlu (validateQuoteForSend "resmi PDF") ama belgede yoktu → 4 nokta eklendi: `QuoteData.custAddress` + `BILINGUAL_LABELS.address` (Adres/Address) + QuoteDocument müşteri satırı + buildQuoteDataFromDetail + **QuoteForm autoSave/savePreviewData** (canlı önizleme — drift trap, advisor yakaladı). **B2 (P2) sent silme** → `canDeleteQuote` + DELETE route `draft`-only (immutable arşiv ON DELETE CASCADE koruması; sent-deletable regression testi tersine). **B3 (P2/P3)** archive-fail toast yanıltıcı "otomatik denenecek" vaadi kaldırıldı (gerçek recover yalnız Faz 6). **B4 (P3)** buton arşivsiz statüde → graceful 404 (info toast) zaten var, `sentAt` map'siz, P3 kabul. **B5 (P3)** doc test sayısı hizalandı. **3951→3960** · tsc/build temiz · eslint src 31/0.
