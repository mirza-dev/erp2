...
**Son tamamlanan iş:** **Quotes modülü derin denetim (erp2-reviewer kampanyası B) + O1 düzeltmesi** (2026-06-18; GREEN; PUSH BEKLİYOR; **migration GEREKMEZ**). REVIEW.md kurallarıyla read-only inceleme (subagent restart gerektirdiğinden tam bağlamla doğrudan); rapor `docs/audit/2026-06-18-quotes-review-bulgular.md` (**K:0 Y:0 O:1 D:1 Nit:1**). **O1 (Orta):** iki ayrı teklif-sona-eriş cron'u çakışıyordu — `crons.yml` hem canonical `/api/quotes/expire` (`quote-service.serviceExpireQuotes`, yeni `quotes` tablosu) hem legacy `/api/orders/expire-quotes` (`order-service.serviceExpireQuotes`, eski `sales_orders.quote_valid_until` modeli; 088/077 bu kolonu yazar) çağırıyordu → her sona eren sent teklif için **çift `quote_expired` alert** (entity=quote + entity=sales_order, farklı entity → dedup'a takılmaz) + `quote_valid_until` geçmiş **draft siparişin bant-dışı iptali**. Önceki audit "legacy duplikatı sil" demiş, yapılmamıştı. **Kullanıcı kararı (AskUserQuestion): tamamen sil.** Silindi: `api/orders/expire-quotes/route.ts` + 2 test (`expire-quotes-route`/`expire-quotes-service`) + `order-service.serviceExpireQuotes` (+ ölü `dbCreateAlert`/`dbListActiveAlerts` import) + `orders.dbListExpiredQuotes` + `crons.yml` adımı/OUTCOMES + `proxy.ts` CRON_PATHS girişi; `smoke.ts [19]` canonical `/api/quotes/expire`'a yönlendirildi. Canonical `quote-service.serviceExpireQuotes`/`quotes.dbListExpiredQuotes`/`/api/quotes/expire` KORUNDU (088 modelini tam kapsar). **D1 (Düşük, zaten izlenen):** GET /api/quotes(+[id]) route-guard'sız (redaction-only) — `project_quotes` "GET view_quotes RBAC" borcu, bu turda değil. **Nit:** preview-pdf filename header interpolasyonu (teorik, düşük). **Temiz doğrulananlar:** share-token (HMAC/timing-safe/fail-closed), public shared route (token-gated/noindex), arşiv HTML (React-escape, raw concat yok), tüm mutasyonlarda RBAC, HTTP-map'ler, sunucu-otoriter validasyon, redaction parity, demo proxy-blok. tsc 0 · lint 0 · **5536 test** (−9: 2 test dosyası silindi; ayrıca A1 artığı ölü `expectPaginationWired` helper'ı temizlendi) · build 0. **Kalan:** push + (opsiyonel) canlı smoke (sona eren sent teklif artık TEK alert; kabul-draft sipariş cron'da iptal olmaz).
