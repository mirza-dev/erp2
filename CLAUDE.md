...
**Son tamamlanan iş:** **Roven hexagon logo — mark + wordmark + ince ayar** (`f3cb649` + `1b760d6`). Kullanıcı görsel paylaştı: yuvarlatılmış hexagon + "Roven" wordmark, koyu zeminde near-white. `/frontend-design` yönü: minimal geometrik, monokrom, tema-uyumlu. **Yapılan:** Yeni `src/components/layout/RovenLogo.tsx` (tek kaynak) — hexagon `<polygon>` SVG + bold wordmark; `currentColor` → otomatik temalanır; Props `size/showWordmark/wordmarkSize/gap`. Topbar `size=20 wordmarkSize=17 gap=6` (ince ayar: daha büyük + sıkı lockup, sayfa genelinde görünür). Landing + login da RovenLogo. `src/app/icon.svg` favicon. Tema-MUAF değil — `var(--text-primary)` üzerinden near-white↔koyu. **+10 test** (`roven-logo.test.tsx`). tsc 0 · lint 0 · 4688 test · build 0. **DURUM: PUSH EDİLDİ (`1b760d6`) — birebir aynı SHA; iki Coolify prod deploy.**
