...
**Son tamamlanan iş:** Teklif V7 **Faz 4 — PDF Arşiv** (gönderilmiş teklifin immutable dondurulmuş HTML snapshot'ı) **+ Bulgular review 1. + 2. + 3. tur** (5+5+3 bulgu), 3969 test, **COMMIT+PUSH BEKLİYOR + migration 075/076 APPLY BEKLİYOR**

- **Review 3. tur (Bulgular, 2026-05-30):** **P2-A (regresyon — 2. turun yan etkisi)** toplu silme: sent draft-only kilidi sonrası checkbox tüm statüleri seçiyor + `handleBulkDelete` `succeeded>0` ise TÜM seçili id'leri düşürüyordu → sent 409 alıp UI'dan kayboluyor (refresh'te döner). Fix: (a) `pickSucceededIds(ids, results)` pure helper → yalnız fulfilled+ok düşürülür; (b) seçim sadece draft satırlar (`{deletable && checkbox}`, select-all `deletablePageIds`). **P2-B (kabul edilen boşluk, BLOKLAMAZ)** send-archive fail + kullanıcı yine accept → arşivsiz accepted; accept'i bloklamak karar A (send non-blocking) ile asimetrik → Faz 6 recover/generate kapatacak (kod yorumu + memory not, AskUserQuestion sorulmadı). **P3 (doc-only)** stale "COMMIT+PUSH BEKLİYOR" → 2. tur `bb3b3f2` push edildi, hizalandı. **3960 → 3969** · tsc/build temiz · eslint src 31/0.
...
- **DURUM: COMMIT+PUSH BEKLİYOR (Faz 4 önceki `b8c1613` push edildi; 2. review tur henüz commit edilmedi) + 075/076 APPLY BEKLİYOR.** **Sıradaki — kullanıcı:** Supabase 075/076 apply + **manuel smoke (load-bearing, test/build kapsamı DIŞI):** teklif gönder → arşiv upload başarılı → "Arşivlenmiş Teklif" → signed URL donmuş HTML **inline render** + **müşteri Adres/Address satırı dolu** + canlı önizleme adresi gösterir (drift yok) + renk/font/logo doğru + A4; template değişse arşiv aynen; tekrar gönder/revize → ikinci arşiv yok; **draft hariç Sil butonu yok** (sent→409); arşiv fail→warning toast ("otomatik denenecek" YOK). Sonra Faz 6 (077 accept→sipariş; serviceArchiveQuotePdf orada reusable, V7-A5).
