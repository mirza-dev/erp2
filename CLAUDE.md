...
**Son tamamlanan iş:** **Stok defteri / Inventory derin denetim (erp2-reviewer) + 2 düzeltme** (2026-06-19; GREEN; PUSH EDİLDİ `4179fd8` [main = codex-experiment = origin, tree-hash `17f61de0` özdeş, ıraksama 0 0]; **migration 105 YENİ → APPLY BEKLİYOR**). Rapor `docs/audit/2026-06-19-inventory-review-bulgular.md`. **No blocking issues — K:0 Y:0 O:0**; tüm stok mutasyonları atomik guard'lı RPC'lerden geçiyor (`record_stock_movement` negatif-stok reddi · `increment_reserved` `least(on_hand,…)` cap · `adjust/decrement_on_hand` `greatest(0,…)` · `record_stock_transfer` ayrı `stock_location_balances`+`FOR UPDATE`, `on_hand`'e dokunmaz · demo merkezi `proxy.ts:207-209` · RBAC A3). **D-O1 (DÜZELTİLDİ):** import stok **sayımı** `delta = quantity - prod.on_hand`'i RPC transaction'ı **dışında** okuyordu → eşzamanlı harekette `on_hand ≠ sayılan` (lost update). YENİ `recount_stock` RPC (**mig 105**, `FOR UPDATE` + **mutlak** on_hand ataması + delta hareketi; negatif red, delta=0 no-op) + wrapper `dbRecountStock` (products.ts); `import-service` sayım dalı JS-delta yerine bunu çağırır (hareket/transfer yolları değişmedi). **D2 (DÜZELTİLDİ — kaldırıldı):** sıfır-çağıranlı ölü atomik-olmayan yardımcılar — `dbRecordMovement` (products) + `orders.ts` tüm `DEPRECATED` blok (`StockConflict`/`dbGetProductStocks`/`dbReserveStock`/`dbReleaseStock`/`dbShipOrder`); footgun temizliği. **Gate notu:** `recount_stock` tek-migration → `REDEFINITION_CHAINS`'e EKLENMEZ (hayalet-zincir gate'ini bozar); `SECURITY DEFINER` yok → DEFINER gate tetiklenmez. tsc 0 · lint 0 · **5581 test** (+1) · build 0. **Kalan:** mig 105 APPLY (Studio) + `check-migrations` probe.

<details><summary>Önceki: codex-experiment aynası: fast-mutation + tema-hydration → mirror</summary>

**codex-experiment aynası: fast-mutation + tema-hydration özelliği main'e adapte edildi → birebir mirror** (2026-06-19; GREEN; PUSH EDİLDİ `a6f002a` [main = codex-experiment = origin, tree-hash `ff198da9` özdeş, ıraksama 0]; **migration YOK**). proje-codex worktree'sinde commit edilmemiş gerçek bir özellik vardı (mirror FF'i bozmuştu); kullanıcı kararı (AskUserQuestion): özelliği main'e adapte et + iki branch'i birebir mirror yap, hiçbir şey kaybolmasın, no semantic errors. **Adapte edilen iş:** (1) **fast-mutation/optimistic-UI** — YENİ `src/lib/fast-mutation.ts` (saf helper: successfulResponseIds/decrementCount/patchCountRecord/removeByIds/upsertFirst) + 2 yeni test; `data-context.tsx` (optimistic liste güncelleme + `updateCustomer`→`Customer|undefined` + `addUretimKaydi`→`{entry?}` + üretim revalidation'ı **bloklamayan arka-plana**); 5 liste client (Orders/Customers/Purchase/Quotes/Vendors) `router.refresh()` yerine optimistic local state + `mutate(COUNTERS_KEY/PRODUCTS_KEY)`; `CustomerDetailPanel.onCustomerUpdated`. (2) **Tema hydration guard** — `ThemeToggle` + login `Chrome` `mounted` flag (SSR/client ikon mismatch). **Reconciliation:** feature dosyaları codex→erp2 verbatim kopya (0a7e7d1↔0cf441a'da AYNI→çakışmasız); login/globals/login-test **main otoriter** (D1/D2/Nit fix'lerim KORUNDU), yalnız `mounted` guard elle eklendi. **Mirror:** `git -C proje-codex reset --hard main`. **İnline REVIEW.md denetim TEMİZ** (no semantic errors; production `refetchFailed` uyarısı artık fire etmez = bilinçli, optimistic add ile gereksiz, zararsız dead-code bırakıldı; demo guard'lar korundu; stok defteri UI-katmanı, server source-of-truth). tsc 0 · lint 0 · **5580 test** (+10) · build 0.
</details>

<details><summary>Önceki: Landing + login UI denetimi + 4 düzeltme</summary>

**Landing + login UI denetimi (erp2-reviewer) + 4 düzeltme** (2026-06-19; GREEN; PUSH EDİLDİ `0cf441a`; **migration YOK**). REVIEW.md read-only (landing `page.tsx` + login `page.tsx` + callback + globals login CSS). **No blocking issues** — K:0 Y:0 O:0 D:2 Nit:2. Landing tema-pinli pazarlama (belgeli istisna), login tema-duyarlı CSS-var, Tailwind/framer yok, React-escape, callback redirect relative/sabit. **Düzeltmeler (hepsi `login/page.tsx`, kullanıcı kararı):** **D1** `attempted` URL param'ı doğrulanmadan hata kutusuna yansıyordu (XSS yok ama crafted link keyfi metin gösterir) → `isEmail(attempted)` guard, değilse genel mesaj; **D2** boş `<a href="mailto:">` → düz metin (`.login-foot-em` accent stili eklendi, sahte link kalktı); **Nit-1** `role="alert"`+`aria-live="polite"` çakışması → aria-live kaldırıldı; **Nit-2** şifre-sıfırlama hatası `errAuth` gösteriyordu → yeni `errReset` mesajı. +2 test (D1 malicious-attempted yansımaz + Nit-2 reset mesajı) + 1 mevcut test güncellendi (aria-live assertion). tsc 0 · lint 0 · **5570 test** · build 0. (Landing'deki sabit hex = bilinçli tema-pinli, bulgu değil.)

</details>

<details><summary>Önceki: C1 — login brick-risk preflight + checklist</summary>

**C1 — Login brick-risk preflight + checklist + kurtarma runbook** (2026-06-19; GREEN; PUSH EDİLDİ `0a7e7d1`; **migration YOK; runtime kodu DEĞİŞMEZ**). Rapor `docs/audit/2026-06-19-c1-login-preflight.md`. **Brick modeli (kod izli):** `proxy.ts:241` her oturumu `isProvisionedUser`'la süzer; kurtarma kolu (`/dashboard/settings/users`) admin ister = `parseRoles ∋ admin` (`permissions.ts:142`). Deploy sonrası hiçbir `auth.users`'ta `app_metadata.roles ∋ admin` YOK + prod `ADMIN_EMAILS` boşsa → kimse admin değil → kimse provize edilemez → **BRICK** (password login bile kurtarmaz). **Kullanıcı kararı (AskUserQuestion): brick-risk preflight + checklist** (LoginMonolith UI redesign DEĞİL). **Çözüm (D'nin `check-migrations` deseni):** YENİ `scripts/check-auth-preflight.ts` (read-only `auth.admin.listUsers` + gerçek `parseRoles`/`isProvisionedUser` import → kalıcı admin [env-bağımsız] / bootstrap admin [ADMIN_EMAILS] / mis-provision [user_metadata rol var app_metadata yok] sayar; kalıcı+bootstrap==0→EXIT 1 BRICK, kalıcı==0&bootstrap>0→⚠️, listUsers hata→fail-closed exit 1) + `npm run preflight:auth` + doc (§1 brick modeli §2 script §3 manuel checklist [ADMIN_EMAILS her iki Coolify env + Supabase signups-OFF + OAuth redirect URLs + tarayıcı smoke] §4 kurtarma [create-admin/ADMIN_EMAILS/Studio]). **Canlı koşu: 3 kalıcı admin → prod brick-korumalı.** tsc 0 · lint 0 · test 5568 (değişmedi) · build (runtime kodu yok). **Sınır:** AI tarafı (preflight+brick modeli) kapandı; tarayıcı smoke + Coolify/Supabase dashboard ayarları kullanıcı-tarafı (prod ADMIN_EMAILS script'çe görülemez). Kalan backlog: C2 Paraşüt Faz12 (kullanıcının Paraşüt API'si yok → ertelendi).

</details>

<details><summary>Önceki: A2 — rate-limit sertleştirme (IP spoof + AI-route Redis)</summary>

**A2 — Rate-limit sertleştirme: IP spoof fix + AI-route Redis-backed (Upstash YOK)** (2026-06-19; GREEN; PUSH EDİLDİ `aae8fd4`; **migration YOK**). Denetim **O5** kapanışı. **Bağlam:** M-3 turunda tam ioredis Redis-backed middleware rate-limit zaten kuruluydu (`rate-limit.ts` + `proxy.ts`); "Upstash" adı bayat. O5'in iki gerçek artık-açığı: (1) **IP spoof** — `extractClientIp` XFF'in **soldaki (client-kontrollü)** değerini alıyordu (Traefik gerçek peer'ı append eder → spoof edilebilir); (2) **`ai-route-limit.ts` in-memory per-instance** (çoklu-instance'da etkisiz, deploy'da sıfırlanır). **Kullanıcı kararı (AskUserQuestion): mevcut altyapıyı sertleştir, yeni dep/infra YOK.** **Düzeltme:** (1) `request-ip.ts` → **X-Real-IP primary** (Traefik-set, spoof edilemez) + XFF **en sağdaki (son) hop** fallback + hafif IP doğrulaması (yanlış-key worst-case over-limit=güvenli); (2) `guardAiRoute` **async hibrit** — önce paylaşımlı ioredis Redis (`rateLimitCheck` + yeni `aiRoutePolicy(route,limit)` helper), `fromRedis=true` otoriter (429/allow), `fromRedis=false` (Redis yok/down/circuit) → mevcut **in-memory `checkAiRateLimit` fallback** (defense-in-depth korunur); 5 AI route'a `await` eklendi. Tradeoff: ai-route-limit artık ioredis taşıyan rate-limit'i import eder (`extractClientIp` yine request-ip'ten). +6 test (request-ip spoof-direnci yeniden yazıldı [eski "ilki alınır" O5 hatasını kodluyordu] + ai-route Redis-primary/fallback). tsc 0 · lint 0 · **5568 test** · build 0. **Sınır:** prod Traefik X-Real-IP set etmiyorsa keying farklılaşır (over-limit; ayrı smoke); REDIS_URL prod'da set olmalı ki AI limit paylaşılsın. Kalan backlog: C1 Login canlı · C2 Paraşüt Faz12.

</details>

<details><summary>Önceki: D — migration durumu + gate hygiene + smoke checklist</summary>

**D — Migration durumu kesinleştirildi + gate hygiene (mig.104) + birikmiş smoke checklist** (2026-06-19; GREEN; PUSH EDİLDİ `9dc8c33`; **migration YOK; runtime kodu değişmedi**). Rapor `docs/audit/2026-06-19-d-migration-smoke.md`. **`npx tsx scripts/check-migrations.ts` → 17/17 auto-probe ✅ GREEN** (073…100 tüm tablo/kolon/RPC canlıda → eski "088 BLOKER / 091 APPLY bekliyor" notları BAYAT). **Gate hygiene:** `check-migrations.ts` MANUAL map'ine **mig.104** (`reverse_production` REDEFINE — production O1 `for update` fix'i) eklendi; önceden hiç izlenmiyordu → artık `⚠️ 104 … elle doğrula` satırı raporlanır (sessiz untracked kapandı; `manuel: 7→8`). **Smoke doc 3 bölüm:** §1 probe sonucu; §2 8 MANUAL redefine doğrulama SQL'i (089/093/094/095/101/102/103/104 — Studio, kullanıcı; OpenAPI fonksiyon gövdesi görmez; ortamda DATABASE_URL/psql yok); §3 birikmiş browser smoke checklist. **Sınır:** AI tarafı (probe+gate) kapandı, kalan iş kaçınılmaz kullanıcı-tarafı (MANUAL SQL + tarayıcı smoke).

</details>

<details><summary>Önceki: A3 — route-guard gate method-seviye + 3 guard</summary>

**A3 — route-guard gate METHOD-SEVİYEYE çıkarıldı + yakaladığı 3 açık kapatıldı** (2026-06-19; GREEN; PUSH BEKLİYOR; **migration YOK**). Rapor `docs/audit/2026-06-19-a3-method-level-guard-gate.md`. **Sorun:** `route-guard-matrix.test.ts` `guarded`'ı dosya-seviye (`src.includes`) hesaplıyordu → bir method guard'lıysa tüm dosya "korunmuş" sayılıyor, guard'sız kardeş GET görünmüyordu (kampanya B'nin 9 modülde elle bulduğu sınıf: import/orders/customers/products-quotes/alerts O1/D1). **Çözüm:** gate yeniden yazıldı — her exported method gövdesi ayrı taranır + **file-local guard-helper çözümü** (gövdesinde guard içeren dosya-yerel fn/const, brace-eşlemeli `blockAfter` → calendar-notes `context()` çözülür; admin/users `requireAdmin(` direkt pattern) + `requireCronSecret(` pattern'e eklendi (email/outbox/process robustça guarded→baseline'dan düştü). `route-guard-baseline.ts` **method-anahtarlı** (methods=kasıtlı guard'sız method listesi; per (path,method) violation+stale). 135 route→26 guard'sız method sınıflandırıldı (dashboard-tier/config/collateral/self-auth/public/tombstone). **Yakaladığı 3 GERÇEK açık → guard eklendi:** `GET /api/quotes` + `GET /api/quotes/[id]` → `view_quotes` (İZLENEN borç kapandı; production+purchasing quote pipeline [müşteri+teklif no+tarih] okuyabiliyordu, redaction yalnız fiyat maskeler; dashboard KPI fail-soft, quotes sayfaları view_quotes-gated→consumer-safe); `GET /api/inventory/movements` → `view_products` (UI tüketicisi yok, accounting/anon'a açıktı). +9 test (YENİ `quotes-inventory-read-guards.test.ts`). tsc 0 · lint 0 · **5562 test** · build 0. **Gate artık gelecekteki TÜM method-seviye guard kör noktalarını PR'da yakalar.** Kalan backlog: A2 Upstash · C1 Login canlı · C2 Paraşüt Faz12 · D migration/smoke.

</details>

<details><summary>Önceki: settings modülü derin denetim TEMİZ + KAMPANYA B TAMAMLANDI</summary>

**settings modülü derin denetim — TEMİZ + erp2-reviewer KAMPANYA B TAMAMLANDI** (2026-06-19; PUSH EDİLDİ `f337856`; **kod değişikliği YOK — yalnız rapor + memory**). REVIEW.md read-only (10 settings route + admin/users + lib + sayfalar); rapor `docs/audit/2026-06-19-settings-review-bulgular.md` (**K:0 Y:0 O:0 D:0 Nit:0 — kampanyanın en olgun modülü**). Bulgu manufacture EDİLMEDİ. **Temiz doğrulananlar:** `admin/users` POST/PATCH/DELETE → `requireAdmin` (app_metadata.roles∋admin + zero-admin bootstrap + **listUsers HATASI fail-CLOSED**) + `normalizeAssignedRoles` (geçersiz rol atılır→privilege injection yok) + **last-admin lockout** (fail-closed countAdmins); `api-keys-status` → `requireInternalOperator` + demo→false + yalnız boolean; `company` GET guard'sız ama **SAFE_COMPANY_FIELDS whitelist** (antet/branding, secret yok — PDF/başlık için view-tier; PATCH/logo manage_settings+validation); `files` GET/download view_settings · POST/DELETE manage_settings + MIME/size/kategori + **SVG-attachment XSS defense** (mig.046); `user/password` → **mevcut-şifre doğrulama** (izole client) + audit; avatar PNG/JPEG/WebP(SVG yok)+1MB; profile/preferences self-auth+validation; **view_settings/manage_settings yalnız admin**. **KAMPANYA B (modül-modül derin inceleme) TAMAMLANDI** — 9 modül: RFQ·Orders·Quotes·Paraşüt·import/AI·production·customers/products·alerts·settings. **Kalan backlog (B dışı):** A2 Upstash rate-limit · A3 gate method-seviye guard matrisi · C1 Login canlı tur (brick riski) · C2 Paraşüt Faz 12 Sandbox · D migration/smoke.
</details>

<details><summary>Önceki: alerts modülü derin denetim + D1 (GET /api/alerts/[id] view_alerts)</summary>

**alerts modülü derin denetim (erp2-reviewer kampanyası B) + D1 düzeltmesi** (2026-06-19; GREEN; PUSH EDİLDİ `e2f8bc1`; **migration YOK**). REVIEW.md read-only (6 alert route + calendar-notes + service + helper'lar + sayfa/component); rapor `docs/audit/2026-06-19-alerts-review-bulgular.md` (**K:0 Y:0 O:0 D:1 Nit:0** — modül çok olgun). **D1 (Düşük):** `GET /api/alerts/[id]` `view_alerts` guard'sızdı → `dbGetAlertById` `select("*")` tam satır (ai_reason + ai_inputs_summary + serbest `user_note` + created_by); liste GET bilinçli DAR kolon + dashboard-tier ama `[id]` detay tam satırı verir. Kardeş route'lar (calendar GET, calendar-notes+[id], [id] PATCH) hepsi view_alerts/manage_alerts guard'lı; bu GET method-seviye kör noktada atlanmış → accounting (view_alerts YOK) + proxy-fail-open/anon tam detayı okuyabiliyordu. **GET'in UI tüketicisi YOK** (alerts/page.tsx tüm `/api/alerts/${id}` çağrıları PATCH) → guard hiçbir şeyi kırmaz, yalnız accounting/anon kapanır. **Düzeltme:** GET'e `requirePermission(view_alerts)` (PATCH + calendar kardeş kalıbı). **By-design (bulgu değil):** liste GET dashboard-tier (accounting AlertsPanel `useAlerts`); scan CRON_SECRET veya oturum (products mount'unda tüm view_products rollerinde oto-tetik → session-tier zorunlu, idempotent+locked+non-destructive); ai-suggest cron-only (`requireCronSecret`); calendar-notes session+view_alerts+ownership/visibility+validation (örnek-temiz); sync-retry manage_alerts. **Temiz:** AI üretimi G1/G2 sanitize + dedup; advisory-lock'lar. +3 test (YENİ `alerts-read-guards.test.ts`). tsc 0 · lint 0 · **5554 test** · build 0. **Kalan:** opsiyonel smoke (accounting→`GET /api/alerts/<id>` 403; alerts sayfası+dashboard tüm rollerde normal).
</details>
