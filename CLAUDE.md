...
**Son tamamlanan iş:** Teklif V7 **Faz 7 — Not Şablonları (note_templates)** — migration 079, QuoteForm picker + admin CRUD, 4094 test, **COMMIT+PUSH BEKLİYOR · migration 079 APPLY BEKLİYOR** (V7 master-plan TAMAMLANDI)

- **Faz 7 (2026-05-31, V7 master-plan'ın SON fazı):** PMT teklif formunun 3 serbest-metin alanı (Notlar/Teslimat/Ödeme) için tekrar kullanılabilir not şablonları. **080 KALICI DÜŞÜRÜLDÜ** — master-plan `079→quote_line_items_sort_order (koşullu)` diyordu ama `quote_line_items.position` zaten var (034:106) + `quotes.ts:88` ona göre sıralıyor + accept RPC ona göre order'lıyor → koşulsuz gereksiz; drag-reorder ertelendi → Faz 7 = **tek migration 079**. **Migration 079** (`note_templates`: kind `notes|delivery|payment|general` CHECK + title/body non-empty + sort_order + is_active soft-delete + RLS + updated_at trigger + partial index + **PMT seed** deterministik UUID/ON CONFLICT). **Erişim ayrımı (load-bearing):** `GET` requireRole YOK (satış picker'da okur) + `?kind=`; `POST/PATCH/DELETE` admin; DELETE soft-delete (hard-delete YOK). **Helper** `note-templates.ts` (validation + audit_log + isValidNoteTemplateKind). **TS/mapper** NoteTemplateRow/NoteTemplate/mapNoteTemplate. **Settings sayfası** `/dashboard/settings/note-templates` (kind filtre + create/edit modal + pasifleştir) + Sidebar linki. **QuoteForm picker:** 3 textarea üstü "+ Şablon ekle…" select; `templatesForField` (kind+general); `applyTemplateToField` (boş→doldur/dolu→append, sessiz üzerine-yazma YOK); readOnly'de gizli; setter'lar zaten autoSave dep'inde (drift trap yok). **Test (+51): 4043→4094** · tsc temiz · npm run lint 0 · build OK (`ƒ /api/note-templates` + `[id]` + settings + `ƒ Proxy`). **DURUM: COMMIT+PUSH BEKLİYOR · 079 APPLY BEKLİYOR.** **V7 master-plan Faz 1-7 + tüm Bulgular turları TAMAMLANDI.**
