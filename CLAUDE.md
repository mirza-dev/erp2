...
(B) Demo e-posta nötrleştirme + arşiv render fix detayı yukarıdaki blokta; `d9ef1c3` mirror'a push edildi.</details></details></details></details></details>

### Açık yükümlülükler (kullanıcı doğrulamalı — yeşil testler kapsamaz)
- **Migration 089 APPLY (Supabase):** `alerts_type_check` += po_overdue. Uygulanana dek po_overdue uyarıları üretilmez (scan non-fatal yutuyor, stok taraması etkilenmez); quote_expired/V7 ve diğer tüm değişiklikler 089'suz da çalışır.
