...
**Son tamamlanan iş:** **Teklif "Gönder" → müşteriye HTML ekli e-posta** (2026-06-09; **PUSH EDİLDİ**, smoke bekliyor). Kullanıcı isteği: teklif "Gönder"e basınca müşteriye teklif belgesi e-posta ile gitsin. **Kullanıcı kararları:** ek formatı = **HTML eki** (sistemde binary PDF yok — dondurulmuş arşiv HTML; gerçek PDF chromium/dış-API gerektirir, bilinçle reddedildi); tetik = **"Gönder" onayına checkbox** (transition'a bağlı tek-sefer, varsayılan işaretli). **Mimari:** transition (status+arşiv) saf kalır; e-posta ayrı reusable endpoint. **Yapılanlar (6 kod + 6 test):** (1) `sendDirectEmail` attachment primitifi (`email-service.ts`; `getResend`/`getEmailFrom` reuse, config eksik→`{ok:false,config_missing}`, loglamaz); (2) `renderQuoteToCustomer` müşteri şablonu (`templates.ts`; `shell()`'e `hideManageFooter` eklendi → müşteriye giden e-postada dashboard "bildirim tercihleri" footer'ı GİZLİ; XSS escape); (3) `serviceSendQuoteToCustomer` (`quote-service.ts`; **arşivle birebir pipeline** mapQuoteDetail→buildQuoteDataFromDetail→renderQuoteArchiveHtml, e-posta yok→`no_email`, email_logs entity_type='quote'); (4) **`POST /api/quotes/[id]/send-email`** (`manage_quotes` RBAC = send transition ile aynı; notFound→404/no_email→400/config→503/Resend→502); (5) generic retry'dan quote dışlama (`email-logs.ts` `dbListFailedEmailsForRetry` **NULL-safe `.or("entity_type.is.null,entity_type.neq.quote")`** — düz `.neq` NULL satırları yutardı; ek yeniden eklenemez); (6) frontend (`[id]/page.tsx`+`quote-display.ts`): draft "Gönder" aksiyonuna confirm + onayda "Müşteriye e-posta da gönder" checkbox (e-posta yoksa disabled+uyarı), başarılı transition SONRASI `/send-email` POST. **ADVISOR done-check (bug yok, 4 not):** (#1) bu "done"=kod-complete+green AMA uçtan uca DOĞRULANMADI — EMAIL_FROM altyapısına biniyor (kullanıcı EMAIL_FROM'u düzeltti); (#2) **`.html` eki Exchange/Outlook'ta strip/karantina edilebilir** (PMT B2B müşteriler risk grubunda) → **kullanıcı kararı: önce smoke ile ölç** (Gmail+Outlook), sorunsa PDF-API yoluna geç; (#3) RBAC asimetri kontrol→temiz (`manage_quotes`=admin+sales, ikisi de `view_sales_prices` tutar); (#4) e-posta eki frozen arşiv yerine yeniden render (normal akışta byte-identical+archiveWarning'de de sağlam, ama gelecek "Tekrar Gönder" frozen arşivi eklemeli — kod yorumuna not). tsc 0 · lint 0 · **4999 test** (önceki+88 yeni; draft-confirm regression testi güncellendi) · build 0 (`ƒ /api/quotes/[id]/send-email`+`ƒ Proxy`). Migration YOK (mevcut `email_logs`+`customer_email` yeterli). **Sıradaki:** smoke — Aşama 1 EMAIL_FROM doğrula (`/api/email/test`), Aşama 2 gerçek teklif→kendi Gmail+Outlook→`.html` eki geliyor mu/spam mı. Önceki turdaki **drift fix** (`458c14b`: CTA fallback URL+`.env.example`→`erp.getmedspace.com`, KokpitERP→Roven) bu push'tan önce mirror'a gitmişti. <details><summary>Önceki: Genel Bakış — panel yerleşimi yeniden düzenlendi (drift fix `458c14b`)</summary>

**Son tamamlanan iş:** **Genel Bakış — panel yerleşimi yeniden düzenlendi** (2026-06-09; GREEN, PUSH BEKLİYOR). Kullanıcı isteği: paneller yeni hizaya alınsın. `src/app/dashboard/page.tsx` salt-JSX yeniden düzenleme (veri/RBAC/KDV/import dokunulmadı — `feedback_no_silent_deletes`): eski tek `overview-grid-1-1`+iki dikey flex-kolon → **Satır 1** `StockPanel`|`FinancePanel` (grid 1fr 1fr) + **Satır 2** `ProductionPanel`|`OrdersPanel` (grid 1fr 1fr) + **alt alta tam genişlik** `ReorderPanel`→`AlertsPanel`→`AiPanel` (dikey flex). `overview-grid-1-1` zaten `@media(max-width:1180px)` tek-kolona düşer (mobil korunur); `align-items:start` farklı yükseklikli yan-yana paneller için doğru. Paneller artık doğrudan grid hücresi (iç flex-kolon sarmalayıcıları kalktı). `dashboard-overview-preservation.test.ts`'e +4 source-regex (2× grid satırı, Stok<Finans, Üretim<Sipariş, Reorder<Alerts<AiPanel sıralaması) — eski grup-gruplamasını asserte eden test yoktu → kırılma yok. tsc 0 · lint 0 · build 0 (`○ /dashboard`) · preservation testi 14/14. **Sıradaki:** mirror push onayı.

</details>

<details><summary>Önceki: Genel Bakış TAM-SADIK yeniden kurulum (`b2481a1`)</summary>

**Son tamamlanan iş:** **Genel Bakış (Executive Dashboard) — TAM-SADIK yeniden kurulum** (2026-06-09; **GREEN, PUSH BEKLİYOR**). Kullanıcı geri bildirimi: Faz 1 tasarıma ne görünüm ne işlev uyuyordu (5/6 KPI, maliyet kapalı, Finansal Özet+Üretim panelleri YOK, uydurma link kartları) → yeni direktif **`roven-dashboard 2/` tasarımına BİREBİR + EKSİKSİZ uy; eksik backend'i kur**. **Görsel doğrulama (kullanıcı istedi, Playwright login→screenshot):** 6 KPI şeridi + Ciro&Maliyet trendi(maliyet kesik çizgi) + Finansal Özet(brüt kâr+money-flow+AgingBars) + Üretim BarChart + Son Siparişler + Stok Donut + Öneriler + Uyarılar + AI = `DashDetailed` BİREBİR; koyu+aydınlık+mobil tek-kolon doğrulandı. **Yapılanlar:** **A backend** — mig.087 `dashboard_monthly_cogs` RPC (`Σ(order_lines.qty × products.cost_price)` ay+ürün-para, iptal/taslak hariç) + `dashboard-finance.ts` + **`GET /api/dashboard/finance`** (reportingCurrency=`company_settings.currency` default USD; `view_purchase_costs` → COGS, yoksa null); **canlı API gerçek EUR+USD COGS döndü → mig.087 KULLANICI APPLY ETMİŞ ✅**. **B view-model** (`dashboard-view-model.ts` yeniden yazıldı) — **tek raporlama para birimine normalizasyon** (`toReporting` orta kur, eksik→değişmez; eski `MoneyByCurrency` helper'ları KORUNDU=no-silent-delete); `monthlyRevenueReporting`/`cogsToReporting`/`stockValueByCategoryReporting`/`receivablesAging`(siparişten türev dürüst)/`financeSummary`/`productionDailySeries`(gerçek scrap)/`recentOrdersView`(normalize+RBAC)/`aiPointsFromOpsSummary`; **`buildKpis` → 6 KPI** (Açık Alacak eklendi). **RBAC:** sales/purchase_costs/financial_summary → "—". **C üretim fire** — `UretimKaydi+=scrap`, `mapProductionEntry+=scrap_qty` (kolon mig.001'den beri var). **D bileşenler** — sadık `KpiCard`(ok+trend ikonu); `FinancePanel`(brüt kâr+money-flow+AgingBars+dürüst etiket "ödeme entegrasyonu beklemede"; `canViewCosts` prop → yetki-yok vs veri-yok AYRI mesaj); `ProductionPanel`(good/scrap); `AiPanel`(collapsible kapalı→lazy `/api/ai/ops-summary` fetch+15dk cache→headline+2-kolon tonlu maddeler). **HeatPanel/MarginGauge KURULMADI** (tasarımda render edilmiyor). **E page.tsx** sadık `DashDetailed` (PageHeader segment **dekoratif**+Rapor-indir→`window.print()`; **shell.jsx PORT EDİLMEDİ**). **no-silent-delete:** link kartları kalktı ama Sidebar linkleri korunur (işlev kaybı yok); 5 bileşen dosyası repoda kalır; dashboard salt-bakış → `faz7-ui-masking`/`button-source-regression` CTA beklentileri güncellendi. **+yeni testler** (`overview-panels-render` 8, `dashboard-finance-route`; `dashboard-view-model` 35). **ADVISOR done-check (push-blocker yakaladı):** brüt kâr/marj KDV-dahil `grandTotal` ile vergisiz COGS'u karıştırıyordu → marj şişiyordu (%53). Fix: `grossToNetRevenue(gross)=gross/1.20` (KDV sabit %20), `financeSummary` NET ciro tabanı; KPI "Aylık Ciro"+trend BRÜT kalır (turnover konvansiyonu); FinancePanel "Ciro"→"Net Ciro" (dürüst etiket). Görsel doğrulandı: marj %53→**%44** (1−1.2×maliyet-oranı). KDV-tuzağı testi eklendi (green≠doğru ayrımı). tsc 0 · lint 0 · build 0 (`ƒ /api/dashboard/finance`+`○ /dashboard`+`ƒ Proxy`) · **4965 test** (4942→). **Kasıtlı tasarım sapması (danışman onaylı):** segment/Rapor-indir dekoratif (tasarımda da işlevsiz); Alacak=siparişten türev (fatura/ödeme tablosu yok). **DURUM: PUSH EDİLDİ** (`b2481a1` mirror, iki Coolify deploy). </details>

<details><summary>Önceki: Uyarılar → Takvim Görünümü — Faz 3 (cila/responsive/a11y) (PUSH `ddac29d`)</summary>

**Son tamamlanan iş:** **Uyarılar → Takvim Görünümü — Faz 3 (cila + responsive + a11y + animasyon)** (2026-06-08; **PUSH EDİLDİ** `ddac29d` — `origin/main == origin/codex-experiment == ddac29d` mirror, iki Coolify deploy; test status check bypass yetkiyle; React Doctor advisory bloklamadı). Faz 3'ün ~%80'i Faz 1'de zaten gönderilmişti (cal-fade/cal-pop-in/cal-fade-up stagger + slide-in-right + reduced-motion `*` guard + 260ms hover popover + role=tablist/aria-selected + aria-label/aria-pressed + tema-uyumlu #fff) → bu tur **gerçek kalan 4 kalem**: **(1) Responsive** (`src/app/dashboard/alerts/page.tsx` — `windowWidth` state SSR-safe init + resize listener, OrderForm precedent; `isMobile=<768` → `layoutStyle` `gridTemplateColumns:"1fr"`+`height:"auto"`+`overflow:"visible"` [advisor kritik: asıl risk grid değil height/overflow modeli — `100vh`+`overflow:hidden` mobilde KIRPAR, scroll olmaz] + `dayPanelStyle` `borderTop`+`maxHeight:50vh`; inert `.alerts-calendar-layout`/`.alerts-day-panel` className'leri korundu = inline-specificity sorunu `!important`siz çözüldü); **(2) Drawer focus-dönüşü** (`AlertCalendarDrawer.tsx` — mount'ta `document.activeElement` capture + Kapat butonuna ref ile ilk odak, unmount cleanup'ta tetikleyiciye `.focus()` geri; full focus-trap DROP=repo-tutarlı); **(3) Tema-audit** (alert bileşenlerinde `var(--...)` dışı hex YOK, yalnız onaylı #fff → test ile kilit); **(4) Backdrop `fade-in 0.2s`**. **+6 davranışsal test** (`alerts-calendar-faz3.test.tsx` RTL/jsdom: responsive mobil/masaüstü/resize 3 + focus-dönüşü 1 + tema-hex-yok 1 + reduced-motion guard 1; advisor: jsdom layout engine'i yok → görsel kırpılma tarayıcı-smoke, test koşullu DEĞERLERİ kilitler). tsc 0 · lint 0 · build 0 (`ƒ Proxy`) · **4899 test** (+6). **ALERTS_CALENDAR_PLAN Faz 0–3 TAMAMLANDI.** **Sıradaki:** Faz 3 PUSH + tarayıcı smoke (mobil tek-kolon tam scroll [kırpılma yok] + drawer kapanınca focus döner + koyu/aydınlık + reduced-motion).

</details>

<details><summary>Önceki: Uyarılar Takvim — Faz 2 (drawer zenginliği)</summary>

**Son tamamlanan iş:** **Uyarılar → Takvim Görünümü — Faz 2 (drawer zenginliği)** (2026-06-08; **PUSH EDİLDİ** — Faz 1 `4d55d9b` + Faz 2 `226e6c8` birlikte; `origin/main == origin/codex-experiment == 226e6c8` mirror, iki Coolify deploy; test status check bypass yetkiyle). `AlertCalendarDrawer`'a 3 tip-özel zengin bölüm + `onExtended`/`onShipped` parent callback (→ `refetch()`+toast): **quote_expired** süre uzatma formu (PATCH `/api/orders/[id]` `{quote_valid_until}`; `newDate≥bugün` validate; server `serviceUpdateQuoteDeadline`→`resolveQuoteExpiredAlerts` alert resolve eder → refetch yeterli); **overdue_shipment** inline sevk formu (shipDate zorunlu + tracking/carrier opsiyonel maxLength 100 → POST `/api/orders/[id]/ship`; endpoint Faz 7 awaited `dbBatchResolveAlerts` resolve eder); **order_shortage** İLGİLİ SİPARİŞLER (`useEffect` GET `/api/products/[id]/shortages` → loading/error/empty/list satır `aria-label` + üretim derin-linki `/dashboard/production?productId&qty=totalShortage` yeni sekme). Tümü `!isResolved`+`entityId` guard'lı; nav linkleri KORUNDU (`feedback_no_silent_deletes`); `Section` reuse; üretim qty = canlı `totalShortage` (eski `extractShortageQty` description-parse → davranış değişimi: tüm açık shortage toplamı). **ADVISOR done-check:** extend-resolution zinciri doğrulandı; source-regression yetmez → `alerts-calendar-render.test.tsx`'e 5 gerçek `renderToStaticMarkup` fixture (open→form / resolved→gizli / null entityId→nav fallback). 5 it.todo → 5 source-regression. tsc 0 · my-files lint 0 · build 0 · 13 alert dosyası/**109 test** yeşil. Repo kökü `dashboard/` scraped junk (charts.jsx/dashboard-app.jsx/data.js/uploads) + `lint` script `eslint src`→bare `eslint` regresyonu → `npm run lint` repo geneli kırmızıydı; **DÜZELTİLDİ**: script `eslint src`'e geri alındı + `eslint.config.mjs` globalIgnores'a `dashboard/**` (cwd-relative, kök dashboard; `src/app/dashboard` etkilenmez — doğrulandı) → `npm run lint` + `eslint .` ikisi de EXIT 0. **Sıradaki:** Faz 1+2 PUSH → Faz 3 (cila/responsive/a11y/animasyon).

</details>

<details><summary>Önceki: Uyarılar Takvim — Faz 1 (iskelet + enrichment + temel drawer)</summary>

**Son tamamlanan iş:** **Uyarılar → Takvim Görünümü — Faz 1** (2026-06-08; ÇALIŞILDI/green, COMMIT/PUSH onayı bekliyor). `design_handoff_alerts_calendar/` tasarımı (Apple Calendar tarzı aylık takvim) fazlı uygulanıyor; kalıcı plan **`ALERTS_CALENDAR_PLAN.md`** (repo kökü — "nerede kaldık?"). **Kullanıcı kararları:** hedef-tarih TAM ilk sürümde / drawer zenginliği ayrı faz / fazlı + MD kayıt. **Çerçeve:** prototip mock; gerçek `AlertRow` `dueDate`/`time`/`resolution` taşımaz → 2.967 satırlık olgun sayfanın takvime **yeniden-derilemesi** (RBAC/demo/scan/ai-suggest/Faz7 sevk/Faz10 shortage/sync-retry KORUNUR — `feedback_no_silent_deletes`). **Faz 1 yapılanlar:** `alert-calendar.ts` (YENİ pure — tarih/occurrence/tip/getMonthDays/SEVERITY_CONFIG/ALERT_CLASSES, +24 test); `alert-due-dates.ts` (YENİ — `enrichAlertsWithDueMeta` order-entity due TEK batch sales_orders join [overdue→planned_shipment_date / quote→quote_valid_until] + order_code; order_deadline due'su client'ta `productMap.orderDeadline`'dan, +7 test, MİGRASYON YOK); `GET /api/alerts/calendar` (YENİ — /api/alerts paterni + enrichment, eski /api/alerts sözleşmesi dokunulmadı, +3 test); `src/components/alerts/` (CalendarHeader[Tara+✦AI Analiz]/ClassificationTabs[role=tablist]/CalendarGrid+DayCell+DayPopover[portal]/DayDetailPanel+AlertCard[timeline]/AlertCalendarDrawer[role=dialog+aria-modal+ESC, nav linkleri + **gerçek sync retry**]/SevBadge — `@/components/ui/Button` reuse); **`page.tsx` TAM yeniden yazıldı** (`toCalendarAlert`/`applyClassFilter` export +13 test; tüm fetch/handler/demo/scan/ai-suggest/ack/resolve/dismiss/sync-retry KORUNDU; **dismissGroup→dismissDay** [gün toplu yoksay, 24h bypass korundu]; search+showResolved korundu; **100vh DEĞİL** `calc(100vh - 52px - 36px)` içerik-alanı); `globals.css` keyframes `cal-fade`/`cal-pop-in`/`cal-fade-up`. **Test migrasyonu:** sync-retry source-regression yeni yapıya migrate (davranış korundu); order-shortage + overdue-ship inline-form regression'ları → **Faz 2 `it.todo`** (endpoint testleri [shortages/ship/production-prefill] TAM kapsamda = backend sözleşmesi korunur) + render smoke + preservation. tsc 0 · lint 0 · build 0 (`ƒ /api/alerts/calendar` + `ƒ Proxy` + `○ /dashboard/alerts`) · vitest tam yeşil (+44 net yeni test, 5 it.todo; mupdf yerel kuruldu `^1.27.0`→pdf-render 11/11). **Sıradaki:** Faz 2 (drawer zenginliği) → Faz 3 (cila/responsive/a11y/animasyon).

</details>

<details><summary>Önceki: Login "Monolith" (F) redesign — TR/EN + tema + Google OAuth + şifre sıfırla</summary>

**Son tamamlanan iş:** **Login "Monolith" (F) redesign — TR/EN + tema + Google OAuth + şifre sıfırla** (2026-06-08; ÇALIŞILDI/green, COMMIT+PUSH onayı bekliyor). `design_handoff_login_monolith/` tasarım paketi uygulandı: editöryel full-bleed düzen (sol marka kilidi + bare form, sağ ekrandan-taşan hexagon grafiği, sağ üst TR/EN + tema). İşlevsel davranış AYNEN korundu (`signInWithPassword`→`clearDemoMode()`→`/dashboard`). **Yeni öğeler:** Beni hatırla (KOZMETİK — session-persistence bağlanmadı), Şifremi unuttum (`resetPasswordForEmail`, yalnız ADIM 1 = e-posta gönderir; yeni-şifre ekranı ayrı tur), Google (`signInWithOAuth`), TR/EN yerel sözlük (`STR` + login-only `useState`; global i18n YOK, tercih persist edilmez). **Kullanıcı kararları:** dil=yerel sözlük; Google=Supabase OAuth (**Clerk REDDEDİLDİ** — uygulama tümüyle Supabase Auth: RLS + `proxy.ts` `sb-*` cookie + RBAC `app_metadata.roles` + demo cookie; ikinci stack altyapı migrasyonu olurdu); reset=resetPasswordForEmail. **ADVISOR kritik yakaladı (prod-kıran):** `@supabase/ssr` PKCE → OAuth dönüşündeki `?code=` **server-side exchange ŞART**; `redirectTo:/dashboard` doğrudan kullanılsaydı `proxy.ts` session'sız görüp /login'e geri atar → buton sessizce çalışmaz (provider etkinleştirmek ÇÖZMEZ) → yeni `GET /auth/callback` route `exchangeCodeForSession` yapar + relative `Location` (Coolify/Traefik reverse-proxy host pass-through fix, `parasut/oauth/callback` precedent'i) + `ALWAYS_PUBLIC`'e eklendi. **Yapılan dosyalar:** `globals.css` eski `.login-shell` kart bloğu → monolith CSS (token'lar zaten var, `.spinner`+`@keyframes spin` global korundu, primary/secondary için `Button` reuse = `.btn*` CSS gerekmez); `login/page.tsx` tam yeniden yazım (`ThemeProvider`+`useTheme` ile login'e tema bağlandı — login dashboard dışında, context yoktu; `data-theme` `<html>`'de kalır); `auth/callback/route.ts` (YENİ); `src/proxy.ts` ALWAYS_PUBLIC `/auth/callback`; `eslint.config.mjs` `design_handoff_*` ignore (referans prototip = production değil). Google "G" SVG tema-muaf (marka renkleri). **+9 net test** (login-page 6→11, auth-callback +3): **4812→4821**. tsc 0 · lint 0 · build 0 (`/auth/callback` + `ƒ Proxy`).

**+ Davetiye-bazlı erişim kilidi (kullanıcı: "kayıtlı olmayan giremesin bile"):** Google OAuth açılınca **herhangi bir Google hesabı** sisteme girebiliyordu (Supabase default signup açık → yeni kullanıcı = `viewer`; finansal redakte + mutasyon 403 ama müşteri/ürün/sipariş listeleri görünür = ticari sızıntı). İki katmanlı kilit: **(1) BİRİNCİL (load-bearing) — Supabase "Allow new users to sign up" OFF** (self-signup oturum bile yaratamaz; KULLANICI yapar). **(2) İKİNCİL kod (defense-in-depth) — `isProvisionedUser` guard.** Temiz sinyal (koddan doğrulandı): admin createUser HER ZAMAN `app_metadata.roles` set eder (panel→en az `["viewer"]`, create-admin→`["admin"]`); self-signup'ta `app_metadata.roles` HİÇ yok. **Yeni pure helper `isProvisionedUser(appMeta, email, adminEmails)`** (permissions.ts) — roles dolu VEYA legacy `role` VEYA email∈ADMIN_EMAILS → true; aksi false (`parseRoles`'un `["viewer"]` fallback'ından farklı = ayrım yapar). **`proxy.ts`** auth'lu kullanıcı doğrulandıktan sonra, /dashboard yönlendirmesinden ÖNCE: provize değilse `/api/**`→403, sayfalar→`/login?error=unauthorized` (`/login` döngüsüz geçer). Login `?error=unauthorized`→"Hesabınız yetkili değil" mesajı (TR/EN). **ADVISOR yakaladı (yıkıcı boşluk):** `/api/seed` ALWAYS_PUBLIC'te → middleware guard'ı BYPASS eder + `checkAuth` `!!user` (herhangi oturum) kabul ediyordu → self-signup oturumu **tüm DB'yi silebilirdi** → seed session-branch **admin-only** sıkılaştırıldı (`parseRoles(...).includes("admin")`; CRON_SECRET yolu korunur). **+13 test** (isProvisionedUser 7, proxy unprovisioned/bootstrap 4, login unauthorized 1, + 4 cascade fixture provize-edildi). **4821→4834 test** · tsc 0 · lint 0 · build 0.

**DURUM: ÇALIŞILDI/green, COMMIT+PUSH onayı bekliyor.**

⚠️ **DEPLOY ÖN KOŞULLARI (kullanıcı doğrulamalı — yeşil testler bunları kapsamaz):**
1. **Supabase "Allow new users to sign up" = OFF** (Authentication → Sign In/Providers veya User Signups). Bu BİRİNCİL kilit — kapatılmazsa `/api/seed` gibi ALWAYS_PUBLIC endpoint'lerde self-signup oturumu hâlâ risk. Kullanıcı transcript'te redirect_uri + provider'ı düzeltti ama signup toggle TEYİT EDİLMEDİ.
2. **⚠️ BRICK RİSKİ — prod admin hesabı `app_metadata.roles` taşımalı VEYA `ADMIN_EMAILS` her iki Coolify env'inde set olmalı.** Guard `/api/admin/users` zero-admin bootstrap'ından ÖNCE çalışır → admin'in rolü yoksa + ADMIN_EMAILS yoksa **herkes kilitlenir** (tek kurtarma: Supabase dashboard'dan app_metadata set). create-admin ile açılan admin `["admin"]` taşır (güvenli); Supabase dashboard'dan açıldıysa kontrol et.
3. **Canlı Google OAuth turu DOĞRULANMADI** (testler mock'lu): Supabase Google provider etkin + redirect-URL allowlist'e `…/auth/callback` + tarayıcı smoke. (redirect_uri_mismatch çözüldü = Google Cloud'a Supabase callback URI eklendi.)

Plan: `~/.claude/plans/design-handoff-login-monolith-dosyas-n-inherited-bunny.md`.

</details>

<details><summary>Önceki: Null-SKU kapatma — İncele ekranında yeni-ürün SKU girişi (`c7db606`/`7e9654b`)</summary>

**Son tamamlanan iş:** **Null-SKU kapatma — İncele ekranında yeni-ürün SKU girişi** (ÇALIŞILDI/green, PUSH onayı bekliyor). Faz D-POC'un dürüst bıraktığı tek sınır kapatıldı. Kullanıcı "sku sorunu ne" sordu → datasheet'ler SKU vermez, `buildCreateProductInput` "SKU eksik" fırlatır → yeni-ürün açma yolu SKU'suz belgede tıkanıyordu (eşleşen+görsel + Excel-şablon zaten çalışıyordu). Kullanıcı **seçenek 2** seçti: İncele ekranında SKU gir. **MİGRASYON YOK** — `import_document_lines.extracted_sku` kolonu zaten var, apply doğrudan okur. **Yapılan (+12 test):** `UpdateLineMatchInput`+`extracted_sku?:string|null` → `dbUpdateLineMatch` koşullu patch (undefined=atla/null/""=clear/string=set); PATCH `/api/import/document-lines/[id]` `body.extracted_sku` parse+validate (trim, >100→400, non-string→400, boş→null); `ExtractionReview` new_product satırında düzenlenebilir SKU input (boşken kırmızı bordür + "datasheet vermedi, SKU girin" uyarısı), `handleSkuChange` **match_action korur** (satır new_product'tan düşmez), no-op'ta PATCH atmaz, diğer satırlar SKU salt-metin. **ADVISOR (yazımdan önce doğrulandı):** (1) SKU TEK blocker — `dbCreateProduct` `category ?? null`, sadece name/sku/unit zorunlu, unit→"adet"; (2) new_product branch (apply satır 402) SKU'yu approval checkbox'ına bağlamadan okur → PATCH'lenen SKU sessizce düşmez; (3) **asıl kanıt testi**: apply-service'de önce-null SKU şimdi-set → ürün oluşur + boş SKU → "SKU eksik" + skipped. **SKU çakışması dostça mesaj (aynı işin polish'i, +4 test):** `friendlyApplyRowError(raw, sku)` pure helper export — apply per-row catch'inde `/unique|duplicate key/i` ihlali → "Bu SKU zaten kullanımda: <sku> — farklı bir SKU girin veya eşleşen ürünü seçin." (ham Postgres `duplicate key...` sızmaz; diğer hatalar passthrough). Kök `dbCreateProduct` DOKUNULMADI — POST `/api/products` zaten `msg.includes("unique")` → 409 "Bu SKU zaten kayıtlı." yapıyor; kökü değiştirmek o tespiti kırardı → downstream map tercih edildi (create + SKU-değiştiren matched-update ikisini de kapsar). tsc 0 · lint 0 · **4812 test** (+12 SKU girişi +4 dostça mesaj) · build 0. **DURUM: SKU girişi PUSH EDİLDİ (`c7db606`); dostça mesaj polish PUSH bekliyor (mirror, migrasyon yok).**

</details>

<details><summary>Önceki: Faz D-POC — tam-otomatik katalog→ürün görseli (mupdf WASM) (`4253696`)</summary>

**Faz D-POC — tam-otomatik katalog→ürün görseli (mupdf WASM)** (`4253696`). Import epic'inin son halkası: vizyonun "görselini bile çıkarıp yazsın" kısmı. Kullanıcı "tam POC planı + uygula" + görsel kapsamı **Hibrit** (AI bbox güvenliyse kırp, değilse tam sayfa). **Yapılan (+39 test):** `pdf-render.ts` `renderPdfPageToPng` lazy `import("mupdf")` (mupdf@1.27.0 Artifex WASM, native-dep yok) + `pickRenderClip` saf hibrit helper (eşik 0.6); `ai-service.ts` `ExtractedProductLine`+`source_page`/`image_region` (yalnız PDF prompt) + `parseImageRegion`; **migration 086** `source_page int`+`image_region jsonb`; lazy `GET .../lines/[lineId]/preview-image` (storage'a YAZMAZ); `import-apply-service` `attachCatalogImageIfPresent` (render→`dbCreateAttachment(image)`→`ensurePrimaryImageIfMissing`, **NON-FATAL**, `images_extracted` sayacı); `ExtractionReview` 📷 önizleme + apply özeti. **Build:** `serverExternalPackages:["mupdf"]`+`outputFileTracingIncludes` → WASM standalone'a doğru konuma trace (bundle'a alınırsa `import.meta.url` chunk'ı gösterir, wasm bulunamaz; external stub node_modules'tan yükler). **2 canlı re-probe (gerçek PMT datasheet) 2 bug ortaya çıkardı** (mock'lu testler göremezdi): (1) **truncation** `max_tokens` 4096/8192→**16384** (per-DN 14-17 meşru ürün); (2) **granülarite** ilk "ölçü tablosu=topla" denemem canlı `products` ile çelişti (PMT **DN/basınç/malzeme per-ürün**: `KV-3P-DN50`≠`DN80`) → ADVISOR+kullanıcı kararı PMT modeline hizala. Re-probe: **17 temiz ürün** `end_turn`, zengin attributes, Vana tipi, bbox hibrit + kırpma gözle doğrulandı (gerçek valf fotoğrafı). **POC hipotezi (Claude kullanılabilir sayfa+bbox veriyor mu?) = EVET.** **Bilinen sınır (ayrı tur):** datasheet'ler `sku=null` → yeni-ürün açma yolu SKU gerektirir (eşleşen+görsel ekleme + Excel-şablon çalışır); source_page tek-sayfada kanıtlı; alpine WASM smoke Coolify'da. tsc 0 · lint 0 · **4796 test** · build 0. React Doctor advisory bloklamadı. **DURUM: PUSH EDİLDİ (`4253696`) — birebir aynı SHA; iki Coolify deploy. Migration 086 KULLANICI UYGULADI ✅.**

</details>

<details><summary>Önceki: Veri Aktarım yazma yolları — Faz A·B·C·D epic (`bdcdbee`)</summary>

**Veri Aktarım yazma yolları geliştirme — Faz A·B·C·D epic** (`bdcdbee`). Kullanıcı sorusu "gelen bilgiler ürün kartlarına hatasız/eksiksiz yazılıyor mu?" → read-only denetim (iki yazma yolu SAĞLAM ama eksik) → "yazma yollarını geliştirmek istiyorum" + vizyon "katalog yüklendiğinde AI ürünün her bilgisini + görselini bile çıkarıp ürün kartına eksiksiz yazsın". **Kullanıcı yönetimi:** kalıcı master MD (`IMPORT_WRITE_UPGRADE_PLAN.md`) + faz faz uygula + "bütün fazlar bitsin sonra push" (tek push) + "bana sorman gereken noktaları sor kendin ilerle" (bkz `feedback_ask_scope_decisions`). **Yapılan (~24 dosya, +46 test):** **Faz A** zengin master-data extraction+apply — `import-center.ts` `IMPORT_CORE_PRODUCT_FIELDS` (16-alan whitelist, finansal/stok HARİÇ) + `normalizeCoreProductFields`; `ai-service.ts` `ExtractedProductLine.core_fields`+prompt JSON şema; **migration 085** `import_document_lines.extracted_core_fields jsonb`; `import-apply-service.ts` yeni ürün tüm core yazar / eşleşen **fill-empty** (unit/currency bozulmaz, ADVISOR); `ExtractionReview.tsx` gösterir. **Faz B** Excel teknik attributes — `/api/import/templates?kind=product_type&typeId=` tip-özel şablon + `collectTypeAttributesFromRow`/`buildProductTypeTemplateColumns`; **parse-survival (ADVISOR blocker)** `apply-mappings` teknik kolon+`urun_tipi` passthrough; `import-service` confirm tip eşleme→attributes (fill-empty). **Faz C** akıllı güncelleme — `pickUpdate` fill-empty default + **overwrite toggle** (tüm yollar; confirm `body.overwrite` + import sayfası checkbox); `user_corrections` fill-empty'de bile uygulanır; `dbUpdateProduct` `PRODUCT_UPDATE_ALLOWED_FIELDS` allow-list (ADVISOR); AI-apply attributes da fill-empty (curated ezilmez, ADVISOR). **Faz D** katalogdan görsel — pragmatik tur (mevcut görsel→kapak akışı + rehber metni); tam-otomatik POC sonraki turda (`4253696`). **Bilinen asimetri (kabul):** AI-apply'da overwrite toggle yok=her zaman fill-empty güvenli; Excel'de toggle var. Canlı yazma smoke (ZZTEST+hard-delete) A/B/C kanıtladı. tsc 0 · lint 0 · **4757 test** · build 0. **DURUM: PUSH EDİLDİ (`bdcdbee`+doc `0d9ac15`) — birebir aynı SHA; iki Coolify deploy. Migration 085 KULLANICI UYGULADI ✅.**

</details>

<details><summary>Önceki: Veri Aktarım Merkezi — rehber + şeffaflık katmanı</summary>

**Veri Aktarım Merkezi — rehber + şeffaflık katmanı** (`fd4499b`). Kullanıcı: "ne nereye nasıl kaydedilir, kullanıcı ne yapacağını bilsin — değerli olsun" (ultrathink). `/dashboard/import` işlevseldi (Faz 3a-3d AI + klasik Excel wizard) ama rehberlik zayıftı. **Kullanıcı kararları:** rehber+şeffaflık katmanı (mevcut akış KORUNUR) + üstte görünür özet + collapsible detay. **Yapılan:** Yeni `src/lib/import-guide.ts` (tek kaynak): `IMPORT_STEPS` (3 adım) + `IMPORT_DATA_TARGETS` (scope→modül/ne olur) + `IMPORT_TRUST_NOTES` + `getActiveTemplateLinks()` (6 Excel şablon → `/api/import/templates`); içerik `ai-import-operations`+`import-center`'dan TÜRETİLDİ. Yeni `src/components/import/ImportGuide.tsx`: 3-adım şeridi + seçili-işlem "veri nereye gider" özeti + collapsible (chevron'lu) hedef haritası + şablon indirme + güven notları (tema-uyumlu). `page.tsx` header altına mount; AI/Excel akışı dokunulmadı. **+24 test.** **ADVISOR fix:** dosya-saklama notu dürüstleştirildi (klasik Excel **tarayıcıda** parse=storage yok, AI akışı `product-files`'a yükler); chevron eklendi; finansal koruma (`canApplyFinancialField`) ikisinde de doğrulandı. tsc 0 · lint 0 · 4711 test · build 0. **DURUM: PUSH EDİLDİ (`fd4499b`) — birebir aynı SHA; iki Coolify deploy.**

</details>

<details><summary>Önceki: Roven hexagon logo — component + topbar/login/favicon ince ayarları</summary>

**Roven hexagon logo — component + topbar/login/favicon ince ayarları** (`f3cb649`→`1b760d6`→`079df51`→`7861594`→`2fd2ef6`→`e8f44c4`). Kullanıcı görsel paylaştı: yuvarlatılmış hexagon + "Roven" wordmark. `/frontend-design`: minimal geometrik, monokrom, tema-uyumlu. **Yapılan:** Yeni `src/components/layout/RovenLogo.tsx` (tek kaynak) — hexagon `<polygon>` SVG + bold wordmark; `currentColor`/inherit → otomatik temalanır; Props `size/showWordmark/wordmarkSize/gap`. **`DEFAULT_GAP=5` global** (kullanıcı "global de değiştir" → topbar'a hardcode etmek yerine component varsayılanı; bkz `feedback_global_over_hardcode`). Topbar `size=22 wordmarkSize=19` (gap global). Landing + login (`size=32 wordmarkSize=26 gap=6` explicit → kullanıcı isteğiyle büyütülüp yaklaştırıldı, global'den izole) da RovenLogo. **`src/app/icon.svg` favicon: kutusuz + tema-uyumlu** (koyu kutu kaldırıldı; SVG-içi `prefers-color-scheme` → koyu sekme `#e6edf3`/açık `#1f2937`; favicon agresif cache → hard-refresh). **+10 test** (`roven-logo.test.tsx`). tsc 0 · lint 0 · 4688 test · build 0. **DURUM: PUSH EDİLDİ (`e8f44c4`) — birebir aynı SHA; her tur iki Coolify deploy.** ⚠️ Push dersi: `git merge --ff-only` mutlaka `cd erp2` içinde — bir turda atlandı, kullanıcının **login redesign** commit'i (`fee0943`) + topbar birlikte main'e ff edildi (mirror geri sağlandı; login redesign artık main prod'da).

</details>

<details><summary>Önceki: Marka rename (KokpitERP → Roven) + main hizalama</summary>

**Marka rename — KokpitERP → Roven** (kullanıcı: "SİSTEMİN ADI BUNDAN SONRA ROVEN"). Wordmark **title-case "Roven"** (caps yazımı vurgu sayıldı; "ROVEN" istenirse tek komut). **Rename edilen** (ürün adı geçen yerler): UI wordmark (`Topbar`/landing `page.tsx`/`login`), metadata + `document.title`'lar (`layout.tsx` "Roven — AI Destekli ERP", orders/purchase-orders), `topbar-title.ts` fallback, Paraşüt fatura `description` (`parasut.ts`+`parasut-service.ts` — hiçbir yerde geri-parse edilmiyor, güvenli), e-posta header (`email/templates.ts`), README + canlı ajan dokümanları (`CLAUDE.md`/`AGENTS.md`/`domain-rules.md`/`ROADMAP.md`/`frontend-renewal.md`/`docs/EMAIL_DEPLOY.md`), tüm `memory/*.md` frontmatter `name:` alanları. **DEĞİŞMEDİ (kasıtlı):** iç altyapı kimliği (repo `mirza-dev/erp2`, paket adı, `~/.claude/skills/erp2-*`, yollar — ürün adı değil); immutable/arşiv (`supabase/migrations/*.sql` header yorumları=uygulanmış tarih, `PARASUT_PLAN.md`+`docs/plans/*`=tarihsel snapshot). Brand-assert eden testler güncellendi (`topbar` `getByText("Roven")`, `orders-page-title`, `parasut-mock-adapter`, `email-service` env). Yöntem: zsh-safe `grep -rIl | xargs perl -pi`. tsc 0 · lint 0 · 4678 test · build 0 (`ƒ Proxy`). **Branch ayrışması → ÇÖZÜLDÜ:** rename önce codex-experiment'e push (`d73e25a`); codex ajanının premium-UI/design-token refactor'u (`4a0161c`, ~37 dosya) yalnız codex-experiment'teydi, main (`995d0b4`) gerideydi. Kullanıcı "maini de hizala" dedi → `main` ff-only `995d0b4`→`d73e25a` (codex refactor + rename main prod'a gitti) + iki branch push → **mirror geri sağlandı** (iki Coolify prod deploy). Bypass uyarısı (test status check) yetkiyle geçti.

</details>

<details><summary>Önceki: Tema sistemi — Koyu + Aydınlık (Cool slate)</summary>

**Tema sistemi — Koyu + Aydınlık (Cool slate), premium, sıfır frontend bozulması** (`globals.css` + `layout.tsx` + yeni `use-theme.tsx`/`ThemeToggle.tsx` + `dashboard/layout.tsx` + `Topbar.tsx` + remediasyon). Sistem yalnız koyu temaydı; kullanıcı koyu+aydınlık istedi. **Kullanıcı kararları (AskUserQuestion):** (1) ilk açılış = **sistem tercihini izle** (`prefers-color-scheme`), elle seçim hatırlanır; (2) geçiş = **Topbar'da küçük ikon** (güneş/ay, avatar öncesi); (3) kayıt = **yalnız localStorage** (backend yok, FOUC-suz); (4) aydınlık palet = **Cool slate** (`#f6f8fa` zemin / `#1f2328` metin / `#0969da` accent — mevcut koyu paletle uyumlu). **Mimari:** `globals.css` palet `:root`'tan `:root[data-theme="dark|light"]` bloklarına ayrıldı — **koyu palet birebir korundu** (mevcut kullanıcıda sıfır değişiklik) + Cool slate aydınlık blok. Yeni tema-bilir tokenlar: `--highlight-inset`, `--accent-bg-strong`, `--success/danger-bg-strong`, `--accent-glow`; her temada `color-scheme` + `prefers-reduced-motion` guard (geçiş **anında swap** — yarım-fade içerik/zemin katmanını ayrıştırır=ucuz görünür, advisor kararı). `layout.tsx` `<head>` **FOUC-suz bootstrap script** (boyamadan ÖNCE `data-theme` set: localStorage→OS→koyu fallback; `suppressHydrationWarning` zaten vardı). Yeni `src/lib/theme/use-theme.tsx` `ThemeProvider`/`useTheme` (`theme: system|dark|light` + türetilmiş `resolved`; ilk `resolved` **DOM'dan okunur** = re-flash yok; `system` iken `matchMedia` canlı izlenir). `dashboard/layout.tsx`'te en dış provider (zaten `"use client"` + Topbar orada — doğrulandı). Yeni `ThemeToggle.tsx` güneş/ay ikonu: **kısa tık** koyu↔aydınlık, **uzun bas** → `system`'e döner (Q1↔Q2 gerilimi geri-dönüş kancası, advisor); Topbar'da avatar öncesi. **Sıfır bozulma remediasyonu:** ~153 sabit renk kategorize edildi — tema-ilgili tintler tokenize (`Button` secondary/toolbar→`--highlight-inset`, danger/success hover→`-bg-strong`; `UserAvatarLink` inset; customers/dashboard count-badge→`--accent-bg-strong`; import dragOver + Sidebar active→`--accent-bg`; parasut status ring→`-bg-strong`); kasıtlı sabit yüzeyler **tema-muaf** yorumuyla korundu (QuoteDocument/PurchaseOrderDocument baskı belgeleri = beyaz kağıt, settings logo kutusu, products lightbox). **ADVISOR done-check (kritik):** root bootstrap landing/login/error'ı da global temalar → tüm `#fff` metinleri statik denetlendi: hepsi `var(--accent)` (doygun mavi, beyaz-üstü kontrast iki temada da güçlü) veya `rgba(0,0,0,0.6)` dark scrim üzerinde → **`var(--bg-*)` üzerinde görünmez-beyaz YOK** = aydınlıkta first-screen kırılması yok. **+18 test** (`theme-system.test.ts`: bootstrap, palet blokları, tokenlar, useTheme mantığı, ThemeToggle+Topbar, baskı-belgeleri-tema-muaf regression) + 3 mevcut test güncellendi (topbar/exchange ThemeToggle context stub, button toolbar-hover→`--highlight-inset`). Backend/migration/permission YOK (localStorage). tsc 0 · lint 0 · 4631→4650 test · build 0 (`ƒ Proxy`). **DURUM: PUSH EDİLDİ — `origin/main == origin/codex-experiment` birebir aynı SHA; iki Coolify prod deploy tetiklendi.** Görsel kabul (kontrast/premium hissi + teklif/PO önizleme aydınlıkta beyaz kağıt) kullanıcı gözüyle (deploy sonrası).

</details>

<details><summary>Önceki: Topbar "Sakin düz" yeniden tasarım + uyarı butonunu kaldırma (`bf28fb0`)</summary>

**Topbar yeniden tasarım — "Sakin düz" + uyarı butonunu kaldırma** (`Topbar.tsx` + `ExchangeRatesTicker.tsx` + `SystemHealthIndicator.tsx` + `globals.css`). Kullanıcı şikâyeti: topbar "çip/kalitesiz" hissi veriyor (ekran görüntüsü). **Kök neden:** çip çorbası (her öğe ayrı bordürlü pill), `ExchangeRatesTicker` kutu-içinde-kutu-içinde-kutu (ticker → rates grup → per-kur dolu pill), yeşil "Bağlı" + kırmızı "19 Uyarı" trafik-ışığı çakışması. **Kullanıcı kararları (AskUserQuestion):** (1) genel yön = **"Sakin düz"** (çip-içinde-çip kalkar, kurlar düz metin, sağlık küçük renkli nokta, sağ taraf tek hafif küme, sayfa başlığı markanın yanına/sol), (2) marka = **"Sadece yazı kalsın"** (logo/ikon yok), (3) **uyarı butonu kaldırılır** (doğrudan istek — **kayıp yok**, Sidebar'da "Uyarılar" sayacı `activeAlertCount` ile `/dashboard/alerts`'e bağlı kalır). **Uygulama:** Topbar uyarı `<Link>` bloğu + ölü `AlertTriangle`/`useData`/`activeAlertCount` silindi; başlık ortadaki pill'den çıkıp markanın yanına (`.topbar-divider`), pill→sessiz metin; `.topbar-shell` grid→flex; sağ küme `.topbar-right-extras` tek sakin yüzeyde. `ExchangeRatesTicker` düzleştirildi (çip-içinde-çip + TCMB/LIVE rozeti + "A/S" kaldırıldı, kaynak tooltip+aria-label'da). **ADVISOR FIX'leri:** mobil küme gizleme uyumlu, kaynak rozeti onaylı görünüme sadık eklenmedi, ayraç ticker `borderRight`'ında (null'da sarkmaz). `SystemHealthIndicator` pill→nokta+etiket. tsc 0 · lint 0 · 4627→4631 test · build 0. **DURUM: PUSH EDİLDİ (`bf28fb0`) — iki branch birebir aynı, iki Coolify prod deploy.**

</details>

<details><summary>Önceki: Dashboard AI Özeti + Aktif Uyarılar collapsible (`44d4e54`)</summary>

**Dashboard — "AI Operasyon Özeti" + "Aktif Uyarılar" tıklanınca açılır (collapsible)** (`/dashboard`). Kullanıcı isteği: bu iki bölüm varsayılan **gizli**, yalnız **tıklanınca görünsün**. **Kullanıcı kararları (AskUserQuestion):** (1) kapalı görünüm = **başlık + özet rozet** (kapalıyken bile `[AI]` / "N açık" rozeti görünür — göz ucu değeri), (2) **her zaman kapalı başla** (localStorage yok). **Uygulama:** `page.tsx` içine yerel `CollapsibleSection` helper (tam-genişlik `<button aria-expanded>` başlık çubuğu + rotate chevron + `badge` slot; `open && children` → **lazy mount**); 2 state `showAiSummary`/`showAlerts` (ikisi `useState(false)`); `useData()` → `openAlerts`/`loading` (rozet sayısı için). `AISummaryCard` ve `AIAlerts` `CollapsibleSection` ile sarıldı. **AISummaryCard lazy mount sayesinde `POST /api/ai/ops-summary` AI çağrısı artık her dashboard yüklemesinde değil, yalnız ilk açılışta yapılır** (sessionStorage 15dk cache tekrarları engeller). **ADVISOR FIX (çift başlık — kritik):** iki bileşen kendi başlık+rozetini de render ediyordu → açıkken başlık 2 kez görünürdü; tsc/lint/build/source-regex YAKALAYAMAZDI. İç başlıklar kaldırıldı (wrapper tek başlık kaynağı): `AIAlerts`'ten loading+loaded başlık satırları + "N açık" rozeti silindi (footer "Tümünü Gör → (N açık uyarı)" KORUNDU); `AISummaryCard`'ın 3 state başlığı (disabled/error/loaded) silindi ama loaded'daki **"Yenile" butonu KORUNDU** (`feedback_no_silent_deletes`). İki bileşen yalnız dashboard'da kullanılıyor (grep doğrulandı) → güvenli. **+11 test** (`dashboard-collapsible-sections.test.ts`). Backend/migration/permission DEĞİŞMEDİ. tsc 0 · lint 0 · 4616→4627 test · build 0 (`ƒ Proxy`). **DURUM: PUSH EDİLDİ (`44d4e54`) — iki branch birebir aynı, iki Coolify prod deploy.**

</details>

<details><summary>Önceki: Teklif formu ürün açılır-listesi kırpılma fix'i (`QuoteForm.tsx`)</summary>

**Teklif formu ürün açılır-listesi kırpılma fix'i** (`QuoteForm.tsx`). Kullanıcı ekran görüntüsü gönderdi (macOS TCC ilk denemede engelledi, harness inline gösterdi): teklif satırında ürün kodu girerken açılan autocomplete listesi üstten/alttan **kırpık**, kapsayıcı içinde sıkışmış + dikey scrollbar. **Kök neden (kod-deterministik, görüntüyle doğrulandı):** lines tablosu `<div style={{ overflowX: "auto" }}>` ile sarılı (`:1118`, Faz 5 `f30b0bd` — eski sorun, görsel QA ile ilgisiz); ürün dropdown'u `<td>` içinde `position:absolute; top:100%` ile satır altına taşıyor; CSS gereği bir eksende `overflow:auto` → diğer eksen `visible` kalamaz → dropdown dikey kırpılıyordu. Müşteri/firma dropdown'u (`:996`, tablo DIŞINDA) etkilenmiyordu — "ürün girerken" şikayetiyle birebir tutarlı. **Fix:** `overflowX: prodOpenRowId !== null ? "visible" : "auto"` — dropdown açıkken kapsayıcı taşmayı serbest bırakır (liste tam görünür), kapalıyken yatay scroll (geniş tablo/mobil) korunur. **+1 source-regression testi** (`quotes-ui-audit-fix.test.ts`: koşullu overflow var + koşulsuz `overflowX:"auto"` yok). tsc 0 · lint 0 · 4615→4616 test · build 0. **DURUM: PUSH EDİLDİ — `origin/main == origin/codex-experiment` birebir aynı SHA; iki Coolify prod deploy tetiklendi.**

</details>

<details><summary>Önceki: Görsel QA (codex) + iki-branch hizalama + PUSH (`1ef3c8e`)</summary>

**Görsel QA (codex) + iki-branch hizalama + PUSH** — codex görsel QA değişiklikleri commit'lendi ve önceki import-fix turuyla birlikte **her iki branch birebir aynı SHA'ya getirilip push edildi.** **Bağlam:** iki ayrı, çakışmayan değişiklik seti vardı — (a) main'de `5265a08` import-fix (CSV injection + AI apply null-clobber + field_key notu, önceki tur), (b) codex worktree'sinde görsel QA (Button forwardRef + mobil responsive + ham `<button>`→`Button`/`ButtonLink` + emoji→lucide ikon, 23 dosya). **Çakışma analizi:** iki setin dosya kesişimi BOŞ (`product-types.ts` helper ≠ `settings/product-types/page.tsx`) → manuel kod değişikliği gerekmedi. **Hizalama yöntemi:** codex görsel QA commit (`4ea770d`) → main'e cherry-pick (çakışmasız) → codex `git reset --hard main` ile özdeşledi. **Görsel QA içeriği + 2-tur Claude review:** Button `forwardRef<HTMLButtonElement>` (lightbox/modal ref — products/[id] focus); ürün detay mobil header scoped `@media(max-width:640px)`; sipariş formu mobil tablo `overflowX:auto`+`minWidth:720px` + desktop summary `{!isMobile}` gizli → sticky alt aksiyon tek kaynak; tüm dashboard ham button→Button (iconOnly'ler `aria-label` korundu, readOnly/demo/RBAC/durum-geçiş guard'ları değişmedi); ButtonLink disabled a11y (`aria-disabled`+`tabIndex=-1`+preventDefault). **Review bulguları (hepsi düzeltildi):** B1 iconOnly leftIcon `aria-hidden` span'e sarıldı; B2 CustomerDetailPanel İptal→Kaydet sırası (kasıtlı, OrderForm tutarlılığı — teyit alındı); B3 ölü `.q-add-btn/.q-btn/.q-btn-primary :hover` CSS kuralları silindi (ghost variant tek hover); B4 `order-form-interaction.test.tsx` **davranışsal mobil testi** (`innerWidth=390` → summary gizli/sticky görünür, source-regex değil gerçek render). **Birleşik suite regresyonu yakalandı + düzeltildi:** `quotes-ui-audit-fix.test.ts` "CSS variable kullanılıyor" testi preview'daki `btnPrimary`/`btnSecondary` inline stillerini (`var(--accent)` + `0.5px solid var(--border-secondary)`) arıyordu — bu stiller Button'a delege edilince obsolet kaldı; 2 obsolet assertion kaldırıldı (niyet korundu: hex yok + kalan 5 CSS-var assertion + Button geçişi `button-source-regression`'da kilitli). **Backend/migration/permission DEĞİŞMEDİ.** **Doğrulama (birleşik main):** tsc 0 · lint 0 · **4615 test yeşil** (320 dosya) · build 0 (`ƒ Proxy`). **DURUM: PUSH EDİLDİ — `origin/main == origin/codex-experiment == <HEAD>` birebir aynı (tree+SHA özdeş); iki Coolify prod deploy tetiklendi. codex worktree (`proje-codex`) main HEAD'e reset edildi.** main geçmişi: `39c0d07 → 5265a08(import) → görsel-QA → context`.

<details><summary>Önceki: Branch hizalama audit'i — son commitlerin detaylı kod incelemesi + 3 bulgu (`5265a08`)</summary>

**Branch hizalama audit'i — son commitlerin (codex↔main, `39c0d07`) detaylı kod incelemesi + 3 bulgu düzeltme** (commit `f24afb1`→`5265a08`). Kullanıcı "iki branch aynı mı / son commitlerin kodunu eksiksiz incele, bug/güvenlik/semantik tutarsızlık kalmasın" dedi. **Branch durumu:** `main == codex-experiment == 39c0d07` birebir aynı (tree+SHA özdeş, 0/0 ahead-behind) — `4176c8d` üzerine bir docs commit'i. İki worktree branch'i (`worktree-rbac-bulgular-kapatma`, `worktree-satis-siparisleri-tamamlama`) main'in atası = tamamen merge edilmiş, benzersiz iş yok. **Audit kapsamı:** codex feature seti (16 commit, `ee3b701`) + merge çözümü (`56ecbd1`); main'in 13 final-ürün commit'i önceki turlarda probe'lanmıştı → delta = codex kodu (129 dosya/~12.5k satır) + el-çözümü. **Derin incelenen** (satır-satır): 3 yeni API route (exchange-rates/import-templates/import-report), migration 083/084, exchange-rates parser, ai-import-operations, import-center, product-types field_key zinciri, import-apply-service, import-service finansal enforcement, settings merge çözümü. **3 bulgu (1 ORTA güvenlik + 2 DÜŞÜK):** **#1 [ORTA] CSV formül injection** — `report/route.ts escapeCsv` yalnız `[",\n\r]` tırnaklıyordu; `= + - @ TAB CR` ile başlayan hücreleri nötrlemiyordu. Rapor verisi yüklenen dosyadan gelen kullanıcı-kontrollü serbest metin (`row_errors`/`risk_flags` asArrayText tek-eleman, ham alanlar) → Excel formül çalıştırır. Tırnaklama ENGELLEMEZ → değer-seviyesi `'` prefix nötrleme (`neutralizeCsvFormula`). XLSX yolu zaten güvenli. **#2 [DÜŞÜK] AI apply attribute null/boş clobber** — `import-apply-service.ts` matched-update `{...current, ...new}` merge null/""/undefined AI değerini mevcut ürün attribute'una yazıp siliyordu (safetyNote yalnız prompt talimatıydı). Merge öncesi null/boş filtrele (`nonEmptyLineAttributes`); skip + `technical_fields_applied` sayacı + no-op update da nonEmpty üzerinden hizalandı. **#3 [DÜŞÜK] field_key migrasyonu transactional değil** — `dbUpdateProductTypeField` N ürün UPDATE + field UPDATE tek transaction'da değil; UI READ-ONLY + nadir admin → SAĞLAMLIK NOTU dokümante (sıklaşırsa atomik RPC). **ÇÜRÜTÜLEN (önce-doğrula):** field_key orphan (backend collision-guard'lı migrate ediyor — UI+backend iki katman), import finansal sızıntı (apply'da price/cost_price yazma yolu YOK; klasik yolda `canApplyFinancialField` server-side permission-gate — RBAC redaction'la tutarlı), product_type_id NULL temiz, 3 route auth-gated, migration 083/084 idempotent+RLS, settings merge (a11y gruplu nav + errBody parite + `:1056 &apos;` JSX text = doğru decode). **Test+build'e güvenilen** (regresyon riski düşük, derin okunmadı): büyük UI rewrite'ları (Button/Topbar/Sidebar/ExchangeRatesTicker/SystemHealthIndicator/ExtractionReview) + 9 oto-merge. **+4 test** (CSV injection 2, attribute null-clobber 2). **tsc 0 · lint 0 · 4604→4608 test · build 0 (`ƒ Proxy`).** Plan: `~/.claude/plans/tamam-bu-son-commitlerin-precious-fox.md`. **DURUM: COMMIT EDİLDİ local (`f24afb1`/`5265a08`); push bu turda görsel QA ile birlikte yapıldı.**

</details>

<details><summary>Önceki: İki branch'i hizalama — codex-experiment ↔ main birleştirme (`56ecbd1`/`39c0d07`)</summary>

**İki branch'i hizalama — codex-experiment ↔ main birleştirme + main hardening re-apply** (integration branch `integrate-codex`, merge commit `56ecbd1`). **Bağlam:** codex işini bitirdi (`origin/codex-experiment` @ `ee3b701`, ayrı Coolify ortamı); iki branch ayrıştı (codex 16 commit önde: TCMB kur göstergesi, profil avatarı + ayarlar redesign, AI/Excel import merkezi, teknik şablonlar, premium button; main 13 commit önde: bu oturumun final-ürün turları). Kullanıcı: **iki branch'i aynı güncelliğe getir** (kod birebir aynı, branch'ler ayrı deploy kalsın). **Yöntem:** `git merge-tree` in-memory trial (working tree'ye dokunmadan) → **4 içerik çakışması, 9 dosya temiz oto-merge.** **Phase 0 baseline gate:** codex kendi başına yeşil doğrulandı (tsc 0/lint 0/**4479 test**/build 0) — kırık "kazanan" adopt etme riski elendi. **Phase 1-2:** integration branch (main'e doğrudan dokunulmadı, main=prod); 4 çakışma **codex scaffold** temel alınarak çözüldü (`settings/page.tsx` + `product-types.ts`/`page.tsx`/`[id]/page.tsx` — codex bu sayfaları paralel YENİDEN YAZMIŞTI: field soft-delete [ürün verisi korunur], `dbListProductTypesWithStats`, teknik şablonlar, kendi `buildFieldPayload`/`openEditField`/edit modal). **Main'in son turlarının sağlamlaştırması codex'in YENİ yapısına re-apply edildi (silinmedi — `feedback_no_silent_deletes`; kullanıcı "tüm sağlamlaştırmayı taşı" seçti):** **(1) [BLOCKER veri-bütünlüğü] field_key edit-modunda READ-ONLY** — doğrulandı codex products page hâlâ `attributes[f.field_key]` ile saklıyor (model değişmedi) ama codex edit modalı field_key'i edit'te `generateTechnicalFieldKey` ile yeniden üretiyordu → mevcut alan key'ini değiştirmek o tipteki TÜM ürünlerin değerini orphan bırakır; edit'te `readOnly`+`disabled`+onChange erken-return+"değiştirilemez/orphan" notu, new'de auto-generate korundu. **(2) settings a11y — codex'in GRUPLU nav'ına uyarlandı** (flat `role=tablist` gruplu yapıya uymaz → invalid ARIA; bunun yerine button `id={settings-tab-${tab.key}}` + dirty erişilebilir ad `aria-label={dirty ? "${tab.label} (kaydedilmemiş değişiklikler)" : undefined}` + content `role=region`+`aria-labelledby` + codex'in mevcut `aria-current`/`aria-hidden` korundu — geçerli ARIA). **(3) settings `&apos;` ham render fix** (`:1027` codex reintroduce etmişti → düz tek tırnak) **+ FirmaTab errBody parite** (`:196` `res.text()` → codex'in diğer handler'larının `errBody.error ?? ...` paternine hizalandı + catch err.message yüzeye). **Test reconciliation:** obsolet `product-types-ui` + `product-types-list-count` silindi (MY impl'i assert ediyordu, codex'e karşı kırılırdı; codex zaten helper/route testlerini kendi shape'ine güncellemiş) → yeni `product-types-field-key-guard.test.ts`; `settings-ui.test.ts` codex yapısına yeniden yazıldı; codex'in `settings-page-tabs.test.tsx` RTL query'si dirty-suffix regex'e uyarlandı (a11y improvement'ın doğal sonucu — dirty erişilebilir adda). **Phase 4 doğrulama (integration branch):** tsc 0 · lint 0 · **4604 test yeşil** (320 dosya) · build 0 (`ƒ Proxy`). 9 oto-merge dosyası da bu net'le doğrulandı (auto-merge ≠ semantik doğru → full suite net'tir). **Migrations 083** (technical templates) **+ 084** (excel import) deploy'da DB'ye uygulanmalı (kullanıcı). **DURUM: TAMAMLANDI** — kullanıcı onayıyla (083/084 codex'çe zaten uygulanmış, tek DB `ryvxpolvhvsycuqyphoa` → push güvenli) `4176c8d` HER İKİ branch'e ff+push edildi: `main 0914b28..4176c8d` + `codex-experiment ee3b701..4176c8d`. **`origin/main == origin/codex-experiment == 4176c8d` (birebir aynı, tree identical)** — iki Coolify prod deploy tetiklendi; `integrate-codex` silindi (iki branch'e de merge oldu). Plan: `~/.claude/plans/stok-r-nler-sayfas-n-detayl-calm-charm.md` (branch hizalama içeriğiyle güncel).

</details>

<details><summary>Önceki: Ürün Tipleri sayfası — final ürün (alan düzenleme UI + N+1 fix + a11y modal) (`0914b28`, codex merge'inde superseded)</summary>

**Ürün Tipleri sayfası — final ürün (alan düzenleme UI + N+1 fix + a11y modal + UI test)** (`/dashboard/settings/product-types` liste + `[id]` detay + 4 API route + `product-types.ts` helper). ⚠️ **NOT: bu turun product-types değişiklikleri `56ecbd1` merge'inde codex'in teknik-şablonlar rewrite'ı ile superseded oldu; field_key guard codex yapısına re-apply edildi.** **Dürüst çerçeve:** sayfa **olgun** (helper 516 satır + 4 RBAC `manage_product_types` route + length validation + 404/409/400 mapping; backend testleri helper ~25 + route ~33 + seed 12). **Güvenlik/veri-bütünlüğü bug'ı YOK.** Danışman triyajı → **1 headline (kullanıcı kapsam onayı "Evet ekle") + 2 sağlamlaştırma + test açığı.** **(1) [HEADLINE — fonksiyonel boşluk] Alan düzenleme UI'ı YOK'tu:** detay tablosu (`[id]/page.tsx`) bir alanın etiket/tip/birim/seçeneklerini yalnız düz metin gösteriyordu; per-row yalnız zorunluluk-toggle + ↑↓ + Sil. Oysa `PATCH /api/product-types/[id]/fields/[fieldId]` + `dbUpdateProductTypeField` etiket/tip/birim/seçenek düzenlemeyi **tam destekliyor** — UI sadece `{required}`/`{sort_order}` gönderiyordu → yazım hatası düzeltmek = sil+yeniden ekle = o tipteki **TÜM ürünlerde attribute kaybı**. Fix: satır-içi **"Düzenle"** butonu → edit modal (`role=dialog`+`aria-modal`+`aria-labelledby="edit-field-title"`); `editDraft`/`editFieldId` state; label_tr (zorunlu)/label_en/field_type/unit/options düzenlenebilir; **`field_key` READ-ONLY** (footgun: ürünlerin `attributes` JSONB kimliği — değiştirmek orphan bırakır) + **tip-değiştirme caveat'ı** (`var(--warning-text)`: "mevcut ürün değerlerini etkileyebilir"); PATCH body `{label_tr,label_en,field_type,unit,options}`. Paylaşılan saf `buildFieldPayload(draft)` helper (Ekle + Düzenle DRY; select→options array, non-select→null, number→unit, **non-number→null=stale unit önlenir**). **(2) [MED] Liste N+1 fetch + sessiz yanlış veri:** `loadTypes` liste sonrası HER tip için ayrı `?withFields=1` çağırıyordu (8 tip=9 istek) yalnız fieldCount için; kısmi fail → 16-alanlı Vana "0 alan" gösteriyordu. Fix: `dbListProductTypes` `.select("*, product_type_fields(count)")` nested-count + unwrap (PostgREST `[{count:N}]`→`fieldCount:N`, **additive** — ProductTypeRow reshape edilmez, product-create selector + import extract/classify etkilenmez); liste GET cache'li (60s, tag) + 4 field route `revalidateTag` (doğrulandı→bayatlamaz); `page.tsx` N fetch kaldırıldı, fieldCount liste response'undan (9 istek→1). **(3) [MED] Yıkıcı native `confirm()` → a11y modal:** `deleteType`+`deleteField` `window.confirm()` → `confirmDeleteType`/`confirmDeleteFieldId` state + inline `role=dialog`+`aria-modal`+`aria-labelledby` modal'lar (deleteField "ürünlerde alan kaybolur" uyarısı taşır); `deleteType` sonrası `window.location.href`→`router.push`. **Çürütülen iddia (danışman flagledi→şema okundu):** reorder `(i+1)*10` döngüsü sort_order çakışır mı? — **HAYIR** (migration 056'da `UNIQUE(product_type_id, sort_order)` YOK; yalnız non-unique index + `UNIQUE(product_type_id, field_key)`), bug değil. **Bilinçli DROP (danışman):** field_key düzenleme (footgun), mevcut ürün attribute migration (backend sözleşmesi), focus-trap/Escape (evrensel), create modal maxLength (server `validateStringLengths`). **+20 test:** yeni `product-types-ui.test.ts` (buildFieldPayload 7 saf + detay edit-modal/Düzenle/PATCH/field_key-readonly/caveat/confirm-yok 8 source-regex + liste fieldCount/N+1-yok 2) + yeni `product-types-list-count.test.ts` (nested-count unwrap 3). Mevcut 70 backend test DOKUNULMADI (product-types 70→92). tsc/lint/build temiz, **4460→4480 test yeşil**. **Canlı E2E — GÜVENLİ salt-okuma probe** (prod `ryvxpolvhvsycuqyphoa`, Playwright login→cookie, port 3000; **MUTASYON YOK — Yeni Tip/Alan Ekle/Düzenle/Sil/reorder ASLA tıklanmadı** çünkü product_type create/delete kalıcı+kısmen geri-alınamaz, audit_log bırakır): login→/dashboard · `GET /dashboard/settings/product-types` 200 + "Ürün Tipleri" başlığı + "N alan" kartları · **`GET /api/product-types` → 9 tip, HER tipte fieldCount, 7/9 >0 (nested-count unwrap CANLI), `Vana fieldCount=16 == detay alan sayısı=16` cross-check (DECISIVE — no-op DEĞİL, drift-proof)** · `GET .../product-types/Vana` 200 + **16 'Düzenle' butonu render** · **headline İNTERAKTİF (kullanıcı "gör" → MUTASYON YOK çünkü `openEdit` saf client state, PATCH yalnız "Kaydet"te): Düzenle→modal açıldı, `field_key` DISABLED+prefill "dn", `label_tr` round-trip "DN (Nominal Çap)", tip→select Seçenekler textarea görünür / tip→text gizlendi [dinamik form iki yönlü], İptal→kapandı [Kaydet ASLA TIKLANMADI]** → 🎉 PROBE PASSED (18/18). +Doğrudan helper prod çağrısı: Vana 16/Conta 13/Flans 9/Fitting 7/Bağlantı 8/Enstrüman 8/Sızdırmazlık 7 = seed birebir, `hasNested=false` (nested temizlendi). Modal-açılış/prefill/dinamik-form CANLI proven; yalnız save-PATCH + alan-silme MUTASYON olduğundan offline vitest + 33 route testi ile (canlı tetiklenmedi). **İlk probe `unstable_cache` bayat-entry yakaladı (önceki dev session'dan 8-tip eski shape) → `.next/cache` temizlenip yeniden çalıştırıldı, fix prod'da doğrulandı.** **DURUM: COMMIT+PUSH EDİLDİ (`0914b28` → main, `2509dcf..0914b28`, Coolify deploy tetiklendi; React Doctor advisory bloklamadı, "test" status check bypass).** Plan: `~/.claude/plans/stok-r-nler-sayfas-n-detayl-calm-charm.md` (Ürün Tipleri içeriğiyle güncel).

<details><summary>Önceki: Satın Alma Siparişi (Yeni) — birim fiyat + KDV auto-fill (`2509dcf`)</summary>

**Satın Alma Siparişi (Yeni) — ürün seçilince birim fiyat + KDV otomatik gelmiyor (fonksiyonel bug)** (`/dashboard/purchase/orders/new`). **Kullanıcı şikâyeti:** ürün seçince birim fiyat otomatik gelmiyor, KDV/toplam hesaplanmıyor (2 ekran görüntüsü — macOS TCC izni nedeniyle OKUNAMADI; sayfa kod incelemesi + AskUserQuestion ile kesinleşti). **Kök neden (danışman onayladı):** `new/page.tsx:274` ürün `<select>` `onChange`'i yalnız `product_id` set ediyor, `unit_price`'a HİÇ dokunmuyor → varsayılan `"0"` kalıyor → `grandTotal` (`:144`) `qty*price` ile hesaplandığından price=0 → "Tahmini Toplam" 0,00 kalıyor. **KDV ayrı bug DEĞİL — fiyatın 0 kalmasının türevi.** 2. ekran görüntüsündeki "hata" muhtemelen submit validasyonu (`:169` "birim fiyat sıfırdan büyük olmalı") — aynı kök neden (GÖRÜLMEDİ, varsayım). **(1) [HEADLINE — fonksiyonel] unit_price auto-fill:** yeni pure helper `pickPurchaseUnitPrice(product)` = ürünün **`cost_price`**'ı (satın alma = MALİYET fiyatı; `price` SATIŞ fiyatı, **fallback KULLANILMADI** — danışman footgun: PO'yu şişirir; cost_price null → boş, kullanıcı elle girer); `handleProductSelect(idx, productId)` `product_id`+`unit_price` birlikte set eder; `onChange` buna yönlendirildi. **(2) [para birimi güvenliği] koşulsuz doldur + görünür uyarı:** `cost_price` `product.currency` cinsinden, PO currency tedarikçiden (`:132`) — farklıysa otomatik dolan rakam yanlış birimde; karar koşulsuz doldur (şikâyet evrensel çözülür, OrderForm paterni) + `product.currency !== currency` satırlarda inline `var(--warning-text)` uyarı ("Ürün fiyatı {X} cinsinden — PO {Y}, kontrol edin"); otomatik kur çevrimi YOK (kur verisi yok). **(3) [görünürlük — "kdv tutarı" şikâyetine doğrudan yanıt] KDV kırılımı:** form yalnız tek "Tahmini Toplam (KDV dahil)" gösteriyordu → yeni pure helper `computePoTotals(lines)` ({subtotal/discount/vat/grand}, KDV %20 sabit, qty/price=0 atlanır = eski davranış); alt blok Ara Toplam · İskonto (varsa) · KDV (%20) · **Tahmini Genel Toplam (KDV dahil)** + "Kesin tutar sipariş kaydedilince hesaplanır" notu (danışman: kaydedilmemiş form = tahmin, sunucu RPC otoritatif → "Tahmini" çerçevesi korundu). **Veri doğrulaması (danışman'ın "fix no-op olur mu?" kritik uyarısı):** `/api/products?all=1` `redactProductsForPerms` ile redakte (cost_price ← `view_purchase_costs`); PO formunu kullanan admin/purchaser yetkili → cost_price gelir; seed (`api/seed/route.ts`) 8/7 ürüne cost_price set (yalnız CT-SS bilinçli null). **Bilinçli DROP:** `price` fallback (footgun), kur çevrimi (veri yok), server/route/RPC/`validatePoLines` (Faz 3 audit'li, sağlam), OrderForm/QuoteForm (kapsam dışı — not: **QuoteForm:442 currency-guard'ı ayrı şikâyet kaynağı olabilir, ayrı tur**). **+11 test:** `purchase-orders-ui.test.ts` genişletme (pickPurchaseUnitPrice 3, computePoTotals 5, source-regex onChange/currency-uyarı/KDV-kırılım 3); mevcut 28 test dokunulmadı (28→41). tsc/lint/build temiz, **4449→4460 test yeşil**. **Canlı E2E — GÜVENLİ salt-okuma probe** (prod `ryvxpolvhvsycuqyphoa`, Playwright login→cookie, dev server port 3000; **MUTASYON YOK — "Sipariş Oluştur" ASLA TIKLANMADI** çünkü POST kalıcı `purchase_orders`+`po_counters`+`audit_log` bırakır): login→/dashboard · `GET /dashboard/purchase/orders/new` 200 + başlık · ürün select + KDV (%20) + Genel Toplam satırları render · **`GET /api/products?all=1` → cost_price non-null 7/8 (REDAKSIYON E2E KULLANICIYI ENGELLEMİYOR — fix production'da çalışır, danışman kritik check CANLI kanıtla kapandı)** → 🎉 PROBE PASSED (10/10). Auto-fill/KDV DOM davranışı offline vitest + build ile kanıtlı. **DURUM: COMMIT+PUSH EDİLDİ (`2509dcf` → main, `b37764a..2509dcf`, Coolify deploy tetiklendi; React Doctor advisory bloklamadı, "test" status check bypass).** **Kullanıcı netleştirmesi (commit sonrası):** birim fiyat otomatik gelmek zorunda değil, gelebilir ama alış fiyatı tedarikçiye göre değişken → mevcut "düzenlenebilir öneri" davranışı bunu zaten karşılıyor; AskUserQuestion → kullanıcı **"şimdiki hali (öneri, ipuçsuz)"** seçti → kod değişmedi. ⚠️ **Ekran görüntüsü #2 GÖRÜLMEDİ** — #2 submit validasyon hatasıysa bu fix kapatır; başka bir hataysa kullanıcı bildirmeli.

</details>

</details>

<details><summary>Önceki: Ayarlar sayfası — final ürün (modal a11y + tablist a11y + entity render bug + hata mesajı paritesi) (`b37764a`)</summary>

**Ayarlar sayfası — final ürün (modal a11y + tablist a11y + entity render bug + hata mesajı paritesi)** (`/dashboard/settings` 5 sekme: Firma/Kullanıcı/Bildirimler/API/Yapay Zeka + `ResetDemoSection` "Tehlikeli Bölge" + 7 route + 3 helper). **Dürüst çerçeve:** sayfa **olgun** (1239 satır page.tsx + 6 mevcut test dosyası); tüm route'lar + helper'lar + ResetDemoSection okundu + danışman triyajı. **Güvenlik/veri-bütünlüğü bug'ı YOK** (RBAC `manage_settings` company PATCH/logo POST, user/* session-auth 401, şifre cookie'siz fresh-client mevcut-şifre doğrulaması, avatar SVG yasağı+orphan cleanup+path traversal sanitize, preferences strict-boolean+whitelist, company GET `SAFE_COMPANY_FIELDS` allow-list, `revalidateTag`, demo guard). Bu tur = **0 fonksiyonel headline + 3 a11y/disiplin + 1 görünür render hatası** (hepsi adlandırılmış precedent'li; danışman dürüstlük onayı — "uydurma headline yok"). **(1) [MED — headline] `ResetDemoSection` onay modalı a11y eksik (en yıkıcı aksiyon):** `ResetDemoSection.tsx:114-190` confirm overlay+panel iki çıplak `<div>` — `role/aria-modal/aria-labelledby` HİÇ yok; bu modal `POST /api/seed` (tüm prod veriyi siler) onaylıyor → panel `role="dialog"`+`aria-modal="true"`+`aria-labelledby="reset-demo-confirm-title"`, başlık "Emin misiniz?" id'li; focus-trap/Escape DROP (önceki turlarla tutarlı). **(2) [MED] Sol sekme menüsü tablist a11y + dirty-dot text-eşdeğeri yok:** `page.tsx:1190` düz `<button>` dizisi (role/aria-selected yok), sağ içerik tabpanel değil, dirty nokta yalnız görsel `<span>` → konteyner `role="tablist"`+`aria-label`, her buton `role="tab"`+`aria-selected`+`id={settings-tab-${key}}`+`aria-controls`, sağ içerik `role="tabpanel"`+`id="settings-tabpanel"`+`aria-labelledby={settings-tab-${activeTab}}` (aktif buton id — kırık-ARIA tuzağı için tek id), dirty nokta `aria-hidden` + dirty durum butonun erişilebilir adında (`${label} (kaydedilmemiş değişiklikler)`); roving-tabindex DROP (Tab-odaklı butonlar = WCAG bar korunur, danışman onayı). **(3) [LOW-MED → görünür] `&apos;` JS string literal'i ham görünüyor:** `page.tsx:927` ApiTab ternary dalı `"Bağlantı yok — &apos;Bağlan&apos; ile…"` JS string'di (JSX text değil) → `&apos;` decode olmaz, kullanıcı ham görür → düz tek tırnak `'` (U+0027; başka HTML entity tuzak — o da decode olmaz). **Doğrulandı (grep):** yalnız `:927`; `:953` `Paraşüt&apos;e bağlan` JSX text → doğru decode, DOKUNULMADI. **(4) [LOW-MED] FirmaTab kayıt hatası jenerik (somut repro):** `handleSave:157` `throw new Error(await res.text())` → catch jenerik "Kayıt başarısız" toast'lıyordu; ama client `validate()` name max-length kontrol etmez, sunucu `validateCompanyPatch` >200 char reddeder → 201-char ad → sunucu spesifik 400 mesajı düşüyordu (kardeş `KullaniciTab/BildirimlerTab` zaten `errBody.error` parse ediyor) → `errBody?.error` parse'a hizalandı + catch `err.message` yüzeye çıkarır. **Bilinçli DROP (danışman doğruladı):** modal focus-trap/Escape (tüm turlarla tutarlı), roving-tabindex (WCAG dışı geliştirme), ApiTab/AiTab refactor (çalışıyor), 7 route+3 helper (denetlendi+6 test kapsıyor), `patchUserMetadata` GET-merge-SET race (dokümante+UI `isMutating` lock). **+10 test:** yeni `settings-ui.test.ts` (source-regex — ResetDemoSection modal a11y 2, tablist/tab/tabpanel+dirty-dot 4, `&apos;` :927 temiz+:953 korundu 2, FirmaTab errBody parse 2). 6 mevcut settings test dosyası dokunulmadı. tsc/lint/build temiz, **4439→4449 test yeşil**. **Canlı E2E — GÜVENLİ salt-okuma probe** (prod `ryvxpolvhvsycuqyphoa`, Playwright login→cookie, port 3000; **MUTASYON YOK — ResetDemoSection'a ASLA tıklanmadı, onay modalı bile AÇILMADI** çünkü `POST /api/seed` tüm prod veriyi siler/geri-alınamaz; Firma PATCH/şifre/avatar/preferences de tetiklenmedi): login→/dashboard · `GET /dashboard/settings` 200 + başlık "Ayarlar" · **`role="tablist"` canlı render (Finding 2 a11y kanıtı)** · FirmaTab+KullanıcıTab+ApiTab içerik render · **sayfada ham `&apos;` YOK (Finding 3 fix kanıtı)** · Tehlikeli Bölge görünür ama TIKLANMADI → 🎉 PROBE PASSED (10/10). Modal a11y/FirmaTab-hata yalnız offline vitest (prod'da zorlanamaz). **DURUM: COMMIT+PUSH EDİLDİ (`b37764a` → main, `c8057e5..b37764a`, Coolify deploy tetiklendi).** React Doctor advisory bloklamadı; "test" status check bypass edildi.

</details>

<details><summary>Önceki: Cariler (Müşteriler) sayfası — final ürün (toplu-silme bayat satır + hover antipattern + modal a11y + validation parity) (`c8057e5`)</summary>

**Cariler (Müşteriler) sayfası — final ürün (toplu-silme bayat satır + hover antipattern + modal a11y + validation parity)** (`/dashboard/customers` liste + filtre/arama + Pagination + toplu silme + Yeni Müşteri modalı + `CustomerDetailPanel` slide-in görüntüle/düzenle + 2 route + `customers.ts` servis). **Dürüst çerçeve:** sayfa **olgun**; servis + 2 route + panel + `data-context` müşteri fonksiyonları okundu + danışman triyajı. **Güvenlik/veri-bütünlüğü bug'ı YOK** (RBAC `manage_customers` POST/PATCH, `delete_customers` DELETE, GET `redactCustomersForPerms`, DELETE order-409 pre-check + audit-after-success Faz 6 paterni, demo guard, POST `validateStringLengths`). Bu tur = **1 fonksiyonel bug + 3 disiplin boşluğu** (hepsi adlandırılmış precedent'li). **(1) [MED — headline] Toplu silme bayat satır bırakıyordu (FONKSİYONEL) — danışman yakaladı, kendi self-check'imde kaçtı:** `handleBulkDelete` (`page.tsx:126`) ham `fetch` DELETE'ler atıp `clearAll()` yapıyordu — **yalnız seçimi temizler, `customers` state'inden silinen satırları KALDIRMAZ, refetch yok** → kullanıcı "N silindi" toast'ı görür, satırlar tabloda durur. Tek-satır silme (`deleteCustomer`→`setCustomers filter`) doğruydu → asimetri. Fix: bulk context `deleteCustomer` üzerinden (`Promise.allSettled(ids.map(id=>deleteCustomer(id)))`, her başarı kendi satırını filter'lar; `failed=rejected` sayımı — `deleteCustomer` `!ok`→throw); açık panel dahilse kapatılır. **Paired:** `DELETE /api/customers/[id]` `revalidateTag("customers")` YOK'tu (POST'ta var) → 30s `unstable_cache` silineni geri verebilir → eklendi. **(2) [MED] Liste hover DOM-mutation antipattern** (`page.tsx:311` `querySelectorAll("td").forEach(td=>td.style.background=...)`) → `hoveredId` state + koşullu `<tr>` background (orders/quotes/products/PO precedent). **(3) [MED] Modal/panel a11y — 3 çıplak modal:** toplu-silme onay modalı (`:482`) + Yeni Müşteri modalı (`:535`) + `CustomerDetailPanel` slide-in (`:127`) hiçbiri `role/aria-modal/aria-labelledby` taşımıyordu → üçüne eklendi (başlık id'li); focus-trap/Escape DROP (önceki turlarla tutarlı). **(4) [LOW-MED] validation parity:** `PATCH /api/customers/[id]` `validateStringLengths` YOK'tu (POST'ta var) → düzenleme panelinin sınırsız notes/address 10k+ geçebiliyordu → eklendi; `addCustomer` `throw new Error(await res.text())` ham `{"error":…}` JSON toast'lıyordu (kardeş `update/deleteCustomer` `errBody.error` parse ediyor) → parse'a hizalandı. **Bilinçli DROP (danışman doğruladı):** header "{N} aktif müşteri" alt-yazısı DOĞRU (liste `is_active=true` + hard-delete → DOKUNMA; "mislabeled" varsayımım yanlıştı), Pasif tab/`passiveCount` vestigial (her zaman 0, ileriye dönük iskele — DOKUNMA), Vergi Dairesi precedence kozmetiği. **+13 test:** yeni `customers-ui.test.ts` (9 source-regex — bulk context-delete 2, hoveredId 2, 3 modal a11y 4, addCustomer parse 1) + `customer-patch-route.test.ts` (+4 — PATCH 10k→400 ×2, DELETE 409/revalidateTag ×2). `customer-update-mapping.test.ts`/`customer-nav.test.ts` dokunulmadı. tsc/lint/build temiz, **4426→4439 test yeşil**. **Canlı E2E — GÜVENLİ salt-okuma probe** (prod `ryvxpolvhvsycuqyphoa`, Playwright login→cookie, port 3000; **MUTASYON YOK** — create kalıcı `customers` + delete kalıcı `audit_log customer_deleted`, hard-delete geri-alınamaz): login→/dashboard · `GET /dashboard/customers` 200 + başlık "Cariler" · `GET /api/customers` admin → **200, 4 müşteri** · loadError yok + arama input · satır tıkla → **CustomerDetailPanel `role=dialog` canlı render** (Finding 3 a11y kanıtı) → 🎉 PROBE PASSED (7/7). Bulk-stale/PATCH-10k/validation yalnız offline vitest. **DURUM: COMMIT+PUSH EDİLDİ (`c8057e5` → main, `0504bdc..c8057e5`, Coolify deploy tetiklendi).** React Doctor advisory bloklamadı; "test" status check bypass edildi.

</details>

<details><summary>Önceki: Üretim Girişi sayfası — final ürün (BOM şeffaflığı + silme onayı + a11y) (`0504bdc`)</summary>

**Üretim Girişi sayfası — final ürün (BOM eksik-bileşen şeffaflığı + silme onayı + a11y)** (`/dashboard/production` çok-satırlı form + sesli giriş + bugünkü/geçmiş kayıt tabloları + kayıt silme). **Dürüst çerçeve:** sayfa **zaten çok olgun** (Sesli Giriş V1–V3 + Faz 10 prefill); servis (`production-service.ts`) + atomik RPC (`complete_production`/`reverse_production`) + 3 route okundu + danışman triyajı. **Güvenlik/veri-bütünlüğü bug'ı YOK** (RBAC `manage_production` POST / `delete_production` DELETE, atomik FOR UPDATE bileşen kilidi + deterministik sıra, demo guard, shortage resolution non-fatal). Bu tur = **3 gerçek ama küçük boşluk.** **(1) [MED — headline] BOM eksik-bileşen detayı UI'a hiç ulaşmıyordu (fonksiyonel):** imalat ürününde bileşen yetersizse `complete_production` (008 hotfix `:158-172`) **zengin 409** döner (`shortages[{component_product_id, component_name, required_qty, available_qty}]`), route geçirir ama `addUretimKaydi` (`data-context.tsx:339`) yalnız jenerik `error`'ı throw edip `shortages`'i düşürüyordu → kullanıcı hangi hammaddeyi tedarik edeceğini bilemiyordu. **+Danışman yakaladı:** zengin mesaj yalnız hepsi-başarısız (`firstError`) dalında; **kısmi** dal (`succeeded>0&&failed>0`) jenerikti → çok-satırlı partide sessizce kaybolurdu. Fix: yeni client-safe pure `src/lib/production-shortage-helpers.ts` `buildShortageMessage(shortages, fallback)` (component_name ?? id + gerekli/mevcut; `voice-note-helpers` boundary precedent — server-only service'e KOYMA); `addUretimKaydi` `errBody.shortages` ile zengin mesaj; `handleSave` **kısmi dal** toast'ına `firstError`. **(2) [MED] Silme = tek-tıkla sessiz stok geri-alımı, onay YOK:** kayıt `×` (`page.tsx:589`) tek tıkta `reverse_production` → bitmiş ürün düşer + BOM bileşenleri geri yüklenir (geri-dönüşü zor envanter mutasyonu, domain §8.1); yalnız `deletingId` çift-tık koruması vardı. Fix: `confirmDeleteId` state + inline onay modalı `role="dialog"`+`aria-modal="true"`+`aria-labelledby="delete-production-title"` (başlık id'li) + ürün/adet özeti + "Vazgeç"/"Evet, sil (stok geri alınır)"; × yalnız modalı açar; `performDelete` handler; demo+`deletingId` korunur (PO `470578c` precedent — bu sayfada modal yoktu, a11y'ye gerçek hedef). **(3) [LOW] a11y:** tarih input `aria-label="Üretim tarihi"`; satır-kaldır × `aria-label`; silme × `aria-label` (title vardı, aria yoktu). **+Danışman (done-öncesi) — tip yalan footgun:** `component_name` RPC→servis→route zincirinde runtime taşınıyordu ama `ComponentShortage` (production-service.ts) + `CompleteProductionResult.shortages` (production.ts:107) tipleri alanı **omit** ediyordu → gelecekteki "tip temizliği" headline'ın yük-taşıyan alanını sessizce düşürebilir, tüm testler yeşil kalırdı; fix: iki tipe `component_name?: string` + `production-service.test.ts`'e davranışsal sözleşme testi (409 shortage component_name korunur). **Bilinçli DROP (danışman):** gelecek-tarih guard (domain §8 production_date tavanı belirtmiyor — spekülatif, feedback_plan_domain_check), ondalık adet reddi (DB integer, ayrık mamulde zayıf), servis/RBAC/RPC/migration (denetlendi). **+16 test:** yeni `production-shortage-helpers.test.ts` (6 saf davranış) + yeni `production-ui.test.ts` (9 source-regex — BOM/firstError 2, modal a11y 4, aria-label 3) + `production-service.test.ts` (+1 component_name sözleşme). `voice-production-page.test.ts` dokunulmadı. tsc/lint/build temiz, **4410→4426 test yeşil**. **Canlı E2E — GÜVENLİ salt-okuma probe** (prod `ryvxpolvhvsycuqyphoa`, Playwright login→cookie, port 3000; **MUTASYON YOK** — create kalıcı `production_entries`+`inventory_movements`+BOM tüketimi, delete ters hareketle stoğu geri alır → ikisi de geri-alınamaz): login→/dashboard · `GET /dashboard/production` 200 + başlık · `GET /api/production` admin → **200, 3 kayıt** · ürün select + 🎤 Sesli Giriş + Kaydet boş-form disabled + Bugünkü Kayıtlar bölümü → 🎉 PROBE PASSED (8/8). BOM-409/kısmi-toast/silme-onay yalnız offline vitest (prod'da zorlanamaz). **DURUM: COMMIT+PUSH EDİLDİ (`0504bdc` → main, `95ad46e..0504bdc`, Coolify deploy tetiklendi).** React Doctor advisory uyarısı bloklamadı; "test" status check bypass edildi. Plan o turda Üretim Girişi içeriğiyle güncel.

</details>

<details><summary>Önceki: Tedarikçiler sayfası — final ürün (a11y modal + görünür yükleme hatası + toplu-seçim kapsamı) (`95ad46e`)</summary>

**Tedarikçiler sayfası — final ürün (a11y modal + görünür yükleme hatası + toplu-seçim kapsamı)** (`/dashboard/vendors` liste + drawer create/edit + bulk deactivate + 2 API route). **Dürüst çerçeve:** sayfa **zaten çok olgun**; servis (`vendors.ts`) + 2 route okundu + danışman triyajı. **Güvenlik/veri-bütünlüğü bug'ı YOK** (RBAC `view/manage/delete_vendors`, validation parity `validateStringLengths`+servis `validateVendorInput`, soft-delete + **aktif-PO guard** 409, audit-actor, demo guard, RLS). Bu tur = **3 gerçek ama küçük boşluk.** **(1) [MED — headline] a11y:** bulk-deactivate confirm modal (`page.tsx:559`) **iki çıplak `<div>`** — role/aria-modal/aria-labelledby HİÇ yoktu; drawer (`:609`) `role=dialog`+`aria-label` vardı ama `aria-modal` yoktu. Fix: modal panel `role="dialog"`+`aria-modal="true"`+`aria-labelledby="bulk-deactivate-title"` (başlık id'li); drawer `aria-modal="true"` (PO turn1 `470578c` bulk-cancel modal precedent'i). **(2) [LOW-MED] görünür yükleme hatası — DÜRÜST: bu "sessiz hata" DEĞİL** (`loadVendors:149` zaten `!ok`→throw→catch→toast); gerçek boşluk: toast sönünce `vendors=[]`→"Henüz tedarikçi eklenmemiş" yanıltıcı empty-state. Fix: `loadError` state → `role="alert"` banner + **"Yeniden dene"** (refetch); empty-state + Pagination `!loadError`'a gate'lendi; toast korundu. **(3) [LOW] toplu-seçim aktif tedarikçilere kısıtlandı** — PO drop'u BURADA GEÇERSİZ (PO'da checkbox zaten yalnız cancellable satırda; burada per-row checkbox `:428` HER satırda guard'sızdı). "Pasifleri göster" AÇIK iken zaten-pasif seçilip "Pasife Al"→409 gürültüsü. Fix (`cancellablePageIds` aynası): `pageIds = pagedItems.filter(v=>v.is_active).map(...)` + per-row checkbox `v.is_active &&` gate. **Bilinçli DROP (danışman):** drawer Escape/focus-trap (PO'da da evrensel yok), client email/tax ipucu (servis doğruluyor), servis/RBAC/migration (denetlendi). **+8 test:** yeni `vendors-ui.test.ts` (source-regex — modal/drawer a11y 4, loadError/banner/empty-gate 3, toplu-seçim is_active 2; not: empty-`pageIds` güvenli — `computeIsPageAllSelected` `length>0 &&`, indeterminate `length===0→false`). `vendors.test.ts` (route/servis 30) dokunulmadı. tsc/lint/build temiz, **4402→4410 test yeşil**. **Canlı E2E — GÜVENLİ salt-okuma probe** (prod `ryvxpolvhvsycuqyphoa`, Playwright login→cookie, port 3000; **MUTASYON YOK** — vendor create/update/deactivate ASLA, kalıcı `audit_log` satırı bırakır): login→/dashboard · `GET /dashboard/vendors` 200 + başlık · `GET /api/vendors` admin → **200, 0 kayıt** (prod boş — empty path doğal) · drawer açıldı "Yeni Tedarikçi" + **loadError banner YOK** → 🎉 PROBE PASSED (7/7). Hata/409/bulk-noise yalnız offline vitest (prod'da zorlanamaz). **DURUM: COMMIT+PUSH EDİLDİ (`95ad46e` → main, Coolify deploy tetiklendi).** Plan: `~/.claude/plans/stok-r-nler-sayfas-n-detayl-calm-charm.md` (o turda Tedarikçiler içeriğiyle güncel).

</details>

<details><summary>Önceki: Satın Alma Siparişleri sayfası — final ürün 2. tur (`448c548`)</summary>

**Satın Alma Siparişleri sayfası — final ürün (2. tur: sessiz yükleme hatası kapatma + tarih tutarlılığı)** (`/dashboard/purchase/orders` liste + `[id]` detay + `new` form + `[id]/print`). **Dürüst çerçeve:** sayfa **zaten `470578c`'de final ürün gönderilmişti**; tüm yüzeyler + 9 API route + servis + 4 migration okundu + danışman triyajı. **Güvenlik/veri-bütünlüğü bug'ı YOK** (CAS race guard, RPC guard, RBAC finansal redaction `view_purchase_costs`, demo guard, modal a11y, validation parity, print veri-min.). Bu tur = **küçük ama gerçek güvenilirlik boşluğu** + minör polish. **2 düzeltme (danışman triyajından geçen):** (1) **[MED — headline] Sessiz yükleme hatası** — liste `loadOrders` (`page.tsx:82`) `if (ordersRes.ok)` ile sarılıydı → server 401/403/500'de toast YOK, bayat/boş liste + vendorMap hiç dolmaz → kullanıcı finansal sayfada yanıltıcı "Henüz sipariş yok" görür (detay `loadPO` dürüst ele alıyordu — tutarsızlık). New form (`new/page.tsx`) `if (vRes.ok)`/`if (pRes.ok)` aynı şekilde yutuyordu. Fix: `loadError` state (ürün-detay `attachmentsError` paterni) → `!ok` ise `role="alert"` banner + **"Yeniden dene"** (refetch); empty state dalı `loadError`'dan SONRA (MED-1 belirsizliği bununla kapanır). New form'a sayfa-üstü banner + retry. (2) **[LOW] expected_date ham ISO → tr-TR** — liste:337 + detay:230 `2026-05-15` basıyordu (created_at tr-TR iken) → yeni export `formatExpectedDate(iso)` (`+"T00:00:00Z"` UTC, null→"—"), iki noktada da. **Bilinçli DROP (danışman):** LOW-2 clear-butonu aria-label; MED-3 bulk-cancel re-validate (checkbox zaten yalnız cancellable PO'larda + backend guard'lı = non-issue). **+9 test (4393→4402).** Canlı GÜVENLİ probe 7/7 (prod 0 PO, MUTASYON YOK). **COMMIT+PUSH EDİLDİ (`448c548` → main).**

</details>

<details><summary>Önceki: Paraşüt Sync sayfası — final ürün (`ca34198`)</summary>

**Paraşüt Sync sayfası — final ürün (kritik: kırık manuel sync + dürüst durum göstergeleri)** (`/dashboard/parasut` + 9 API route). Sayfa **finansal olarak kritik** (muhasebeye cari/ürün/fatura/e-fatura/irsaliye buradan akıyor). "Önce doğrula sonra düzelt" + danışman. **Kapsam sınırı (açık):** bu tur = sync **sayfası** UI + endpoint kablolaması + durum dürüstlüğü; **servis finansal doğruluğu** (fatura/e-belge/iskonto mutabakatı, `parasut-service.ts` 1626 satır) Faz 1-11 + Bulgular denetimli → kapsam DIŞI. **5 düzeltme:** (1) **[HIGH] Kırık Manuel Sync** — `runSync` (+ scope-kart "Sync Et") `POST /api/parasut/sync-all` çağırıyordu ama `sync-all` `CRON_PATHS`'te (`proxy.ts:23`) → tarayıcıdan Bearer yok → `proxy.ts:129` **401 "CRON_SECRET gerekli"** → sayfanın ana eylemi tamamen çalışmıyordu (M-3'te dokümante-ertelenen bug). Fix: yeni **`POST /api/parasut/sync-pending`** (`requirePermission(manage_parasut)` → `serviceSyncAllPending`, per-order `/sync` paterninin aynası); UI hedefi `sync-pending`'e taşındı; `sync-all`+CRON_PATHS DOKUNULMADI (cron yolu korunur). (2) **[MED] Sahte "Son sync"** — `lastSyncTime` hardcoded `"17 Mar 2026 · 14:30"` (finansal sayfada yanıltıcı) → `stats/route.ts`'e **filtresiz** `last_sync_at` (en yeni `integration_sync_logs.requested_at`; danışman: `logs[0]` filtreye bağlanırdı→reddedildi); UI `stats.last_sync_at` türevli, yoksa "Henüz sync yapılmadı". (3) **[MED] Sahte ilerleme tiyatrosu** — "Cariler/Faturalar/**Ödemeler** sync ediliyor" (ödeme hiç sync edilmiyor) + sahte 20/50/80/100% → tek dürüst "Senkronize ediliyor…" belirsiz durum (`syncStep`/`syncProgress`/`syncStepLabel` state kaldırıldı). (4) **[LOW] config RBAC paritesi** — `GET /api/parasut/config` kardeşlerinden farklı guard'sızdı → `requirePermission(view_parasut)` (demo branch guard öncesi korundu). (5) **[LOW] a11y** — hata-genişletme `<span>` klavye erişilebilir (role/tabIndex/onKeyDown/aria-expanded) + 3 log filtresine aria-label. **Kapsam dışı (bilinçli):** bağlantı "Bağlı" semantiği `config.enabled` türevli. **+11 test (4382→4393).** Canlı GÜVENLİ probe 6/6 (PARASUT disabled→sıfır mali belge; sync-pending 200 / sync-all 401). **COMMIT+PUSH EDİLDİ (`ca34198` → main).**

</details>

<details><summary>Önceki: Satın Alma Siparişleri sayfası — final ürün 1. tur (`470578c`)</summary>

**Satın Alma Siparişleri sayfası — final ürün (eksik kapatma + salt-okuma canlı E2E)** (`/dashboard/purchase/orders` liste + `[id]` detay + `new` form + `[id]/print` + 9 API route). "Önce doğrula sonra düzelt": tüm sayfalar/route'lar/servis okundu + danışman. Sayfa ÇOK olgun (Faz 3-9 + Bulgular turları); **güvenlik/veri-bütünlüğü bug'ı YOK** (CAS race guard, RPC guard'ları, RBAC redaction, print veri-minimizasyonu sağlam). **5 gerçek polish düzeltmesi (çekirdek + a11y + parite):** (1) **Liste hover DOM-mutation antipattern** (`page.tsx:283-285` `e.currentTarget.style.background`) → `hoveredId` state + `isHovered` koşullu background (orders/quotes precedent'i; tek hedef, confirmId tuzağı yok). (2) **Validation parity** — `notes`/`lines[].notes` doğrulamasızdı → `POST /api/purchase-orders` + `PATCH [id]` + `PUT [id]/lines` üçüne `validateStringLengths(body)` (safeParseJson sonrası, RPC öncesi; recursive → top-level + array-of-objects `lines[].notes` kapsanır; kardeş `from-recommendations` zaten taşıyordu). (3) **Toplu iptal seçimi** received/cancelled PO'ları seçiyordu (→ 409 gürültüsü) → `isPoCancellable` export + `cancellablePageIds` → select-all+satır checkbox yalnız iptal edilebilir PO'larda (satış siparişleri `isOrderCancellable` paritesi). (4) **Client/server fiyat tutarsızlığı** (`new/page.tsx:161` `price < 0` → `unit_price=0` geçiyordu ama server `validatePoLines` `<= 0` reddediyor) → client `<= 0`'a hizalandı. (5) **a11y** — liste toplu-iptal modalı `role=dialog`/`aria-modal`/`aria-labelledby` HİÇ yoktu + detay iptal modalına `aria-labelledby` eklendi. **+12 test (4370→4382).** Commit `470578c` → main. Plan o turda PO içeriğiyle günceldi.

</details>

<details><summary>Önceki: Öneriler (Satın Alma Önerileri) sayfası — eksik kapatma + canlı E2E (`5e6e097`)</summary>

**Öneriler (Satın Alma Önerileri) sayfası — eksik kapatma + canlı E2E** (`/dashboard/purchase/suggested`, Sidebar "Öneriler"). "Önce doğrula sonra düzelt": 3 Explore agent + kod doğrulaması. Sayfa zaten ÇOK olgun (G11 audit 12 tur + Sprint C). **Çürütülen iddialar:** demo guard (middleware+client zaten var), PurchaseOrderModal/AIDetailDrawer a11y (role/aria-modal/aria-label/Escape zaten var), parallel-PATCH race (DB-katmanı CAS guard'ları serileştiriyor) — hepsi no-op. **4 gerçek düzeltme (çekirdek + kozmetik):** (1) **Validation parity** — `PATCH /api/recommendations/[id]` `feedbackNote` + `POST /api/purchase-orders/from-recommendations` `notes`/`lines[].notes` doğrulamasızdı → her ikisine `validateStringLengths(body)` tek çağrı (standalone `@/lib/validation/string-lengths`'ten; recursive → PO `lines[].notes` array kapsanır, plain nested obj kapsanmaz=projenin paylaşılan helper davranışı, genişletilmedi). (2) **RBAC** — `GET /api/purchase/suggestions` kardeşleri `view_purchase_suggestions` isterken guard'sızdı (orphan route, frontend/cron yok → hardening) → `requirePermission` eklendi (SİLİNMEDİ). (3) **a11y** — arama input `aria-label`. (4) **Kozmetik** — 2× hardcoded `rgba(248,81,73,0.04)` → yeni `var(--danger-bg-subtle)` (globals.css). **+11 test (4359→4370):** `recommendations-validation-parity.test.ts` (7: PATCH/POST 10k→400 servis çağrılmaz + RBAC guard) + `purchase-suggested-ui-polish.test.ts` (4 source-regex). tsc/lint/build temiz. **Canlı E2E smoke** (prod `ryvxpolvhvsycuqyphoa`, salt-okuma+accept→undo, /api/seed ASLA, PO oluşturmaz): sayfa 200 · copilot 200 (4 öneri) · `GET /api/purchase/suggestions` admin→200 · **PATCH feedbackNote 10k→400 + POST notes 10k→400 (yeni guard CANLI kanıt)** · accept→undo status=suggested'a döndü → 🎉 SMOKE PASSED (8/8). Not: accept bir zararsız `ai_feedback` audit satırı bırakır, undo status'u geri alır. **DURUM: çalışıldı, offline+canlı yeşil; COMMIT/PUSH kullanıcı onayı bekliyor.** Plan: `~/.claude/plans/oneriler-sayfasi-final-urun.md`.

</details>

<details><summary>Önceki: Stok & Ürünler sayfası — eksik kapatma + canlı E2E (`982c4bb`)</summary>

**Stok & Ürünler sayfası — eksik kapatma + canlı E2E.** "Önce doğrula sonra düzelt": Explore agent'larının öne sürdüğü "8 UX bug + eksik enrichment" iddialarının çoğu kod karşısında YANLIŞ çıktı (handleSave edit-mode'dan çıkıyor, kaydet loading var, create form resetleniyor, POST enrichment tam, detay GET stockout/deadline UI'da tüketilmiyor). Sayfa olgun; **2 gerçek düzeltme:** (1) **Liste hover DOM-mutation antipattern** (`products/page.tsx:832` `td.style.background` doğrudan yazım) → `hoveredId` state + koşullu `background: rowBg` (orders/quotes precedent'i; `confirmDeleteId` korundu). (2) **PATCH/POST validation parity** — `PATCH /api/products/[id]` hiç doğrulama yapmıyordu + POST negatifleri geçiriyordu → yeni saf modül `src/lib/validation/product-input.ts` `validateProductInput(body,{requireCore})` (string uzunluğu + numeric `>MAX` + yeni `<0` guard); her iki route'ta. **Mock blast-radius:** helper products.ts'te DEĞİL (62 test mock'luyor); `validateStringLengths` `src/lib/validation/string-lengths.ts`'e ayrıldı, `api-error.ts` re-export eder (request-ip precedent'i). (3) detay 3 dialog ZATEN `aria-modal`+`aria-label` taşıyor → non-issue. **Test +20 (4339→4359), tsc/lint/build temiz.** **Canlı E2E smoke** (prod, ZZTEST prefix + temizlik, /api/seed ASLA): gerçek session→ssr cookie→gerçek route'lar — GET list/POST/PATCH/DELETE + **PATCH negatif→400 (yeni guard canlı kanıt)** + 10k string→400 → 🎉 SMOKE PASSED; tek test satırı soft-deleted. (Playwright `products.spec.ts` ssr session-replay quirk'i ile /login'e düşüyor — creds geçerli, pre-existing env sorunu, kodla ilgisiz.) **DURUM: çalışıldı, offline+canlı yeşil; COMMIT/PUSH kullanıcı onayı bekliyor.** Plan: `~/.claude/plans/stok-r-nler-sayfas-n-detayl-calm-charm.md`.

</details>

<details><summary>Önceki: Satış Siparişleri Faz 3 — pending_approval rezervasyon (`40731af`, migration 082)</summary>

**Satış Siparişleri — Faz 3: HARD rezervasyonu "Bekliyor"a (pending_approval) taşı** (ürün sahibi kararı; domain-rules §5.1 BİLİNÇLİ DEĞİŞTİ). Yeni akış: **Taslak → "Onaya Gönder" (onay diyaloğu + HARD rezervasyon + stok kontrol + shortage uyarısı, alt-not "Teklif yapıldı") → Bekliyor → "Onayla" (light ticari teyit, alt-not "Stok rezerve edildi") → Onaylı**. LOJİSTİK: Bekliyor'da fulfillment=allocated → "Rezerveli" otomatik işaretlenir. **Migration 082** (`allocate_order_lines` helper + `submit_order_for_approval` [draft guard, zero-stock reddi, hedef pending] + `approve_order` [light + legacy unallocated fallback] + rezervsiz-pending backfill; eski `approve_order_with_allocation` health-check için korundu; ROLLBACK). **Çift sayma fix (ZORUNLU):** `dbGetQuotedQuantities`/`dbGetQuotedBreakdownByProduct` filtresi `['draft','pending_approval']`→`['draft']` (pending artık `reserved`'da; `quoted`=draft, net `promisable` invariant korunur). **Servis:** `dbSubmitOrderForApproval` helper; `dbApproveOrder`→`approve_order` RPC; order-service pending dalı artık allocate eder + shortages döner, approved dalı light. **POST /api/orders:** pending_approval istenince DRAFT oluştur→`serviceTransitionOrder(pending_approval)` (allocate), shortages/submitError yanıtta. **UI** (`[id]/page.tsx`): STATUS_META pending="Teklif yapıldı"; "Onaya Gönder"→requestTransition (confirm); "Onayla" sade confirm; shortage dialog pending dalında. **cancel_order/ship_order_full DEĞİŞMEDİ** (cancel pending rezervi release eder, ship approved guard korunur). **Test: 4334 yeşil** (DR-5.1 invariant tersine çevrildi, db-quoted/quoted-breakdown mock güncel, +order-submit-allocation-migration + order-service pending + create-and-send) · tsc temiz · lint 0 · build OK (`ƒ Proxy`). **DURUM: COMMIT+PUSH EDİLDİ (`40731af` → main, Coolify deploy tetiklendi) · ⚠️ Migration 082 APPLY BEKLİYOR — deploy biterken uygula (082 backfill ORD-0001/0002'yi rezerve eder; eski kod hâlâ quoted=draft+pending sayarsa geçici çift sayma).** domain-rules.md §4.4/§5.1/§5.2 + CLAUDE.md stok modeli güncellendi.
...
**Önceki:** **RBAC Faz 4/6/7 denetim bulguları (F1-F7) KAPATILDI** (worktree `rbac-bulgular-kapatma`; 4269 test). Salt-okuma denetim raporu (`~/.claude/plans/...role-wobbly-rainbow.md`) 7 bulgu çıkardı; hepsi **server-side** kapatıldı (substrate honesty — kozmetik UI maskelemesi gerçek redaction'la desteklenir, "tiyatro" değil). **F1 (YÜKSEK):** `/api/products/[id]` GET+PATCH + `POST /api/products` redaksiyonsuzdu (ham `price`/`cost_price` demo dahil sızıyordu) → liste route paterni (`getCurrentUserPermissions`+`redactProductsForPerms([x])[0]`, cache DIŞINDA). **F2:** `/api/products/[id]/quotes` `lineTotal` hamdı (`lineTotal/quantity`→fiyat türetilir) → `view_sales_prices`'a bağlandı. **F3+F4 (kuplajlı):** denetim "PATCH'ten shipped'i kaldır" diyordu ama detay "Sevket" PATCH `{transition:"shipped"}` kullanıyor → **per-transition guard** (shipped→`ship_sales_orders`: sales 403/production çalışır; diğer→`manage_sales_orders`; guard validation'dan ÖNCE — authz-önce, advisor catch). F4 production'a PATCH ship açtığından PATCH + `/ship` response'ları `redactOrderForPerms` ile redakte edildi. F4-UX: detay "Sevket" `has("ship_sales_orders")` gate. **F5:** aging `boundCapital` fallback `cost_price ?? price` → price fallback YALNIZ `canSales` true iken (purchasing satış fiyatını türetemez). **F6:** `dbDeactivateVendor(id, actor)` + route `getCurrentUserId()` (Faz 6 audit-actor disiplini; vendor soft-delete bilinçli — hard-delete'e çevrilMEDİ). **F7:** `CustomerDetailPanel` CTA'ları `has("manage_sales_orders")`/`has("manage_customers")` gate. **+25 test** (F3 için order-ship-parasut diskriminatif redaction lock dahil — advisor) · `api-mappers.ts` DOKUNULMADI · tsc · 4269 test · lint 0 · build OK (`ƒ Proxy`). **Kapsam dışı (named):** quote preview/print + PO print kendi server fetch'i → server-side preview redaction ayrı follow-up. **DURUM: worktree branch'te COMMIT EDİLDİ; main merge/push BEKLİYOR.** **Önceki:** **RBAC Faz 7 TAMAMLANDI — dashboard & UI maskeleme (RBAC planının SON fazı)** (worktree `rbac-faz7-ui-masking`; 4244 test). Kozmetik ikinci-katman: yetkisiz role yanıltıcı "₺0,00" yerine **"—"**. Gerçek koruma zaten API redaction'da (Faz 4) — bu faz UI'ı doğru gösterir + birincil mutasyon CTA'larını gizler. **`api-mappers.ts`'e DOKUNULMADI** (redakte null→0 mapper'da kalır); maskeleme **permission tabanlı** (null değere göre değil). Yeni `src/lib/auth/use-permissions.tsx` (`PermissionProvider`+`usePermissions()` — `/api/auth/me`'yi BİR KEZ fetch, Sidebar ad-hoc fetch'i dedupe; `perms===null`→`has()` true, server gate korur) + `maskCurrency(amount,currency,canView)` (utils). Maskeleme redact.ts ile AYNI yetki: RecentOrders/orders/quotes→sales; products price→sales/cost→cost; customers + CustomerDetailPanel revenue→`view_financial_summary` (per-order grandTotal→sales); aging boundCapital→cost. **Substrate honesty (advisor blocker):** 2 route Faz 7'de **server-side redaction kazandı** (önceden ham fiyat sızdırıyordu → kozmetik mask "tiyatro" olurdu): `GET /api/products/aging` + `GET /api/products/[id]/quotes`. **CTA gating** (Faz 3 zaten 403; pür UX): Yeni Sipariş/Teklif/Müşteri/Ürün/PO/Tedarikçi→`manage_*`, Sil→`delete_*`. Purchase list/detail total-mask ATLANDI (matris hepsi view_purchase_costs tutar→no-op). **KAPSAM DIŞI (named):** quote preview/print + PO print kendi server fetch'i (route redaction bypass; PO print leak yok—matris cost-yetkili; quote preview viewer için teorik gap—Mod A localStorage+manage_quotes yok→draft yaratamaz) → server-side preview redaction ayrı follow-up. **4244 test · tsc · lint 0 · build OK (`ƒ Proxy`). DURUM: COMMIT EDİLDİ (main rebase + push bekliyor).** **Önceki:** **Faz 8 Bulgular 1. tur — Paraşüt reconcile retry-bypass + doc/P3** (2026-05-31, "önce doğrula sonra düzelt", 3 bulgu; 2 commit). **P1/P2 (FİNANSAL, `f7fc9f8`):** Faz 8e reconciliation guard yalnız ana sync'teydi (`serviceSyncOrderToParasut`); manuel invoice retry (`serviceRetryParasutStep`, step=invoice → `upsertInvoice` header iskontoyu `discount_value`'ya uygular) guard'sız claim alıp sessiz yanlış fatura üretebiliyordu. Fix: `guardDiscountReconciliation` helper'ı çıkarıldı, her iki yolda claim ÖNCESİ; retry'da `step==="invoice"` gating. **P2 doc:** migration 080 APPLY EDİLDİ. **P3 (kozmetik):** stale yorum/test temizliği. **4215 test** · tsc temiz · lint 0 · build OK (`ƒ Proxy`). **Önceki:** **RBAC Faz 6 TAMAMLANDI — delete policy** (commit `5e0df84`, worktree-rbac-faz6-delete → FF→main; 4212 test). DELETE uçları domain `delete_*` yetkisine bağlandı (customers→delete_customers, vendors→delete_vendors, production→delete_production, orders→delete_sales_orders; quotes zaten delete_quotes/Faz 8a) — **davranış koruyucu** (manage_X tutan rol delete_X'i de tutuyor → kimse erişim kaybetmez, yalnız semantik). `getCurrentUserId` helper + 3 hard-delete helper'ına (customer/order/quote) **audit-after-success** pattern: before-snapshot silmeden ÖNCE çekilir, audit YALNIZ delete başarılı olunca yazılır → FK restrict throw ederse yalan `*_deleted` audit kalmaz (advisor blocker; dbDeactivateVendor güvenli paterni). 409 ilişki kontrolleri zaten mevcuttu. **Bilinen refinement:** customers→invoices, orders→shipments/invoices restrict FK'leri 409 pre-check'te değil → dürüst 500 (audit-after sayesinde correctness etkilenmez), temiz 409 mesajı gelecek tur. Migration YOK (audit_log.actor mevcut). **Önceki:** **RBAC Faz 4 TAMAMLANDI** — quotes + PO finansal GET redaction + archive gate (commit `1db5865`, FF→origin/main; 4197 test). quotes (camelCase, view_sales_prices) + PO (snake_case, view_purchase_costs) + donmuş arşiv PDF gate. Sızıntı yüzeyi advisor ile doğrulandı (preview=localStorage/Mod A, [id] inline PDF render etmez).

- **RBAC Faz 4 merge (2026-05-31):** `worktree-rbac-foundation` (7 commit: Slice 1+2A/B1/B2/C/D/E) güncel main'e merge edildi. Foundation (Faz 1+2+5) zaten main'deydi (merge-base `a0130de`). **R3 finansal redaction** (route seviyesi, snake_case): products GET `price`/`cost_price`, customers `total_revenue`, orders `grand_total`/`subtotal`/`vat_total`+satır fiyatları → yetkiye göre null (`view_sales_prices`/`view_purchase_costs`/`view_financial_summary`; per-request, cache key'e girmez). **R1** ~40 mutation `requirePermission`. **R2** vendors/PO/commitments/recommendations + parasut invoices/stats/logs GET guard'ları. **R4** fail-closed admin, **R5** fail-closed unknown-route. **product-types** `requireRole(admin)`→`requirePermission(manage_product_types)`. **Çakışma çözümü:** 11 quotes dosyası (4 route+7 test) main'in **Faz 8a** RBAC'ını korudu (DELETE→`delete_quotes`, `/convert`→410 tombstone, `accepted`→410, sent→409) — Faz 4 quotes guard'ları gereksizdi (main daha rafine+güncel). `rbac-mutation-guards` convert testi 403→410 güncellendi. Branch+worktree merge sonrası temizlendi. **Bilinen kapsam:** quotes GET redaction YOK (`view_quotes` tam fiyat görür — Faz 4 bilinçli; gelecek tur). **tsc temiz · 4174 test · lint 0 · build OK (`ƒ Proxy`).** Detay: `memory/project_rbac.md`.

- **Faz 8 (2026-05-31, "ertelenenleri halledelim"; 5 bağımsız kalem/commit):** **8a (`4935e88`) RBAC** — quotes yazma uçlarına requirePermission (POST/PATCH/revise→manage_quotes, DELETE→delete_quotes; GET auth-only); 7 mevcut route testine role-guard mock. **8b (`71a22cd`) convert ölü kod temizliği** — serviceConvertQuoteToOrder+ConvertResult kaldırıldı (yerini Faz 6 atomik accept aldı); dbFindOrderByQuoteId + /convert 410 stub KORUNDU. **8c (`034f8ea`) quotes audit katmanı** — helper seviyesi (RPC değil, product-types paterni; advisor): dbCreate/Update/Revise → audit_log (best-effort, actor'sız=codebase-tutarlı); RPC repro riski elendi. **8d (`4218d3e`) order_line_description — migration 080 (APPLY BEKLİYOR)** — order_lines+=description, accept RPC 078 gövdesi birebir+qli.description delta (master p.name korundu), TS/mapper/UI. **8e (`4b9c938`) Paraşüt iskonto orantılı** — guard→reconciliation: computeHeaderDiscountPct builder'da per-satır discount_value + reconcileParasutDiscount (orantılı toplam kendi kodumuzda, grand_total ile tolerans; mock net_total iskonto yok sayıyor); tolerans aşımı/subtotal=0→blok+alert, uyuşursa fatura OLUŞUR (order_lines mutate edilmez). **Kullanıcı kararları:** sig rename ATLA (kabul edilen isimlendirme), drag-reorder ERTELE. **4098→4098 (8e builder drift-guard +2)** · tsc/lint/build temiz. **DURUM: 5 commit COMMIT+PUSH EDİLDİ · 080 APPLY BEKLİYOR (yalnız 8d).**

- **Faz 7 Bulgular 2. tur (2026-05-31, kullanıcı review, P1 yok):** **#1 (YENİ P2 FIX)** unsaved draft restore'da not/teslimat/ödeme kaybı — `autoSave` `teklif_v3` draft key'ine yalnız `{currency,rows,descDirty,discount}` yazıyordu (notes/delivery/payment sadece `teklif_v3_full`'da); restore `teklif_v3` okuduğundan şablonla eklenen metin kaydetmeden refresh/preview-dönüşünde kayboluyordu → `teklif_v3`'e notes/deliveryMethod/paymentMethod eklendi + restore geri yükler (Faz 3 `discount` precedent'i; +2 test, 3 mevcut regex güncel). **#2 (P3 FIX)** settings liste tüm body'yi basıyordu (5000 char) → `-webkit-line-clamp:3` önizleme. **#3-5 (ZATEN DÜZELTİLDİ `0b9398c`)** rapor DB→404 / ?kind→tüm liste / doc drift'i tekrar gündeme getirdi; 1. turda düzeltilmişti (kod karşısında doğrulandı, ek yok; rapor eski snapshot). **#6 (no-op)** `[id]` GET inactive → bırakıldı. **4096→4098** · tsc/lint/build temiz.

- **Faz 7 Bulgular 1. tur (2026-05-31, kullanıcı review, P1 yok):** **#1 (P2 FIX)** `dbGetNoteTemplate` + update/deactivate ön-okumaları `.single()`+`if(error||!data)return null` gerçek DB/RLS hatasını not-found'a düşürüyordu → `.maybeSingle()`+`if(error)throw` (DB hatası artık 500, 404 değil). **#2 (P3 FIX)** geçersiz `?kind=` typo'su sessizce tüm şablonları döndürüyordu → **400 fail-closed** (picker zaten `?kind`'siz fetch eder → etkilenmez). **#3 (no-op)** `[id]` GET inactive döndürüyor ama tüketici yok + hassas değil → bırakıldı. **#4 (ertelendi)** pasif görme/reaktivate UI yok (soft-delete DB'de korunur; reaktivasyon PATCH is_active gelecek tur). **#5 (FIX)** QUOTES_V2_PLAN final numbering'e hizalandı + historical V5 migration blokları "⟵ SUPERSEDED" işaretlendi. **4094→4096** · tsc/lint/build temiz.

- **Faz 7 (2026-05-31, V7 master-plan'ın SON fazı):** PMT teklif formunun 3 serbest-metin alanı (Notlar/Teslimat/Ödeme) için tekrar kullanılabilir not şablonları. **080 KALICI DÜŞÜRÜLDÜ** — master-plan `079→quote_line_items_sort_order (koşullu)` diyordu ama `quote_line_items.position` zaten var (034:106) + `quotes.ts:88` ona göre sıralıyor + accept RPC ona göre order'lıyor → koşulsuz gereksiz; drag-reorder ertelendi → Faz 7 = **tek migration 079**. **Migration 079** (`note_templates`: kind `notes|delivery|payment|general` CHECK + title/body non-empty + sort_order + is_active soft-delete + RLS + updated_at trigger + partial index + **PMT seed** deterministik UUID/ON CONFLICT). **Erişim ayrımı (load-bearing):** `GET` requireRole YOK (satış picker'da okur) + `?kind=`; `POST/PATCH/DELETE` admin; DELETE soft-delete (hard-delete YOK). **Helper** `note-templates.ts` (validation + audit_log + isValidNoteTemplateKind). **TS/mapper** NoteTemplateRow/NoteTemplate/mapNoteTemplate. **Settings sayfası** `/dashboard/settings/note-templates` (kind filtre + create/edit modal + pasifleştir) + Sidebar linki. **QuoteForm picker:** 3 textarea üstü "+ Şablon ekle…" select; `templatesForField` (kind+general); `applyTemplateToField` (boş→doldur/dolu→append, sessiz üzerine-yazma YOK); readOnly'de gizli; setter'lar zaten autoSave dep'inde (drift trap yok). **Test (+51): 4043→4094** · tsc temiz · npm run lint 0 · build OK (`ƒ /api/note-templates` + `[id]` + settings + `ƒ Proxy`). **DURUM: COMMIT+PUSH EDİLDİ (3551302) · 079 APPLY EDİLDİ ✅.** **V7 master-plan Faz 1-7 + tüm Bulgular turları TAMAMLANDI.**

<details><summary>Önceki: Faz 6 Bulgular 2. tur (`9a57d66`, 077/078 APPLY EDİLDİ ✅)</summary>

- **Faz 6 Bulgular 2. tur (2026-05-31, "önce doğrula sonra düzelt", 5 bulgu):** **#1 (P2) arşiv create-race** `serviceArchiveQuotePdf` create-catch'i UNIQUE 23505'te satır görünce direkt success dönüyordu → kazanan henüz upload etmemiş/fail edip silmek üzere olabilir → accept arşivsiz referansa kayar; fix: catch'te satır + OBJE present birlikte, değilse throw (502→retry self-heal), yeniden üretmez (UNIQUE slot dolu). **#2 (P2/P3) accept fail-open→fail-closed** (advisor: tek doğru cevap, AskUserQuestion yok) — RPC 23514 guard'ı arşiv SATIRINA bakar, OBJEYE erişemez → `serviceArchiveQuotePdf` "dosya var mı" invariant'ının TEK noktası; üç-durumlu `dbArchiveObjectStatus` (present→ok / missing→sil+üret / **unknown→throw 502 retryable**); yıkma yalnız missing (sağlam arşiv korunur); `dbArchiveObjectConfirmedMissing`→tri-state, `dbArchiveObjectExists` (GET lenient) türer. **#3 (P3) order detail arşiv PDF linki** `quotePdfArchiveId` taşınıyordu ama UI'da yoktu → "Arşivlenmiş Teklif → 📄 Belgeyi Aç" (GET /api/quotes/{quoteId}/archive). **#4 (P3) doc drift** dddb1f9 push sonrası hâlâ BEKLİYOR+4034 diyen doc'lar hizalandı. **#5 (P3) lint** kullanıcı b17181e'de (lint fix öncesi) review → 3 set-state görmüş; HEAD npm run lint=0 (kod değişmez). **4040→4043** · tsc temiz · npm run lint 0 · build OK. **⚠️ 078 APPLY BEKLİYOR.** **DURUM (o tur): COMMIT+PUSH EDİLDİ (`9a57d66`); 077/078 APPLY EDİLDİ ✅.** Faz 6 yakınsadı. Faz 7 → 079-080.

</details>

---

<details><summary>Önceki: Teklif V7 Faz 6 Bulgular 1. tur (`b17181e`, 077 APPLY EDİLDİ ✅)</summary>

- **Faz 6 Bulgular 1. tur (2026-05-31, "önce doğrula sonra düzelt", 5 bulgu — 4×P2+1×P3):** **#1 (P2) phantom recover** Faz 4 vaadi tam kapanmamıştı — `serviceArchiveQuotePdf` existing-row yalnız DB satırına bakıyordu → accept eksik-dosyalı arşive sipariş bağlıyordu; fix: `dbArchiveObjectExists` doğrula + `dbDeleteQuoteArchive` (stale sil) → yeniden üret (sent donmuş → HTML birebir). **#2 (P2) sipariş detay** "KDV (%20)" hardcoded + iskonto yok → dinamik özet (Ara Toplam/İskonto/KDV Matrahı/KDV %vatRate/Genel Toplam). **#3 (P2) accept RBAC** proxy yalnız page-gate → viewer POST atabilirdi → `requirePermission(req,"manage_quotes")` (admin+sales; diğerleri 403). **#4 (P2/P3) RPC qty** yalnız küsürat kontrolü + jenerik 23514 "arşiv" yanlış-map → migration **078** qty `<=0 OR <>trunc`→22003 + service 23514→archive map kaldırıldı (kalan check ihlali dürüstçe 500). **#5 (P3) doc** 077 APPLY EDİLDİ hizalandı. **4021→4034** · tsc/build temiz · eslint src 31/0. **⚠️ 078'i bu deploy'la apply et** (yoksa legacy qty<=0 → 500). **DURUM: COMMIT+PUSH EDİLDİ (`b17181e`); 077 ✅/078 BEKLİYOR.**

</details>

---

<details><summary>Önceki: Teklif V7 Faz 6 — Accept → Sipariş (atomik) (`d4988ca`, migration 077 APPLY EDİLDİ ✅)</summary>

- **Faz 6 (2026-05-30, V5-A4 + V4-A8):** V7 master-plan'ın son büyük halkası. **Migration 077** (kullanıcı apply eder): `sales_orders` +4 kolon (discount_amount/vat_rate/source_quote_revision_no/quote_pdf_archive_id, V7-A9) + `accept_quote_and_create_order(p_quote_id, p_actor)` atomik RPC (V7-A1 SECURITY INVOKER): FOR UPDATE → idempotency (mevcut sipariş→`already:true`) → status guard sent|accepted (42501) → null product_id (23502) + küsürat qty (22003) + arşiv NULL (23514) pre-check → order INSERT (**donmuş totaller kopyalanır** — `sales_orders` totals trigger'ı YOK doğrulandı → arşiv PDF ile bayt-bayt tutarlı; `LEFT JOIN customers` country/tax) → order_lines `JOIN products` (V7-A8 master kimlik) + `v_quote.vat_rate` satır snapshot (V7-A3) → ROW_COUNT verify mismatch→ROLLBACK (V7-A8) → item_count=v_inserted (V7-A10) → quote sent→accepted → audit_log. **Faz 3 iskonto-convert-bloğu KALKTI** (sipariş artık discount_amount taşır). **Service/route:** `serviceAcceptQuoteToOrder` (status guard → valid_until → **V7-A5 arşiv recover/generate** serviceArchiveQuotePdf reuse [fail→502] → RPC kod map) + `POST /api/quotes/[id]/accept`. **Deprecation (V4-A8):** PATCH transition:accepted→410, /convert→410, `QuoteTransition` accepted çıkarıldı; serviceConvertQuoteToOrder deprecate+korundu. **Paraşüt guard (V7-A4, coupled):** discount_amount>0 → claim öncesi early return + zorunlu sync_issue alert. **UI:** tek "Kabul Et ve Siparişe Dönüştür" butonu. **3974→4021** · tsc/build temiz (`ƒ Proxy` + `/api/quotes/[id]/accept`) · eslint src 31/0. **DURUM: COMMIT+PUSH EDİLDİ (`d4988ca`) + 077 APPLY EDİLDİ ✅.**

</details>

---

<details><summary>Önceki: Teklif V7 Faz 4 — PDF Arşiv (`6c9c317`, migration 075/076 APPLY EDİLDİ ✅) + Bulgular 1-4. review tur</summary>

- **Review 4. tur (Bulgular, 2026-05-30, 3 P3 — convergence):** **P3-1 (doc-only)** stale "BEKLİYOR" → 3. tur `da09dce` push + 075/076 apply EDİLDİ, hizalandı. **P3-2 (orphan phantom — contained fix)** `dbCreateQuoteArchive` insert-sonra-upload (concurrency için bilinçli; reorder ETMEDİK) → nadir crash penceresinde DB satırı var/dosya yok ("phantom"). Advisor: P2-B'den FARKLI, phantom **bugün kullanıcı-görünür** (signed URL üretilir, 404 window.open SONRASI → kırık sekme, graceful toast kapsamaz). Fix: `dbArchiveObjectExists` (storage `.list`) → archive GET signed URL'den önce varlık kontrolü → graceful 404. Kalıcı recover (object-existence, row değil) Faz 6. **P3-3 (kullanıcı: caveat kabul)** logo byte-freeze: arşiv logo URL'si saklar (byte değil), `upsert:true` → eski arşiv yeni logoyu gösterebilir; AskUserQuestion → caveat kabul (frozen-HTML pragmatik tercihiyle tutarlı; base64-inline reddedildi), kod değişmez. **3969→3974** · tsc/build temiz · eslint src 31/0.
- **Review 3. tur (Bulgular, 2026-05-30, COMMIT+PUSH `da09dce`):** **P2-A (regresyon — 2. turun yan etkisi)** toplu silme: sent draft-only kilidi sonrası checkbox tüm statüleri seçiyor + `handleBulkDelete` `succeeded>0` ise TÜM seçili id'leri düşürüyordu → sent 409 alıp UI'dan kayboluyor (refresh'te döner). Fix: (a) `pickSucceededIds(ids, results)` pure helper → yalnız fulfilled+ok düşürülür; (b) seçim sadece draft satırlar (`{deletable && checkbox}`, select-all `deletablePageIds`). **P2-B (kabul edilen boşluk, BLOKLAMAZ)** send-archive fail + kullanıcı yine accept → arşivsiz accepted; accept'i bloklamak karar A (send non-blocking) ile asimetrik → Faz 6 recover/generate kapatacak (kod yorumu + memory not, AskUserQuestion sorulmadı). **P3 (doc-only)** stale "COMMIT+PUSH BEKLİYOR" → 2. tur `bb3b3f2` push edildi, hizalandı. **3960 → 3969** · tsc/build temiz · eslint src 31/0.
- **Review 2. tur (Bulgular, 2026-05-30, COMMIT+PUSH `bb3b3f2`):** **B1 (P2) müşteri adresi** gönderimde zorunlu (validateQuoteForSend "resmi PDF") ama belgede yoktu → 4 nokta eklendi: `QuoteData.custAddress` + `BILINGUAL_LABELS.address` (Adres/Address) + QuoteDocument müşteri satırı + buildQuoteDataFromDetail + **QuoteForm autoSave/savePreviewData** (canlı önizleme — drift trap, advisor yakaladı). **B2 (P2) sent silme** → `canDeleteQuote` + DELETE route `draft`-only (immutable arşiv ON DELETE CASCADE koruması; sent-deletable regression testi tersine). **B3 (P2/P3)** archive-fail toast yanıltıcı "otomatik denenecek" vaadi kaldırıldı (gerçek recover yalnız Faz 6). **B4 (P3)** buton arşivsiz statüde → graceful 404 (info toast) zaten var, `sentAt` map'siz, P3 kabul. **B5 (P3)** doc test sayısı hizalandı. **3951→3960** · tsc/build temiz · eslint src 31/0.
- **Review 1. tur (Bulgular):** **P1 MIME** upload `text/html; charset=utf-8` ⟂ bucket `['text/html']` → her upload fail + sessiz yutma → fix contentType `"text/html"`. **P2 send arşiv** (kullanıcı kararı A): non-blocking ama görünür — `archiveWarning` flag → route → UI warning toast (sessiz değil). **P2 concurrency** serviceArchiveQuotePdf create fail → re-read existing (UNIQUE 23505 yarışı idempotent). **P2 doc drift** QUOTES_V2_PLAN.md V7-A5 Puppeteer→frozen-HTML hizalandı (Faz 6 serviceArchiveQuotePdf reuse). **P3** explicit git add. İyi: XSS temiz, no leak, totaller DB snapshot.

- **Mimari karar (kullanıcı):** dondurulmuş **HTML snapshot** — send anında `QuoteDocument` server-side `renderToStaticMarkup` → self-contained `.html` → 'quote-pdfs' private bucket (immutable). View = signed URL (template drift'e bağışık). PDF = browser-print. Reddedilen: binary PDF (Coolify chromium ~300MB + Faz 6 erken yük), JSON snapshot (template drift).
- **Phase 0 (kritik, çözüldü):** `QuoteDocument`'ten `"use client"` KALDIRILDI — Next App Router server graph'inde `"use client"` modülü client-reference proxy'ye çevrilir → `renderToStaticMarkup` BOŞ çıktı verir (vitest direkt-import maskeler = "testte geçer prod'da patlar"). Saf fonksiyon (hook/browser API yok) → kaldırma deterministik çözer; client preview shared component'i import etmeye devam (tek template). `PAGE_CSS`/`PRINT_CSS` export.
- **Build engeli (çözüldü):** route graph'inde `react-dom/server` statik import Turbopack reddi → `renderQuoteArchiveHtml` async + dinamik `await import("react-dom/server")`.
- **Migration 075:** `quote_pdf_archives` (quote_id FK cascade, revision_no, file_path, content_hash, byte_size) + **UNIQUE(quote_id, revision_no)** (V3-A5 INSERT-only immutability backstop) + RLS service_role + idempotent + ROLLBACK. **076:** `quote-pdfs` **private** bucket (text/html, 5MB) + storage policy.
- **Kod:** `quote-archive-html.ts` (`buildQuoteDataFromDetail` DB QuoteDetail→QuoteData server tek source [seller snapshot/company fallback; currency/status defansif map: bilinmeyen→TRY, revised→sent] + `renderQuoteArchiveHtml` async [renderToStaticMarkup + font wrapper `:root --font-doc-*` + Google Fonts link + PAGE_CSS]); `quote-pdf-archives.ts` (dbGetQuoteArchive eq+eq+maybeSingle / dbCreateQuoteArchive orphan-safe insert→upload→fail'de delete, path `quotes/{id}/r{rev}.html` / dbGetArchiveSignedUrl); `serviceArchiveQuotePdf` (idempotent V3-A5; **send hook NON-FATAL** — arşiv fail send'i bozmaz, Faz 6 recover/generate telafi); `GET /api/quotes/[id]/archive` (lookup-only, **file_path/content_hash SIZMAZ**); UI `[id]/page.tsx` Mod B "📄 Arşivlenmiş Teklif" butonu (status≠draft, window.open). V3-B6 (QuoteDocument `isRealRow` → 0 fiyat "0,00", boş filler "—").
- **Logo faithfulness:** `logo_url` = company-assets **public** bucket getPublicUrl → public absolute URL → arşivde `<img>` standalone çalışır (faithful; minör caveat: aynı path re-upload eski arşiv logosunu değiştirir, nadir). **Self-containment sınırı:** Google Fonts `<link>` view-time external (tam offline değil — kabul).
- **Test:** Faz 4 + 1. review (43) + 2. review (+9: quote-archive-html custAddress builder/render 3, quotes-faz4a-helper-mapper Bulgu 1 source-regex 4, canDeleteQuote revised 1, faz4 toast 1; sent-delete testi flip). **→ 3960 yeşil** · tsc temiz · build OK (`ƒ Proxy` + archive route) · eslint src 31/0.
- **DURUM: 1.+2.+3.+4. tur push edildi (`6c9c317`) + migration 075/076 APPLY EDİLDİ ✅.** Manuel smoke: signed URL donmuş HTML inline render + Adres/Address satırı + önizleme drift yok + A4; draft hariç Sil yok; bulk delete yalnız draft; phantom→graceful 404.

</details>

---

<details><summary>Önceki: Teklif V7 Revizyon Zinciri (`1d96211`, migration 074 APPLY EDİLDİ + review pass)</summary>
...
- **Tasarım:** V2 root_quote_id **flat chain** (tüm revizyonlar köke işaret; original'da NULL, revision_no=1). `create_quote_revision(p_source_id)` RPC (atomik, V7-A1 INVOKER): **kaynak atomik consume** (`update ... set status='revised' where id=p_source_id and status in (sent,rejected,expired) returning * into v_src`) → kök `FOR UPDATE` (revision_no serialize) → `revision_no = chain max+1` → suffix `-R{n}` → tüm header (discount_amount dahil) + satır kopya. quote_number UNIQUE backstop.
- **Review P1 fix (concurrency):** ilk versiyon kaynağı kilitsiz SELECT+guard ile okuyup sonda flip ediyordu → aynı kaynağa iki eşzamanlı revize ÇİFT revizyon üretebiliyordu (kök kilidi yalnız revision_no'yu serialize ediyordu). Atomik consume ikinci eşzamanlı isteği 42501'e düşürür. 074 apply edilmediği için yerinde düzeltildi. **Review P3:** `anyMutating` (loading/converting/revising) → 3 buton grubu da disable (eşzamanlı çift mutasyon önlendi).
- **Migration 074:** `revision_no`(default 1)/`root_quote_id`(FK, idx) kolonları + status CHECK `+revised` (034 deseni idempotent) + RPC. **APPLY EDİLDİ.**
- **Service/route/UI:** `serviceCreateQuoteRevision` (RPC 42501→invalidStatus/409, P0002→notFound/404); `POST /api/quotes/[id]/revise` (201 {newQuoteId}); GET enrichment `revisedBy`(zincirin en yenisi — "en güncele git") + `revisionOf`(kök) via `dbListQuoteChain`; `[id]/page.tsx` "Revize Et" butonu (`getQuoteReviseEligible`) → onay → `router.push(yeni draft)`; revisedBy/revisionOf rozetleri. STATUS_META+tab `revised`, QUOTE_TRANSITIONS `revised:[]` terminal.
...
- **DURUM: COMMIT+PUSH EDİLDİ** (`cb061c8` ilk + `1d96211` review fix [P1/P3] → main, Coolify redeploy) **+ migration 074 (DÜZELTİLMİŞ) APPLY EDİLDİ.** **Review pass (074 apply sonrası, 4 bulgu):** P1 → 074 `create or replace`+idempotent, orijinal hiç DB'ye girmemişti (atomik-consume yerinde fix) → apply edilen zaten düzeltilmiş gövde, patch gerekmez (DB onayı `\df+`); P2 audit → quotes modülünün hiçbir RPC'si (create/update/convert/revise) `audit_log` yazmıyor (modül-geneli mevcut borç, revizyona özel regresyon değil) → **kullanıcı kararı: kabul + "quotes audit katmanı" gelecek faz olarak dokümante**; P2 RBAC → hiçbir quotes route `requireRole` kullanmıyor, revise convert aynası (proxy.ts korur) → RBAC merge'de `/api/quotes/[id]/revise` permission matrix'e eklenecek (project_rbac.md); P3 → doc/memory drift düzeltildi. Kod/migration/test DEĞİŞMEDİ. **Sıradaki:** **manuel smoke** (sent→Revize Et→`TKL-2026-001-R2` draft, valid_until boş; kaynak Revize Edildi+rozet; **R2 draft'ı edit→kaydet→reload: revision_no=2/root_quote_id korunur, "Revizyon 2" rozeti çözülür** [advisor: 071 UPDATE omission ile korur, regression testi kilitledi]; expired→revize→CRON expire ETMEZ; R2→tekrar revize R3=`-R3` [kökten taban]; draft/accepted'ta buton YOK; aynı kaynağa çift-tık→ikincisi 42501) + Faz 4 (PDF arşiv 075-076).

</details>

<details><summary>Faz 5 infra dilim — numara katmanı (`942ee0d`, migration 073 APPLY EDİLDİ)</summary>

- **Migration 073:** company_settings += quote_number_prefix/separator; quote_yearly_counters + RLS; next_quote_number() rewrite (atomik ON CONFLICT per-yıl reset + prefix); signature korundu; V7-A1 DEFINER YOK; backfill 034 defansif precedent mirror; idempotent. quote_number UNIQUE → backfill güvenli. ERTELENEN: sig rename, status CHECK (revizyona). 3821 test.
</details>
...
- **Migration tahsisi V5 (12 toplam):**
  ...
  - 076-077: Faz 7 (note_templates + sort_order)  ⟵ SUPERSEDED: final = **079 note_templates**; sort_order KALICI DÜŞÜRÜLDÜ (position zaten var)
