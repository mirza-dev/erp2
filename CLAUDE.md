...
**Son tamamlanan iş:** **Topbar yeniden tasarım — "Sakin düz" + uyarı butonunu kaldırma** (`Topbar.tsx` + `ExchangeRatesTicker.tsx` + `SystemHealthIndicator.tsx` + `globals.css`). Kullanıcı şikâyeti: topbar "çip/kalitesiz" hissi veriyor (ekran görüntüsü). **Kök neden:** çip çorbası (her öğe ayrı bordürlü pill), `ExchangeRatesTicker` kutu-içinde-kutu-içinde-kutu (ticker → rates grup → per-kur dolu pill), yeşil "Bağlı" + kırmızı "19 Uyarı" trafik-ışığı çakışması. **Kullanıcı kararları (AskUserQuestion):** (1) genel yön = **"Sakin düz"** (çip-içinde-çip kalkar, kurlar düz metin, sağlık küçük renkli nokta, sağ taraf tek hafif küme, sayfa başlığı markanın yanına/sol), (2) marka = **"Sadece yazı kalsın"** (logo/ikon yok), (3) **uyarı butonu kaldırılır** (doğrudan istek — **kayıp yok**, Sidebar'da "Uyarılar" sayacı `activeAlertCount` ile `/dashboard/alerts`'e bağlı kalır). **Uygulama:** Topbar uyarı `<Link>` bloğu + ölü `AlertTriangle`/`useData`/`activeAlertCount` silindi; başlık ortadaki pill'den çıkıp markanın yanına (`.topbar-divider` ince dikey ayraç), pill→sessiz metin; `.topbar-shell` grid→flex; sağ küme (`ExchangeRatesTicker`+`SystemHealthIndicator`) `.topbar-right-extras` tek sakin yüzeyde (subtle bg+tek border+radius). `ExchangeRatesTicker` düzleştirildi: per-kur dolu pill + dış kutu + dolu TCMB/LIVE rozeti + "A/S" etiketi kaldırıldı → kurlar düz metin (`USD 40,123 / 40,288 · EUR …`); kaynak tooltip+aria-label'da korundu (işlev kaybı yok). **ADVISOR FIX'leri:** (#1 mobil — en yüksek risk) `.topbar-right-extras` zaten `@media(max-width:768px)`'te gizlenen öğe → küme yüzeyi ona kondu, mobilde tüm küme gizli + avatar/hamburger kalır; (#2) onaylanan preview'da TCMB rozeti yoktu → görünür kaynak etiketi eklenmedi (tooltip yeterli, `feedback_no_silent_deletes` sağlandı); (#3) ayraç ticker'ın kendi `borderRight`'ında → kur null'da (yüklenemezse) sarkan separatör kalmaz. `SystemHealthIndicator` dolu yeşil/kırmızı pill→küçük renkli nokta + etiket, "checking" `pulse-dot`, lucide importları temizlendi. Backend/migration/permission/veri akışı/a11y mantığı DEĞİŞMEDİ. **+4 kaynak-regresyon** (`topbar.test.tsx`: AlertTriangle/useData/alerts-link yok, başlık sola+divider, ticker rateStyle/sourceStyle/A-S yok+borderRight, health nokta+pulse-dot) + 2 mevcut test güncellendi (`exchange-rates-ticker.test.tsx`: LIVE/TCMB görünür rozet→aria-label, 15 Uyarı→queryByText null). tsc 0 · lint 0 · 4627→4631 test · build 0 (`ƒ Proxy`). **DURUM: PUSH EDİLDİ — `origin/main == origin/codex-experiment` birebir aynı SHA; iki Coolify prod deploy tetiklendi.** Görsel kalite değerlendirmesi kullanıcı gözüyle (deploy sonrası); ince ayar turu istenirse açık.
