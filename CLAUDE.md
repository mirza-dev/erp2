...
**Son tamamlanan iş:** Teklif V7 **Faz 4 — PDF Arşiv** (gönderilmiş teklifin immutable dondurulmuş HTML snapshot'ı) **+ Bulgular review pass** (5 bulgu doğrulandı+düzeltildi), 3951 test, **COMMIT+PUSH BEKLİYOR + migration 075/076 APPLY BEKLİYOR**
...
- **DURUM: COMMIT BEKLİYOR + 075/076 APPLY BEKLİYOR.** **Sıradaki — kullanıcı:** commit+push + Supabase 075/076 apply + **manuel smoke (load-bearing, test/build kapsamı DIŞI):** teklif gönder → "Arşivlenmiş Teklif" → signed URL donmuş HTML **inline render** (download değil → Supabase Content-Disposition) + renk/font/logo doğru + A4; template değişse arşiv aynen (drift); tekrar gönder/revize → ikinci arşiv yok/revizyon ayrı; draft→buton yok. Sonra Faz 6 (077 accept→sipariş; serviceArchiveQuotePdf orada reusable, V7-A5).
