...
**Son tamamlanan iş:** Teklif V7 **Faz 4 — PDF Arşiv** (gönderilmiş teklifin immutable dondurulmuş HTML snapshot'ı) **+ Bulgular review pass** (5 bulgu doğrulandı+düzeltildi), 3951 test, **COMMIT+PUSH b8c1613 + migration 075/076 APPLY BEKLİYOR**

- **Review pass (Bulgular):** **P1 MIME** upload `text/html; charset=utf-8` ⟂ bucket `['text/html']` → her upload fail + sessiz yutma → fix contentType `"text/html"`. **P2 send arşiv** (kullanıcı kararı A): non-blocking ama görünür — `archiveWarning` flag → route → UI warning toast (sessiz değil). **P2 concurrency** serviceArchiveQuotePdf create fail → re-read existing (UNIQUE 23505 yarışı idempotent). **P2 doc drift** QUOTES_V2_PLAN.md V7-A5 Puppeteer→frozen-HTML hizalandı (Faz 6 serviceArchiveQuotePdf reuse). **P3** explicit git add. İyi: XSS temiz, no leak, totaller DB snapshot. **3880→3951** · tsc/build temiz · eslint src 31/0.
...
- **Test (+43):** quote-archive-html (13: builder+render/self-containment+Phase 0 gerçek render+V3-B6) + quote-pdf-archives (10) + quotes-faz4-archive (21: service+send-hook+route no-leak+migration+UI+Phase 0 lock). **3837 → 3880 yeşil** · tsc temiz · build OK (`ƒ Proxy` + archive route, preview hâlâ static) · lint 32 baseline/0 warning (`eslint src`; bare `eslint` `.next` artefaktı tarar).
- **DURUM: COMMIT+PUSH `b8c1613` (Faz 4 + Bulgular review pass tek commit) + 075/076 APPLY BEKLİYOR.** **Sıradaki — kullanıcı:** Supabase 075/076 apply + **manuel smoke (load-bearing, test/build kapsamı DIŞI):** teklif gönder → arşiv upload başarılı (P1 fix) → "Arşivlenmiş Teklif" → signed URL donmuş HTML **inline render** (download değil → Supabase Content-Disposition) + renk/font/logo doğru + A4; template değişse arşiv aynen (drift); tekrar gönder/revize → ikinci arşiv yok/revizyon ayrı; draft→buton yok; arşiv fail→**warning toast** (sessiz değil). Sonra Faz 6 (077 accept→sipariş; serviceArchiveQuotePdf orada reusable, V7-A5).
