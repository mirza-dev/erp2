...
**Son tamamlanan iş:** Teklif V7 **Faz 4 — PDF Arşiv** (gönderilmiş teklifin immutable dondurulmuş HTML snapshot'ı) **+ Bulgular review 1.+2.+3.+4. tur** (5+5+3+3 bulgu), 3974 test, **COMMIT+PUSH BEKLİYOR (1.+2.+3. push edildi `da09dce`) + migration 075/076 APPLY EDİLDİ ✅**
...
- **DURUM: 1.+2.+3. tur push edildi (`da09dce`) + migration 075/076 APPLY EDİLDİ ✅; 4. tur COMMIT+PUSH BEKLİYOR.** **Sıradaki — kullanıcı:** **manuel smoke (load-bearing, test/build kapsamı DIŞI):** teklif gönder → "Arşivlenmiş Teklif" → signed URL donmuş HTML **inline render** + **müşteri Adres/Address satırı dolu** + canlı önizleme adresi gösterir (drift yok) + renk/font/logo doğru + A4; template değişse arşiv aynen; tekrar gönder/revize → ikinci arşiv yok; **draft hariç Sil butonu yok** (sent→409); **bulk delete yalnız draft + başarısız satır ekranda kalır**; arşiv fail→warning toast ("otomatik denenecek" YOK); **phantom (dosya yok) → graceful 404**. Sonra Faz 6 (077 accept→sipariş; serviceArchiveQuotePdf orada reusable + object-existence recover, V7-A5).
