...
**Son tamamlanan iş:** **Roven hexagon logo — component + topbar büyütme + favicon ince ayar** (`f3cb649`→`1b760d6`→`079df51`→`7861594`). Kullanıcı görsel paylaştı: yuvarlatılmış hexagon + "Roven" wordmark. `/frontend-design`: minimal geometrik, monokrom, tema-uyumlu. **Yapılan:** Yeni `src/components/layout/RovenLogo.tsx` (tek kaynak) — hexagon `<polygon>` SVG + bold wordmark; `currentColor`/inherit → otomatik temalanır; Props `size/showWordmark/wordmarkSize/gap` (varsayılan gap 6). Topbar iki tur büyütme → `size=24 wordmarkSize=19 gap=7` (sayfaya oranla görünür, 52px bar güvenli). Landing + login (kullanıcı redesign'ı `28/22`) da RovenLogo. **`src/app/icon.svg` favicon: kutusuz + tema-uyumlu** (koyu kutu kaldırıldı; SVG-içi `prefers-color-scheme` → koyu sekme `#e6edf3`/açık `#1f2937`; favicon agresif cache → hard-refresh). **+10 test** (`roven-logo.test.tsx`). tsc 0 · lint 0 · 4688 test · build 0. **DURUM: PUSH EDİLDİ (`7861594`) — birebir aynı SHA; her tur iki Coolify deploy.** ⚠️ Push dersi: `git merge --ff-only` mutlaka `cd erp2` içinde — bir turda atlandı, kullanıcının **login redesign** commit'i (`fee0943`) + topbar birlikte main'e ff edildi (mirror geri sağlandı; login redesign artık main prod'da).
