-- KokpitERP — Inventory Engine RPCs
-- Atomic inventory operations: stock movement, production completion,
-- shortage resolution, and reservation audit trails.
-- Run after 003_order_rpcs.sql
...
-- ════════════════════════════════════════════════════════════════
-- 5. approve_order_with_allocation() — updated with movement audit
--    CREATE OR REPLACE over 003's version; adds reservation_create
...
begin
    ...
    loop
        ...
        if v_reservable > 0 then
            ...
            -- ★ NEW: reservation_create movement for audit trail
            insert into inventory_movements (product_id, movement_type, quantity, reference_type, reference_id, notes, source)
            values (
                v_line.product_id,
                'reservation_create',
...
-- ════════════════════════════════════════════════════════════════
-- 6. cancel_order() — updated with movement audit
--    CREATE OR REPLACE over 003's version; adds reservation_release
--    movement records for audit trail
-- ════════════════════════════════════════════════════════════════

-- ════════════════════════════════════════════════════════════════
-- 5b. reverse_production() RPC
...
begin
    ...
    loop
        ...
        -- ★ NEW: reservation_release movement for audit trail
        insert into inventory_movements (product_id, movement_type, quantity, reference_type, reference_id, notes, source)
        values (
            v_res.product_id,
            'reservation_release',
