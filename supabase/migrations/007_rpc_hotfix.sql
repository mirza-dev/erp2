-- KokpitERP — RPC Hotfix (idempotent consolidation)
-- Combines 003_order_rpcs + 004_inventory_rpcs final RPC versions.
-- Safe to run regardless of DB state — all CREATE OR REPLACE.
-- Run via: Supabase Dashboard → SQL Editor → paste → Run

-- ════════════════════════════════════════════════════════════════
-- 1. order_counters (generate_order_number dependency)
-- ════════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════════
-- 3. approve_order_with_allocation() — 004 final version
--    Line-by-line partial allocation + shortage records
--    + reservation_create movement audit trail
-- ════════════════════════════════════════════════════════════════
...
begin
    ...
    loop
        ...
        if v_reservable > 0 then
            ...
            -- reservation_create movement for audit trail
            ...
            values (
                ...
                'order',
                p_order_id,
                format('Sipariş onayı: %s birim ayrıldı', v_reservable),
                'system'
            );
...
-- ════════════════════════════════════════════════════════════════
-- 5. cancel_order() — 004 final version
--    Release reservations + cancel shortages + guard shipped
--    + reservation_release movement audit trail
-- ════════════════════════════════════════════════════════════════
...
begin
    ...
    loop
        ...
        -- reservation_release movement for audit trail
        insert into inventory_movements (product_id, movement_type, quantity, reference_type, reference_id, notes, source)
        values (
            v_res.product_id,
            'reservation_release',
            ...
            'order',
            p_order_id,
            format('Sipariş iptali: %s birim serbest bırakıldı', v_res.reserved_qty),
            'system'
        );
    end loop;
