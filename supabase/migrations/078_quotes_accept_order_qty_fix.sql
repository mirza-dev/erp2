-- ============================================================
-- 078 — Faz 6 Bulgular: accept RPC quantity pre-check düzeltmesi (P2/P3)
-- 077'nin accept_quote_and_create_order RPC'sini CREATE OR REPLACE eder.
-- TEK değişiklik: quantity pre-check'i `<> trunc` (yalnız küsürat) → `<= 0 OR
-- <> trunc` (pozitif tam sayı). Önceki sürümde legacy/bypass quantity=0 veya
-- negatif satır pre-check'ten geçip order_lines `check (quantity > 0)`
-- constraint'ine takılıyor, 23514 (check_violation) üretiyordu; service bunu
-- yanlışlıkla "Teklif arşivi bulunamadı" diye map ediyordu. Artık 22003 ile
-- erken + doğru mesajla yakalanır. (Service tarafı 23514→archive map'i kaldırıldı
-- → kalan check ihlalleri dürüstçe 500.)
-- 077 zaten apply edildi → bu append-only düzeltme. İdempotent (CREATE OR REPLACE).
-- ============================================================
...
BEGIN
    ...
    --    `<= 0` eklendi: order_lines check(quantity>0) constraint'ine düşmeden,
    --    22003 ile doğru mesajla yakala (legacy/bypass 0 veya negatif satır).
    IF EXISTS (SELECT 1 FROM quote_line_items
    ...
    -- 6. Taslak sipariş — DONMUŞ totaller kopyalanır (recompute YOK). Müşteri
    --    country/tax LEFT JOIN ile. Domain §4.4: draft order → rezervasyon YOK.
...
-- ── ROLLBACK ──────────────────────────────────────────────────────────────────
-- 077'deki sürüme geri dönmek için 077_quotes_accept_order.sql'i yeniden çalıştırın
-- (CREATE OR REPLACE). Kolon değişikliği YOK.
