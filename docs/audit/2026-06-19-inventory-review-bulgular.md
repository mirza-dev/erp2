...
- **Migration 105 APPLY EDİLDİ ✅** (2026-06-19, kullanıcı). `check-migrations.ts` MANUAL
  map'ine 105 eklendi (otomatik probe yok → ⚠️ elle doğrula satırı:
  `SELECT prosrc LIKE '%for update%' FROM pg_proc WHERE proname='recount_stock'`).
